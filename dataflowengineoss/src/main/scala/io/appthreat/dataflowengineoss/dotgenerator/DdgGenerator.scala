...
class DdgGenerator:
  ...
  def generate(methodNode: Method)(implicit semantics: Semantics = DefaultSemantics()): Graph =
    ...
    val ddgNodes = visibleNodes
        ...
        .map(surroundingCall)
        .filterNot(node =>
            node.isInstanceOf[Call] && isGenericMemberAccessName(node.asInstanceOf[Call].name)
        )
    ...
    val ddgEdges = edges.flatten
        ...
        .filterNot(e =>
            e.dst.isInstanceOf[Call] && isGenericMemberAccessName(e.dst.asInstanceOf[Call].name)
        )
        .filterNot(e =>
            e.src.isInstanceOf[Call] && isGenericMemberAccessName(e.src.asInstanceOf[Call].name)
        )
        .distinct
  ...
  private def inEdgesToDisplay(dstNode: StoredNode, visited: List[StoredNode] = List())(implicit
    ...
    if edgeCache.contains(dstNode) then
      return edgeCache(dstNode)

    if visited.contains(dstNode) then
      List()
    else
      val parents = expand(dstNode)
      val (visible, invisible) =
          parents.partition(x => shouldBeDisplayed(x.src) && x.srcVisible)
      val result = visible.toList ++ invisible.toList.flatMap { n =>
        ...
        parentInEdgesToDisplay.map(y =>
            Edge(y.src, dstNode, y.srcVisible, edgeType = edgeType, label = y.label)
        )
      }.distinct
      edgeCache.put(dstNode, result)
      result
  end inEdgesToDisplay
  ...
  private def expand(v: StoredNode)(implicit semantics: Semantics): Iterator[Edge] =
    ...
    val allInEdges = v
        ...
        .map(x =>
            Edge(
              x.outNode.asInstanceOf[StoredNode],
              v,
              srcVisible = true,
              x.property(Properties.VARIABLE),
              edgeType
            )
        )
    ...
    v match
      case cfgNode: CfgNode =>
          cfgNode
              ...
              .map(x =>
                  Edge(
                    x.node.asInstanceOf[StoredNode],
                    v,
                    x.visible,
                    x.outEdgeLabel,
                    edgeType
