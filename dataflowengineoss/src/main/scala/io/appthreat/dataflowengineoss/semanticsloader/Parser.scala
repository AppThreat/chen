package io.appthreat.dataflowengineoss.semanticsloader

import io.appthreat.dataflowengineoss.SemanticsParser.MappingContext
import io.appthreat.dataflowengineoss.{SemanticsBaseListener, SemanticsLexer, SemanticsParser}
...
class Semantics private (methodToSemantic: mutable.Map[String, FlowSemantic]):
    ...
    /** The map below keeps a mapping between results of a regex and the regex string it matches.
      * e.g.
      *
    ...
    /** Initialize all the method semantics that use regex with all their regex results before query
      * time.
      */
    ...
    def forMethod(fullName: String): Option[FlowSemantic] =
        regexMatchedFullNames.get(fullName) match
    ...
    def from(methodFullName: String, mappings: List[?], regex: Boolean = false): FlowSemantic =
        FlowSemantic(
          ...
          mappings.map {
              ...
              case (srcIdx: Int, src: String, dstIdx: Int, dst: String) =>
                  FlowMapping(srcIdx, src, dstIdx, dst)
...
/** Collects parameters and return nodes under a common trait. This trait acknowledges their
  * argument index which is relevant when a caller wants to coordinate relevant tainted flows
  * through specific arguments and the return flow.
  */
...
/** A parameter where the index of the argument matches the position of the parameter at the callee.
  * The name is used to match named arguments if used instead of positional arguments.
  *
  * @param index
...
object FlowMapping:
    ...
    def apply(from: Int, toIdx: Int, to: String): FlowMapping =
        FlowMapping(ParameterNode(from), ParameterNode(toIdx, to))

/** Represents an instance where parameters are not sanitized, may affect the return value, and do
  * not cross-taint. e.g. foo(1, 2) = 1 -> 1, 2 -> 2, 1 -> -1, 2 -> -1
  *
  * The main benefit is that this works for unbounded parameters e.g. VARARGS. Note this does not
  * taint 0 -> 0.
  */
