...
import io.appthreat.dataflowengineoss.queryengine.{PathElement}
...
class ExtendedCfgNodeMethods[NodeType <: CfgNode](val node: NodeType) extends AnyVal:
    ...
    /** Traverse back in the data dependence graph by one step and generate corresponding
      * PathElement, taking into account semantics
      * @param path
      *   optional list of path elements that have been expanded already
      */
    def ddgInPathElem(
    ...
    private def ddgInPathElemInternal(
      ...
      cache: mutable.HashMap[CfgNode, Vector[PathElement]]
        ...
        val result = if withInvisible then
            elems
        else
            (elems.filter(_.visible) ++ elems
                .filterNot(_.visible)
                .flatMap(x =>
                    x.node.asInstanceOf[CfgNode].ddgInPathElem(
                      x +: path,
                      withInvisible = false,
                      cache
                    )
        ...
        result
    end ddgInPathElemInternal
end ExtendedCfgNodeMethods
