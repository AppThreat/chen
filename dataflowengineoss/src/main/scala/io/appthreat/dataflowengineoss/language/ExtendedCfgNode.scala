...
import io.appthreat.dataflowengineoss.queryengine.*
import SourcesToStartingPoints.sourceTravsToStartingPoints
...
class ExtendedCfgNode(val traversal: Iterator[CfgNode]) extends AnyVal:
    ...
    private def removeConsecutiveDuplicates[T](l: Vector[T]): List[T] =
        l.headOption.map(x =>
            x :: l.sliding(2).collect { case Seq(a, b) if a != b => b }.toList
        ).getOrElse(List())
    ...
    private def reachableByInternal(
      startingPointsWithSources: List[StartingPointWithSource]
        ...
        val res = result.par.map { r =>
            ...
            if sources.contains(startingPoint) || !startingPointToSource(
                  startingPoint
                ).isInstanceOf[AstNode]
            then
                r
            else
                r.copy(path =
                    PathElement(
                      startingPointToSource(startingPoint).asInstanceOf[AstNode]
                    ) +: r.path
                )
        }
        res.toVector
    end reachableByInternal
end ExtendedCfgNode
