...
class ReachingDefPass(cpg: Cpg, maxNumberOfDefinitions: Int = 4000)(implicit s: Semantics)
    ...
    /** Before we start propagating definitions in the graph, which is the bulk of the work, we
      * check how many definitions were are dealing with in total. If a threshold is reached, we
      * bail out instead, leaving reaching definitions uncalculated for the method in question.
      * Users can increase the threshold if desired.
      */
