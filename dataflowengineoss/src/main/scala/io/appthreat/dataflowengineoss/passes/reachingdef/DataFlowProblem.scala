...
/** A general data flow problem, formulated as in the Dragon Book, Second Edition on page 626, with
  * mild modifications. In particular, instead of allowing only for the specification of a boundary,
  * we allow initialization of IN and OUT.
  */
...
/** In essence, the flow graph is the control flow graph, however, we can compensate for small
  * deviations from our generic control flow graph to one that is better suited for solving data
  * flow problems. In particular, method parameters are not part of our normal control flow graph.
  * By defining successors and predecessors, we provide a wrapper that takes care of these minor
  * discrepancies.
  */
...
/** As a practical optimization, OUT[N] is often initialized to GEN[N]. Moreover, we need a way of
  ...
  * flow problem to specify initializers for IN and OUT.
  */
