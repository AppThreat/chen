...
import io.appthreat.dataflowengineoss.semanticsloader.{PassThroughMapping, Semantics}
import io.appthreat.dataflowengineoss.language.*
import io.appthreat.dataflowengineoss.queryengine.Engine.isOutputArgOfInternalMethod
...
object EdgeValidator:
    ...
    /** Determines whether the edge from `parentNode`to `childNode` is valid, according to the given
      * semantics.
      */
        ...
        (childNode, parentNode) match
            ...
            case (childNode: Expression, parentNode: Expression)
                if parentNode.isArgToSameCallWith(
                  childNode
                ) && childNode.isDefined && parentNode.isUsed =>
