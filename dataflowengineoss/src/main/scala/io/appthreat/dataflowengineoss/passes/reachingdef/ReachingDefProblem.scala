...
/** The variables defined/used in the reaching def problem can all be represented via nodes in the
  * graph, however, that's pretty confusing because it is then unclear that variables and nodes are
  * actually two separate domains. To make the definition domain visible, we use the type alias
  * `Definition`. From a computational standpoint, this is not necessary, but it greatly improves
  * readability.
  */
object Definition:
...
class ReachingDefFlowGraph(val method: Method) extends FlowGraph[StoredNode]:
    ...
    val numberToNode: Map[Int, StoredNode] =
        allNodesEvenUnreachable.zipWithIndex.map { case (x, i) => i -> x }.toMap
    ...
    private val _pred: Map[StoredNode, List[StoredNode]] =
        initPred(allNodesReversePostOrder, method)
    ...
    private def initPred(ns: List[StoredNode], method: Method): Map[StoredNode, List[StoredNode]] =
        ns.map {
            ...
            case paramOut: MethodParameterOut =>
                paramOut -> previousOutputParamOrLastNodeOfBody(paramOut)
            case n: CfgNode if method.cfgFirst.headOption.contains(n) =>
                n -> List(lastParam.getOrElse(method))
    ...
    private def nextParamOrBody(param: MethodParameterIn): List[StoredNode] =
        ...
        if nextParam.isDefined then nextParam.toList
    ...
    private def previousParamOrEntry(param: MethodParameterIn): List[StoredNode] =
        ...
        if prevParam.isDefined then prevParam.toList
        else List(method)

    private def previousOutputParamOrLastNodeOfBody(paramOut: MethodParameterOut)
      : List[StoredNode] =
    ...
    private def lastOutputParamOrLastNodeOfBody(): List[StoredNode] =
        if lastOutputParam.isDefined then lastOutputParam.toList
        else lastActualCfgNode.toList
end ReachingDefFlowGraph

/** For each node of the graph, this transfer function defines how it affects the propagation of
  * definitions.
  */
class ReachingDefTransferFunction(flowGraph: ReachingDefFlowGraph)
    ...
    /** For a given flow graph node `n` and set of definitions, apply the transfer function to
      * obtain the updated set of definitions, considering `gen(n)` and `kill(n)`.
      */
    override def apply(n: StoredNode, x: mutable.BitSet): mutable.BitSet =
        gen(n).union(x.diff(kill(n)))

    /** Initialize the map `gen`, a map that contains generated definitions for each flow graph
      * node.
      */
    ...
    /** Initialize the map `kill`, a map that contains killed definitions for each flow graph node.
      *
      * All operations in our graph are represented by calls and non-operations such as identifiers
      * or field-identifiers have empty gen and kill sets, meaning that they just pass on
    ...
    /** The only way in which a call can kill another definition is by generating a new definition
      * for the same variable. Given the set of generated definitions `gens`, we calculate
      * definitions of the same variable for each, that is, we calculate kill(call) based on
      * gen(call).
      */
...
/** Lone Identifier Optimization: we first determine and store all identifiers that neither refer to
  * a local nor a parameter and that appear only once as a call argument and not also in a return
  * statement. For these identifiers, we know that they are not used in any other location in the
  * code, and so, we remove them from `gen` sets so that they need not be propagated through the
  * entire graph only to determine that they reach the exit node. Instead, when creating reaching
  * definition edges, we simply create edges from the identifier to the exit node.
  */
class OptimizedReachingDefTransferFunction(flowGraph: ReachingDefFlowGraph)
    ...
    lazy val loneIdentifiers: Map[Call, List[Definition]] =
        ...
        callArgPairs
            .groupBy(_._1)
            .collect {
                case (_, v) if v.size == 1 => v.map { case (_, call, arg) => (call, arg) }.head
            }
            .toList
            .groupBy(_._1)
            .map { case (k, v) =>
    ...
    private def withoutLoneIdentifiers(g: Map[StoredNode, mutable.BitSet])
      : Map[StoredNode, mutable.BitSet] =
        g.map { case (k, defs) =>
            k match
...
class ReachingDefInit(gen: Map[StoredNode, mutable.BitSet])
    extends InOutInit[StoredNode, mutable.BitSet]:
