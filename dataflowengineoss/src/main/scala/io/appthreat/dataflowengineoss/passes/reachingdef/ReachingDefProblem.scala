...
class ReachingDefTransferFunction(flowGraph: ReachingDefFlowGraph)
  ...
  /** For a given flow graph node `n` and set of definitions, apply the transfer function to obtain
    * the updated set of definitions, considering `gen(n)` and `kill(n)`.
    */
  ...
  /** Initialize the map `kill`, a map that contains killed definitions for each flow graph node.
    *
    * All operations in our graph are represented by calls and non-operations such as identifiers or
    * field-identifiers have empty gen and kill sets, meaning that they just pass on definitions
    * unaltered.
    */
  ...
  /** The only way in which a call can kill another definition is by generating a new definition for
    * the same variable. Given the set of generated definitions `gens`, we calculate definitions of
    * the same variable for each, that is, we calculate kill(call) based on gen(call).
    */
