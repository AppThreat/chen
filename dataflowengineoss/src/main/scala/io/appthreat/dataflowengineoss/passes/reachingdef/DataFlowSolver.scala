...
class DataFlowSolver:
    ...
    /** Calculate fix point solution via a standard work list algorithm (Forwards). The result is
    ...
    /** Calculate fix point solution via a standard work list algorithm (Backwards). The result is
      * given by two maps: `in` and `out`. These maps associate all CFG nodes with the set of
      * definitions at node entry and node exit respectively.
      */
    def calculateMopSolutionBackwards[Node, T <: Iterable[?]](problem: DataFlowProblem[Node, T])
      : Solution[Node, T] =
        ...
        Solution(in, out, problem)
    end calculateMopSolutionBackwards
end DataFlowSolver
