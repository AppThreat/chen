...
object JavaFrameworkSemantics:
  ...
  /** Request-data accessors of the servlet/Spring request objects.
    ...
    * (parameter 0) to return (-1).
    *
    * Two lists serve the two resolution states of a Java graph:
    *   - [[fullNames]] match RESOLVED calls, whose `methodFullName` is fully qualified - precise,
    *     and part of the JVM-language-gated list below.
    *   - [[callNames]] are BARE names for UNRESOLVED calls (the common configuration, where the
    *     servlet jar is not on the inference classpath). A declared semantic is authoritative, so a
    *     bare name also applies to an unrelated same-named method in the same graph; the list is
    *     therefore restricted to request-shaped names, and generic ones (`body`, `getAttribute`,
    *     `getRemoteAddr`, ...) are deliberately absent.
    *
    * Both are gated on JVM languages through `DefaultSemantics.flowsForLanguage`, exactly like the
    * PHP sanitizer names.
    */
  object RequestReaders:
    /** Bare names, for unresolved calls. */
    ...
    /** Receivers whose accessors return request data, for resolved calls. */
    val receiverFullNames: Set[String] = Set(
    ...
    /** The accessor names; on a RESOLVED receiver even the generic ones (`getAttribute`,
      * `getRemoteAddr`, `body`) are request data, so they are only in this combined set.
      */
    val accessorNames: Set[String] =
        callNames ++ Set("getAttribute", "getRemoteAddr", "getRemoteHost", "body")

    /** Every resolved-shape entry: `receiver.accessor` for each combination. Consumed as a REGEX
      * semantic with a `:.*` suffix, because a resolved call's methodFullName carries a
      * `:signature` that varies across overloads.
      */
    def fullNames: Set[String] =
        for receiver <- receiverFullNames; accessor <- accessorNames yield s"$receiver.$accessor"
  end RequestReaders
