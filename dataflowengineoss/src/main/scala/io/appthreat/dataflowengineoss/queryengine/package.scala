...
package object queryengine:
    ...
    /** We represent data flows as sequences of path elements, where each path element consists of a
      ...
      *
      * @param callSiteStack
      *   The call stack when this path element was created. Since we may enter the same function
      *   via two different call sites, path elements should only be treated as the same if they are
      *   the same node and we've reached them via the same call sequence.
      *
      * @param visible
      *   whether this path element should be shown in the flow
      * @param isOutputArg
      *   input and output arguments are the same node in the CPG, so, we need this additional flag
      *   to determine whether we are on an input or output argument. By default, we consider
      *   arguments to be input arguments, meaning that when tracking `x` at `f(x)`, we do not
      *   expand into `f` but rather upwards to producers of `x`.
      * @param outEdgeLabel
    ...
    /** @param taskStack
      *   The list of tasks that was solved to arrive at this task, including the current task,
      *   which is to be solved. The current task is the last element of the list.
      *
      * @param initialPath
    ...
    case class ReachableByTask(taskStack: List[TaskFingerprint], initialPath: Vector[PathElement]):
        ...
        /** The call sites we have expanded downwards during this analysis. We need to keep track of
          * this so that we do not end up expanding one call site and then returning to a different
          * call site, which would produce an unreachable path.
          */
        def callSiteStack: List[Call] = fingerprint.callSiteStack
