...
class TaskCreator(context: EngineContext):
    ...
    private def removeTasksWithLoopsAndTooHighCallDepth(tasks: Vector[ReachableByTask])
      : Vector[ReachableByTask] =
        val tasksWithValidCallDepth = if context.config.maxCallDepth == -1 then
            tasks
        else
    ...
    /** Create new tasks from all results that start in a parameter. In essence, we want to traverse
      * to corresponding arguments of call sites, but we need to be careful here not to create
      * unrealizable paths. We achieve this by holding a call stack in results.
      *
      * Case 1: we expanded into a callee that we identified on the way, e.g., a method `y =
      * transform(x)`, and we have reached the parameter of that method (`transform`). Upon doing
      * so, we recorded the call site that we expanded in `result.callSite`. We would now like to
      * continue exploring from the corresponding argument at that call site only.
      *
      * Case 2: walking backward from the sink, we have only expanded into callers so far, that is,
      * the call stack is empty. In this case, the next tasks need to explore each call site to the
      * method.
      */
    ...
    private def startsAtParameter(results: Vector[ReachableByResult]) =
        results.collect {
            case r: ReachableByResult if r.path.head.node.isInstanceOf[MethodParameterIn] => r
        }

    /** For a given parameter of a method, determine all corresponding arguments at all call sites
      * to the method.
      */
    ...
    /** Create new tasks from all results that end in an output argument, including return
      * arguments. In this case, we want to traverse to corresponding method output parameters and
      * method return nodes respectively.
      */
    private def tasksForUnresolvedOutArgs(results: Vector[ReachableByResult])
      : Vector[ReachableByTask] =
                    ...
                    methodReturns.flatMap { case (call, methodReturn) =>
                        ...
                        if method.isExternal || method.start.isStub.nonEmpty then
                            ...
                            returnStatements.map { returnStatement =>
                                val newPath =
                                    Vector(PathElement(methodReturn, result.callSiteStack)) ++ path
                                val taskStack =
        ...
        val forArgs = outArgsAndCalls.flatMap { case (result, args, path, callDepth) =>
            args.toList.flatMap {
                case arg: Expression =>
                    val outParams = if result.callSiteStack.nonEmpty then
