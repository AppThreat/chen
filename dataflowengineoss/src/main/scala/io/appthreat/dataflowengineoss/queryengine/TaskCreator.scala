...
class TaskCreator(context: EngineContext):
  ...
  /** Create new tasks from all results that start in a parameter. In essence, we want to traverse
    ...
    * transform(x)`, and we have reached the parameter of that method (`transform`). Upon doing so,
    * we recorded the call site that we expanded in `result.callSite`. We would now like to continue
    * exploring from the corresponding argument at that call site only.
    *
  ...
  /** For a given parameter of a method, determine all corresponding arguments at all call sites to
    * the method.
    */
