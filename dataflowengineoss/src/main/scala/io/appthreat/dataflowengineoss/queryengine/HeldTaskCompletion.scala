...
class HeldTaskCompletion(
  ...
  resultTable: mutable.Map[TaskFingerprint, List[TableEntry]]
    ...
    /** Add results produced by held task until no more change can be observed.
      ...
      * for the `task` were produced. We initialize the Booleans to be true. Computation is
      * terminated when all Booleans are false, that is, when no more changes in the result table
      * can be observed.
      *
      * If we do detect a change, we determine all tasks for which changed results exist and
      * recompute their results. We compare the results with those produced previously (stored in
    ...
    /** In essence, completing a held task simply means appending the path stored in the held task
      * to all results that are available for the held task in the table. In practice, we create one
      * result for each task of the parent task's `taskStack`, so that we do not only get a new
      * result for the sink, but one for each of the parent nodes on the way.
      */
    ...
    /** This method creates a list of results from a held task and a table entry by appending paths
      * of the held task to the path stored in the held task (`initialPath`) up to each of its
      * parent tasks.
      *
    ...
    /** This method deduplicates the list of entries stored in a table cell.
      ...
      *
      * For a group of flows that we treat as the same, we select the flow with the maximum length.
      * If there are multiple flows with maximum length, then we compute a string representation of
      * the flows - taking into account all fields
