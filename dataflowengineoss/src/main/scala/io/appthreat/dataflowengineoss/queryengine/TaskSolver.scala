...
import io.appthreat.dataflowengineoss.queryengine.QueryEngineStatistics.{
    PATH_CACHE_HITS,
    PATH_CACHE_MISSES
}
import io.appthreat.dataflowengineoss.semanticsloader.Semantics
...
/** Callable for solving a ReachableByTask
  *
  * A Java Callable is "a task that returns a result and may throw an exception", and this is the
  * callable for calculating the result for `task`.
  *
  * @param task
...
class TaskSolver(task: ReachableByTask, context: EngineContext, sources: Set[CfgNode])
    ...
    /** Entry point of callable. First checks if the maximum call depth has been exceeded, in which
      * case an empty result list is returned. Otherwise, the task is solved and its results are
      * returned.
      */
    override def call(): TaskSummary =
        ...
        val finalResults = table.get(task.fingerprint).get.map { r =>
            r.copy(
              taskStack =
                  r.taskStack.dropRight(1) :+ r.fingerprint.copy(callDepth = task.callDepth),
    ...
    /** Recursively expand the DDG backwards and return a list of all results, given by at least a
      * source node in `sourceSymbols` and the path between the source symbol and the sink.
      *
      * This method stays within the method (intra-procedural analysis) and terminates at method
      * parameters and at output arguments.
      *
      * @param path
      *   This is a path from a node to the sink. The first node of the path is expanded by this
      *   method
      *
      * @param table
      ...
      *
      * @param callSiteStack
      *   This stack holds all call sites we expanded to arrive at the generation of the current
      *   task
      */
    private def results[NodeType <: CfgNode](
      ...
      callSiteStack: List[Call]
        ...
        /** For each parent of the current node, determined via `expandIn`, check if results are
        ...
        def createResultsFromCacheOrCompute(elemToPrepend: PathElement, path: Vector[PathElement]) =
            val cachedResult =
                createFromTable(table, elemToPrepend, task.callSiteStack, path, task.callDepth)
            if cachedResult.isDefined then
        ...
        /** For a given path, determine whether results for the first element (`first`) are stored
          * in the table, and if so, for each result, determine the path up to `first` and prepend
          * it to `path`, giving us new results via table lookup.
          */
        def createFromTable(
          table: mutable.Map[TaskFingerprint, Vector[ReachableByResult]],
          first: PathElement,
        ...
        val res = curNode match
            ...
            case x if sources.contains(x.asInstanceOf[NodeType]) =>
                if x.isInstanceOf[MethodParameterIn] then
                    Vector(
