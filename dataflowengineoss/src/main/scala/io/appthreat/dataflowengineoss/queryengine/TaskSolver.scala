...
class TaskSolver(task: ReachableByTask, context: EngineContext, sources: Set[CfgNode])
  ...
  private def results[NodeType <: CfgNode](
    ...
    /** For a given path, determine whether results for the first element (`first`) are stored in
      * the table, and if so, for each result, determine the path up to `first` and prepend it to
