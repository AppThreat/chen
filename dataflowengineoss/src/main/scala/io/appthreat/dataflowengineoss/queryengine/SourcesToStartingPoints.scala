...
object SourcesToStartingPoints:
  ...
  def sourceTravsToStartingPoints[NodeType](sourceTravs: IterableOnce[NodeType]*)
...
/** The code below deals with member variables, and specifically with the situation where literals
  * that initialize static members are passed to `reachableBy` as sources. In this case, we
  * determine the first usages of this member in each method, traversing the AST from left to right.
  * This isn't fool-proof, e.g., goto-statements would be problematic, but it works quite well in
  * practice.
  */
class SourceToStartingPoints(src: StoredNode) extends RecursiveTask[List[CfgNode]]:
      ...
      src match
        ...
        case x: Identifier =>
            (withFieldAndIndexAccesses(
              List(x).collectAll[CfgNode].toList ++ x.refsTo.collectAll[Local].flatMap(
                sourceToStartingPoints
              )
            ) ++ x.refsTo.capturedByMethodRef.referencedMethod.flatMap(m =>
                usagesForName(x.name, m)
            )).flatMap {
      ...
      astNode match
        ...
        case fieldIdentifier: FieldIdentifier =>
            val fieldIdentifiers =
                m.ast.isFieldIdentifier.sortBy(x => (x.lineNumber, x.columnNumber)).l
            fieldIdentifiers
                .canonicalNameExact(fieldIdentifier.canonicalName)
                .inFieldAccess
                // TODO `isIdentifier` seems to limit us here
                .where(_.argument(1).isIdentifier.or(
                  _.nameExact("this", "self"),
                  _.typeFullNameExact(typeDecl.fullName)
                ))
                .takeWhile(notLeftHandOfAssignment)
                .l
        case _ => List()
  ...
  /** For a literal, determine if it is used in the initialization of any member variables. Return
    * list of initialized members. An initialized member is either an identifier or a
    * field-identifier.
    */
      ...
      lit.inAssignment
          ...
          .flatMap {
              case identifier: Identifier
                  ...
                  ) == identifier.method.typeDecl.fullName.headOption ||
