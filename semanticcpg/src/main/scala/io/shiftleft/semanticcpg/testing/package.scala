    ...
    def withMethod(
      ...
      fileName: String = ""
        ...
        withCustom { (graph, _) =>
          ...
          val method =
              NewMethod().isExternal(external).name(name).fullName(name).signature(
                "asignature"
              ).filename(fileName)
