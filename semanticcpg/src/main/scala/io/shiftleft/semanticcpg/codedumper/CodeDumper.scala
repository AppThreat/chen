...
object CodeDumper:
    ...
    def arrow(locationFullName: Option[String] = None): CharSequence =
        s"/* <=== ${locationFullName.getOrElse("")} */ "
      ...
      withArrow: Boolean = true
        ...
        (location.node, language) match
            case (None, _) =>
                ...
                ""
            case (_, None) =>
                logger.debug("dump not supported; language not set in CPG")
                ""
            case (_, Some(lang)) if !supportedLanguages.contains(lang) =>
                logger.debug(s"dump not supported for language '$lang'")
                ""
                ...
                method
                    .collect {
                        case m: Method if m.lineNumber.isDefined && m.lineNumberEnd.isDefined =>
                            val rawCode = if lang == Languages.GHIDRA || lang == Languages.JAVA then
                                val lines = m.code.split("\n")
                                lines.zipWithIndex
                                    .map { case (line, lineNo) =>
                                        if lineNo == 0 && withArrow then
                                ...
                                val filename = rootPath.map(
                                  toAbsolutePath(location.filename, _)
                                ).getOrElse(location.filename)
                                code(
                                  filename,
                                  m.lineNumber.get,
                                  m.lineNumberEnd.get,
    ...
    /** For a given `filename`, `startLine`, and `endLine`, return the corresponding code by reading
      * it from the file. If `lineToHighlight` is defined, then a line containing an arrow (as a
      * source code comment) is included right before that line.
      */
    def code(
      filename: String,
      startLine: Integer,
