...
object DotSerializer:
    ...
    private def limit(str: String): String = if str.length > charLimit then
    ...
    private def stringRepr(vertex: StoredNode): String =
        ...
        escape(vertex match
            ...
            case contrl: ControlStructure =>
                (contrl.label, contrl.controlStructureType, contrl.code).toString
            case expr: Expression =>
                (expr.label, limit(expr.code), limit(toCfgNode(expr).code)).toString
            ...
            case annoAssign: AnnotationParameterAssign =>
                (annoAssign.label, annoAssign.code).toString()
            ...
            case _                              => ""
        ) + (if maybeLineNo.isPresent then s"<SUB>${maybeLineNo.get()}</SUB>" else "")
    end stringRepr
    ...
    private def edgeToDot(edge: Edge, withEdgeTypes: Boolean): String =
        val edgeLabel = if withEdgeTypes then
            edge.edgeType + ": " + escape(edge.label)
        else
            escape(edge.label)
        val labelStr =
            Some(s""" [ label = "$edgeLabel"] """).filter(_ => edgeLabel != "").getOrElse("")
    ...
    private def escapedChar(ch: Char): String = ch match
        ...
        case '&' => "&amp;"
        case _ =>
            if ch.isControl then "\\0" + Integer.toOctalString(ch.toInt)
