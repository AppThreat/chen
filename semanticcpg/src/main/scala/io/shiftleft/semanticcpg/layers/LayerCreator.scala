...
abstract class LayerCreator:
  ...
  /** If the LayerCreator modifies the CPG, then we store its name in the CPGs metadata and disallow
    * rerunning the creator, that is, applying the layer twice.
    */
  ...
  protected def initSerializedCpg(
    ...
    index: Int = 0
      ...
      outputDir match
        case Some(dir) =>
            new SerializedCpg((File(dir) / s"${index}_$passName").path.toAbsolutePath.toString)
