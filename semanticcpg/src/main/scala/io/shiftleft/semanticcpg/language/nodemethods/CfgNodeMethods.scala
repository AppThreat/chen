...
class CfgNodeMethods(val node: CfgNode) extends AnyVal with NodeExtension:
    ...
    /** Using the post dominator tree, will determine if this node passes through the included set
      * of nodes and filter it in.
      * @param included
    ...
    /** Using the post dominator tree, will determine if this node passes through the excluded set
      * of nodes and filter it out.
      * @param excluded
    ...
    /** Obtain hexadecimal string representation of lineNumber field.
      ...
      * with other binary analysis tooling, it is convenient to allow retrieving these as hex
      * strings.
      */
