...
class Steps[A](val traversal: Iterator[A]) extends AnyVal:
    ...
    /** Execute this traversal and pretty print the results. This may mean that not all properties
      * of the node are displayed or that some properties have undergone transformations to improve
