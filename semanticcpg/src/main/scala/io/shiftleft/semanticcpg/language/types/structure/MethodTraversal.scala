...
class MethodTraversal(val traversal: Iterator[Method]) extends AnyVal:
    ...
    /** The type declaration associated with this method, e.g., the class it is defined in. Alias
      * for 'definingTypeDecl'
      */
    ...
    /** Traverse only to methods that are stubs, e.g., their code is not available or the method
      * body is empty.
      */
