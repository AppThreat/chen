...
class AstNodeTraversal[A <: AstNode](val traversal: Iterator[A]) extends AnyVal:
    ...
    /** All nodes of the abstract syntax tree rooted in this node, which match `predicate`.
      * Equivalent of `match` in the original CPG paper.
      */
    ...
    /** Nodes of the AST obtained by expanding AST edges backwards until `root` or the method root
      * is reached
      */
