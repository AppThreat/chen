...
object AccessPathHandling:
  ...
  def leafToTrackedBaseAndAccessPathInternal(node: StoredNode)
    : Option[(TrackedBase, List[AccessElement])] =
      node match
  ...
  def memberAccessToPath(memberAccess: Call, tail: List[AccessElement]): List[AccessElement] =
      memberAccess.name match
        case Operators.memberAccess | Operators.indirectMemberAccess =>
            if !hasWarnedDeprecations then
              logger.debug(s"Deprecated Operator ${memberAccess.name} on $memberAccess")
            ...
            memberAccess
                ...
                .collect {
                    ...
                    case other if other.propertyOption(PropertyNames.NAME).isPresent =>
                        logger.warn(
                          s"unexpected/deprecated node encountered: $other with properties: ${other.propertiesMap()}"
                        )
        ...
        case Operators.getElementPtr =>
            // we will reverse the list in the end
            AddressOf :: extractAccessStringTokenForFieldAccess(
              memberAccess
            ) :: IndirectionAccess :: tail
  ...
  def lastExpressionInBlock(block: Block): Option[Expression] =
      block._astOut
          .collect {
              case node: Expression if !node.isInstanceOf[Local] && !node.isInstanceOf[Method] =>
                  node
          }
          .toVector
          .sortBy(_.order)
          .lastOption
end AccessPathHandling
