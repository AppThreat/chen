...
package object language extends operatorextension.Implicits with LowPrioImplicits
  ...
  /** Warning: implicitly lifting `Node -> Traversal` opens a broad space with a lot of accidental
    * complexity and is considered a historical accident. We only keep it around because we want to
    * preserve `reachableBy(Node*)`, which unfortunately (due to type erasure) can't be an overload
    * of `reachableBy(Traversal*)`.
    *
