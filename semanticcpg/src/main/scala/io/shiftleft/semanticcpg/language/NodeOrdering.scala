...
object NodeOrdering:
  ...
  /** For a given CFG with the entry node `cfgEntry` and an expansion function `expand`, return a
    * map that associates each node with an index such that nodes are numbered in post order.
    */
  def postOrderNumbering[NodeType](
    ...
    while stack.nonEmpty do
      ...
      if successors.hasNext then
        ...
        nextNumber += 1
    numbering
  end postOrderNumbering

  /** For a list of (node, number) pairs, return the list of nodes obtained by sorting nodes
    * according to number in reverse order.
    */
