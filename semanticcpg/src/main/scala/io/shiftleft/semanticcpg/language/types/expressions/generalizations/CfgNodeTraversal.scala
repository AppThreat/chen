...
class CfgNodeTraversal[A <: CfgNode](val traversal: Iterator[A]) extends AnyVal:
    ...
    def method: Iterator[Method] =
        traversal.map(
          _.method
        ) // refers to `semanticcpg.language.nodemethods.CfgNodeMethods.method`
    ...
    /** Recursively determine all nodes on which any of the nodes in this traversal are control
      * dependent
      */
