  ...
  def usesCleartextTraffic =
      traversal
          ...
          .flatMap { applicationNode =>
            val activityName =
                applicationNode.attribute(Constants.androidUri, "usesCleartextTraffic")
  ...
  def hasReadExternalStoragePermission =
      traversal
          ...
          .flatMap { applicationNode =>
            ...
            activityName match
              case Some(n) if n.toString == "android.permission.READ_EXTERNAL_STORAGE" =>
                  Some(true)
              case _ => None
          }
  ...
  def exportedAndroidActivityNames =
      traversal
        ...
        will resolve to your activity.
            ...
            if hasIntentFilterWithDefaultCategory then
