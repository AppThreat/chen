...
class ExpressionTraversal[NodeType <: Expression](val traversal: Iterator[NodeType]) extends AnyVal:
  ...
  /** Traverse to it's parent expression (e.g. call or return) by following the incoming AST It's
    * continuing it's walk until it hits an expression that's not a generic "member access
    * operation", e.g., "<operator>.memberAccess".
    */
  ...
  /** Traverse to related parameter, if the expression is an argument to a call and the call can be
    * resolved.
    */
