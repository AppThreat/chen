...
trait ICallResolver:
  ...
  /** Starts data flow tracking to find all method which could be called at the given callsite. The
    * result is stored in the resolver internal cache.
    */
  def triggerCallsiteResolution(callsite: CallRepr): Unit

  /** Starts data flow tracking to find all callsites which could call the given method. The result
    * is stored in the resolver internal cache.
    */
