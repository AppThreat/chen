...
object AccessPath:
    ...
    /** This class contains operations on `Elements` that are only used in the `AccessPath` class
      * and are not part of the public API of `Elements`
      */
    private implicit class ElementsDecorations(el: Elements):
        ...
        /** In all sane situations, invertibleTailLength is 0 or 1:
          *   - we don't expect <i> &, because you cannot take the address of pointer+i (can only
          *     take address of rvalue)
          *   - we don't expect & <i>: The & should have collapsed against a preceding *. An example
          *     where this occurs is (&(ptr->field))[1], which becomes ptr: * field & <2> * This
          *     reads the _next_ field: It doesn't alias with ptr->field at all, but reads the next
          *     bytes in the struct, after field.
          *
...
case class AccessPath(elements: Elements, exclusions: Seq[Elements]):
    ...
    def matchAndDiff(other: Elements): (MatchResult.MatchResult, Elements) =
        ...
        /** We now try to greedily match more elements. We know that one of the two paths will only
          * contain invertible elements. The issue is the following: prefix <1> & x prefix <?> &
          * With greedy matching, we end up with a diff: x. If we just did the invert-append
          * algorithm, we would end up with a less precise diff: * <?> <1> & x == * <?> & x.
          */
        ...
        if thisHead >= otherHead then
            ...
            /** we need to overtaint if any either otherTail or thisTail has variable PointerShift:
              * e.g. suppose we track "a" <4> and encounter "a" "b" <?> "c", or suppose that we
              * track "a" <?> and encounter "a" "b"
              */
