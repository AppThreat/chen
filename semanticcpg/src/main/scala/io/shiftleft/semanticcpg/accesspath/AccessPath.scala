...
object AccessPath:
  ...
  def apply(elements: Elements, exclusions: Seq[Elements]): AccessPath =
      if elements.isEmpty && exclusions.isEmpty then
        empty
      else
  ...
  /** This class contains operations on `Elements` that are only used in the `AccessPath` class and
    * are not part of the public API of `Elements`
    */
  private implicit class ElementsDecorations(el: Elements):
    ...
    /** In all sane situations, invertibleTailLength is 0 or 1:
      *   - we don't expect <i> &, because you cannot take the address of pointer+i (can only take
      *     address of rvalue)
      *   - we don't expect & <i>: The & should have collapsed against a preceding *. An example
      *     where this occurs is (&(ptr->field))[1], which becomes ptr: * field & <2> * This reads
      *     the _next_ field: It doesn't alias with ptr->field at all, but reads the next bytes in
      *     the struct, after field.
      *
...
case class AccessPath(elements: Elements, exclusions: Seq[Elements]):
  ...
  override def hashCode(): Int =
      if cachedHash == 0 then
        val computedHash = elements.hashCode() + exclusions.hashCode() ^ 0x404f92ab
        cachedHash = if computedHash == 0 then 1 else computedHash
        cachedHash
      else cachedHash
  ...
  def ++(other: Elements): Option[AccessPath] =
      if isExtensionExcluded(other) then None
  ...
  def matchFull(other: Elements): FullMatchResult =
    ...
    matchRes match
      ...
      case MatchResult.VARIABLE_PREFIX_MATCH | MatchResult.VARIABLE_EXACT_MATCH =>
          FullMatchResult(
            Some(this),
            Some(AccessPath(matchDiff, this.exclusions)),
            Elements.empty
          )
  ...
  def matchAndDiff(other: Elements): (MatchResult.MatchResult, Elements) =
    ...
    while idx < cmpUntil do
      (elements.elements(idx), other.elements(idx)) match
        case (VariableAccess, VariableAccess) | (_: ConstantAccess, VariableAccess) |
            (VariableAccess, _: ConstantAccess) | (
              VariablePointerShift,
              VariablePointerShift
            ) |
            (_: PointerShift, VariablePointerShift) | (
              VariablePointerShift,
              _: PointerShift
            ) =>
            overTainted = true
        case (thisElem, otherElem) =>
            if thisElem != otherElem then
              return (MatchResult.NO_MATCH, Elements.empty)
      idx += 1
    var done = false

    /** We now try to greedily match more elements. We know that one of the two paths will only
      * contain invertible elements. The issue is the following: prefix <1> & x prefix <?> & With
      * greedy matching, we end up with a diff: x. If we just did the invert-append algorithm, we
      * would end up with a less precise diff: * <?> <1> & x == * <?> & x.
      */
    val minlen = scala.math.min(elements.elements.length, other.elements.length)
    while !done && idx < minlen do
      (elements.elements(idx), other.elements(idx)) match
    ...
    if thisHead >= otherHead then
      // prefix or exact
      val diff = Elements.inverted(other.elements.drop(idx)) ++ Elements.unnormalized(
        elements.elements.drop(idx)
      )

      /** we don't need to overtaint if thisTail has variable PointerShift: They can still get
        * excluded e.g. suppose we track "a" "b" <?> and encounter "a" <4>.
        */
      overTainted |= !other.noOvertaint(otherHead)
      if !overTainted & thisHead == otherHead then (MatchResult.EXACT_MATCH, diff)
      else if overTainted && thisHead == otherHead then
        (MatchResult.VARIABLE_EXACT_MATCH, diff)
      else if !overTainted && thisHead != otherHead then (MatchResult.PREFIX_MATCH, diff)
      else if overTainted && thisHead != otherHead then
        (MatchResult.VARIABLE_PREFIX_MATCH, diff)
      else throw new RuntimeException()
    else
      // extended
      val diff = Elements.inverted(elements.elements.drop(idx)) ++ Elements.unnormalized(
        other.elements.drop(idx)
      )

      /** we need to overtaint if any either otherTail or thisTail has variable PointerShift: e.g.
        * suppose we track "a" <4> and encounter "a" "b" <?> "c", or suppose that we track "a" <?>
        * and encounter "a" "b"
        */
      overTainted |= !elements.noOvertaint(thisHead) | !other.noOvertaint(otherHead)

      if overTainted then (MatchResult.VARIABLE_EXTENDED_MATCH, diff)
      else if isExtensionExcluded(diff) then (MatchResult.NO_MATCH, Elements.empty)
      else (MatchResult.EXTENDED_MATCH, diff)
    end if
  end matchAndDiff

  private def truncateExclusions(compareExclusion: Elements): AccessPath =
    if exclusions.isEmpty then return this
  ...
  private def addExclusion(newExclusion: Elements): AccessPath =
      if newExclusion.noOvertaint() then
        val ex =
            Elements.unnormalized(
              newExclusion.elements.dropRight(newExclusion.invertibleTailLength)
            )
        if isExtensionExcluded(ex) then return this
...
object MatchResult extends Enumeration:
  ...
  val NO_MATCH, EXACT_MATCH, VARIABLE_EXACT_MATCH, PREFIX_MATCH, VARIABLE_PREFIX_MATCH,
    EXTENDED_MATCH, VARIABLE_EXTENDED_MATCH = Value

/** Result of `matchFull` comparison
  *
  * @param stepOverPath
  *   the unaffected part of the access path. Some(this) for no match, None for perfect match; may
  *   have additional exclusions to this.
  * @param stepIntoPath
  *   The affected part of the access path, mapped to be relative to this stepIntoPath.isDefined if
  *   and only if there is a match in paths, i.e. if the call can affect the tracked variable at
  *   all. Outside of overtainting, if stepIntoPath.isDefined && stepIntoPath.elements.nonEmpty
  *   then: path.elements == other.elements ++ path.matchFull(other).stepIntoPath.get.elements
  *   extensionDiff.isEmpty
  *
  * @param extensionDiff
  *   extensionDiff is non empty if and only if a proper subset is affected. Outside of over
  *   tainting, if extensionDiff is non empty then: path.elements ++
  *   path.matchFull(other).extensionDiff == other.elements
  *   path.matchFull(other).stepIntoPath.get.elements.isEmpty
  *
  * Invariants:
  *   - Exclusions have no invertible tail
  *   - Only paths without overTaint can have exclusions TODO: Figure out sensible assertions to
  *     defend these invariants
  */
...
/** For handling of invertible elements, cf AccessPathAlgebra.md. The general rule is that elements
  ...
  *
  * Elements(&) ++ Elements(*) == Elements() Elements(*) ++ Elements(&) == Elements() Elements(<0>)
  * \== Elements() Elements(<i>) ++ Elements(<j>) == Elements(<i+j>) Elements(<?>) ++ Elements(<j>)
  * \== Elements(<?>) Elements(<i>) ++ Elements(<?>) == Elements(<?>) Elements(<?>) ++ Elements(<?>)
  * \== Elements(<?>)
  *
  * From this, once can see that <i>, * and & are invertible, <?> is idempotent and <0> is a
  * convoluted way of describing and empty sequence of tokens. Nevertheless, we mostly consider * as
  * noninvertible (because it is, in real computers!) and <?> as invertible (because it is in real
  * computers, we just don't know the offset)
  *
  * Elements get a private constructor. Users should use the no-argument Elements.apply() factory
  * method to get an empty path, and the specific concat operators for building up pathes. The
  * Elements.normalized(iter) factory method serves to build this in bulk.
  *
  * The unnormalized factory method is more of an escape hatch.
  *
  * The elements field should never be mutated outside of this file: We compare and hash Elements by
  * their contents, not by identity, and this breaks in case of mutation.
  *
  * The reason for using a mutable Array instead of an immutable Vector is that this is the lightest
  * weight datastructure for the job.
  *
  * The reason for making this non-private is simply that it is truly annoying to write wrappers for
  * all possible uses.
  */
...
object Elements:
  ...
  def newIfNonEmpty(elems: Array[AccessElement]): Elements =
      if !elems.isEmpty then new Elements(elems)
      else empty
  ...
  private def destructiveNormalized(elems: Array[AccessElement]): Elements =
    var idxRight = 0
    var idxLeft  = -1
    while idxRight < elems.length do
            ...
            if idxLeft == -1 then
              ...
              (lastE, nextE) match
                case (last: PointerShift, next: PointerShift) =>
                    ...
                    if newShift != 0 then elems(idxLeft) = PointerShift(newShift)
                    else idxLeft -= 1
                case (VariablePointerShift, _: PointerShift) | (
                      VariablePointerShift,
                      VariablePointerShift
                    ) =>
                case (_: PointerShift, VariablePointerShift) =>
...
final class Elements(val elements: Array[AccessElement] = Array[AccessElement]())
      ...
      other match
        case otherElements: Elements =>
            Array.equals(
              ...
              otherElements.elements.asInstanceOf[Array[AnyRef]]
            )
        case _ => false
  ...
  override def compareTo(other: Elements): Int =
    ...
    while idx < until do
      ...
      idx += 1
    if idx < elements.length then +1
    else if idx < other.elements.length then -1
    else 0

  def ++(otherElements: Elements): Elements =

    if elements.isEmpty then return otherElements
    if otherElements.isEmpty then return this
