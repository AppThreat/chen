...
class AccessPathTests extends AnyWordSpec:
        ...
        case _ if !elements.isEmpty =>
            (elements.last, element) match
              case (last: PointerShift, elem: PointerShift) =>
                  ...
                  if newOffset == 0 then Elements.newIfNonEmpty(elements.dropRight(1))
                  else
                    Elements.newIfNonEmpty(elements.updated(
                      elements.length - 1,
                      PointerShift(newOffset)
                    ))
              ...
              case (IndirectionAccess, AddressOf) =>
                  /** We also collapse *&. This is WRONG (you cannot deref a pointer and then
                    * un-deref the result). However, it is sometimes valid as a syntactic construct,
                    * and the language should make sure that this only occurs in such settings. I.e.
                    * valid code should only produce such paths when their contraction is valid. We
                    * still treat * as un-invertible, though!
                    */
                  Elements.newIfNonEmpty(elements.dropRight(1))
  ...
  "Test matchAndDiff" in {
      ...
      AccessPath(E("a"), Seq(E("b", "c"))).matchAndDiff(E("a", "b")) shouldBe (
        EXTENDED_MATCH,
        E("b")
      )
      ...
      AccessPath(E(V), Seq(E("b"))).matchAndDiff(E("a", "b")) shouldBe (
  ...
  "Test normalization and concatenation" in {
      ...
      E(2) :+ PointerShift(-1) :+ ConstantAccess("a") :+ I :+ PointerShift(3) :+ PointerShift(
        -5
      ) :+ PointerShift(
        2
      ) :+ A shouldBe E(1, "a")
  ...
  "Test matchAndDiff with inverses" in {
      ...
      AccessPath(E("a", 1, A, 2), Seq(E("c")))
          .matchAndDiff(E("a", "b", 8, A, 16)) shouldBe (
        EXTENDED_MATCH,
        E(-2, I, -1, "b", 8, A, 16)
      )
