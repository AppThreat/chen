package io.appthreat.console.testing

import better.files.Dsl.mkdir
import better.files.File
import io.appthreat.console.cpgcreation.ImportCode
import io.appthreat.console.{Console, ConsoleConfig, FrontendConfig}
import io.appthreat.console.cpgcreation.{CCpgGenerator, CpgGenerator, CpgGeneratorFactory, ImportCode}
import io.appthreat.console.workspacehandling.{Project, ProjectFile, WorkspaceLoader}
import io.appthreat.console.{Console, ConsoleConfig, FrontendConfig, InstallConfig}
...
class TestCpgGeneratorFactory(config: ConsoleConfig) extends CpgGeneratorFactory(config) {
  private def newCCpgGenerator() = {
    CCpgGenerator(
      config.frontend,
      Path.of(ProjectRoot.relativise("platform/frontends/c2cpg/target/universal/stage/bin"))
    )
