...
package object console:
  ...
  implicit class UnixUtils[A](content: Iterable[A]):
    ...
    /** Iterate over left hand side operand and write to file. Think of it as the Ocular version of
      * the Unix `>` shell redirection.
      */
    @deprecated("please use `#>` instead", "2.0.45 (August 2023)")
    def |>(outfile: String): Unit =
        content #> outfile

    /** Iterate over left hand side operand and append to file. Think of it as the Ocular version of
      * the Unix `>>` shell redirection.
      */
    @deprecated("please use `#>>` instead", "2.0.45 (August 2023)")
    def |>>(outfile: String): Unit =
