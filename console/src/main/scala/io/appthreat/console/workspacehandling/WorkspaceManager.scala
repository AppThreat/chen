...
class WorkspaceManager[ProjectType <: Project](
  ...
  /** Create project for code stored at `inputPath` with the project name `name`. If `name` is
    ...
    * exists, it is deleted from the workspace first. If no file or directory exists at `inputPath`,
    * then no project is created. Returns the path to the project directory as an optional String,
  ...
  /** Remove currently active project from workspace and delete all associated workspace files from
    * disk.
    */
  ...
  /** Remove project with name `name` from workspace and delete all associated workspace files from
    * disk.
    * @param name
