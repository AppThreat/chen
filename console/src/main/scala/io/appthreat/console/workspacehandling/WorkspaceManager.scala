package io.appthreat.console.workspacehandling

import better.files.Dsl.*
import better.files.*
import io.appthreat.console.Reporting
import io.appthreat.console.Reporting
...
/** WorkspaceManager: a component, which loads and maintains the list of projects made accessible
  * via Ocular/Joern.
  *
  * @param path
...
class WorkspaceManager[ProjectType <: Project](
  ...
  loader: WorkspaceLoader[ProjectType] = DefaultLoader
    ...
    private val LEGACY_BASE_CPG_FILENAME = "app.atom"
    private val OVERLAY_DIR_NAME         = "overlays"

    /** Create project for code stored at `inputPath` with the project name `name`. If `name` is
      * empty, the project name is derived from `inputPath`. If a project for this `name` already
      * exists, it is deleted from the workspace first. If no file or directory exists at
      * `inputPath`, then no project is created. Returns the path to the project directory as an
      * optional String, and None if there was an error.
      */
    def createProject(inputPath: String, projectName: String): Option[Path] =
        Some(File(inputPath)).filter(_.exists).map { _ =>
            ...
            if project(projectName).isDefined then
    ...
    private def writeProjectFile(projectFile: ProjectFile, dirPath: Path): File =
        ...
        val content =
            jsonWrite(Map("inputPath" -> projectFile.inputPath, "name" -> projectFile.name))
    ...
    def cpgExists(inputPath: String, isLegacy: Boolean = false): Boolean =
        val baseFileName = if isLegacy then LEGACY_BASE_CPG_FILENAME else BASE_CPG_FILENAME
    ...
    /** The safe directory name for a given input file that can be used to store its base CPG, along
      * with all overlays. This method returns a directory name regardless of whether the directory
      * exists or not.
      */
    ...
    /** Set active project to project with name `name`. If a project with this name does not exist,
      * does nothing.
      */
    ...
    /** Open project by name and return it. If a project with this name does not exist, None is
      * returned. If the CPG of this project is loaded, it is unloaded first and then reloaded.
    ...
    def openProject(
      ...
      loader: String => Option[Cpg] = { x =>
        ...
        if !projectExists(name) then
            report(
              s"Project does not exist in workspace. Try `importCode/importAtom(inputPath)` to create it"
            )
            None
        else if !File(baseCpgFilename(name)).exists then
            report(s"CPG for project $name does not exist at ${baseCpgFilename(name)}, bailing out")
            None
        else if project(name).exists(_.cpg.isDefined) then
    ...
    private def setCpgForProject(newCpg: Cpg, projectPath: Path): Unit =
        ...
        project match
            ...
            case None =>
                System.err.println(
                  s"Error setting CPG for non-existing/unloaded project at $projectPath"
                )
    ...
    /** Remove currently active project from workspace and delete all associated workspace files
      * from disk.
      */
    ...
    /** Remove project with name `name` from workspace and delete all associated workspace files
      * from disk.
      * @param name
