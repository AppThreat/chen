...
package object cpgcreation:
  ...
  def cpgGeneratorForLanguage(
    ...
    language.toUpperCase() match
      case Languages.C | Languages.NEWC | Languages.JAVA | Languages.JAVASRC | Languages
              .JSSRC | Languages.JAVASCRIPT |
  ...
  /** Guess the main language for an entire directory (e.g. a whole project), based on a group count
    * of all individual files. Rationale: many projects contain files from different languages, but
    * most often one language is standing out in numbers.
    */
  ...
  private def isGoFile(filename: String): Boolean =
      filename.endsWith(".go") || Set("gopkg.lock", "gopkg.toml", "go.mod", "go.sum").contains(
        filename
      )
  ...
  /** check if given filename looks like it might be a C/CPP source or header file mostly copied
    * from io.appthreat.c2cpg.parser.FileDefaults
    */
  private def isCFile(filename: String): Boolean =
      Seq(".c", ".cc", ".cpp", ".h", ".hpp", ".hh", ".ccm", ".cxxm", ".c++m").exists(
        filename.endsWith
      )
