...
case class AtomGenerator(config: FrontendConfig, rootPath: Path, language: String, sliceMode: String = "usages")
  ...
  override def generate(inputPath: String, outputPath: String = "app.atom"): Try[String] = {
    val arguments = Seq(sliceMode, "--output", outputPath, "--language", language, inputPath) ++ config.cmdLineParams
