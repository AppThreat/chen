...
/** C/C++ language frontend that translates C/C++ source files into code property graphs using
  * Eclipse CDT parsing / preprocessing.
  */
case class CCpgGenerator(config: FrontendConfig, rootPath: Path) extends CpgGenerator:
    private lazy val command: Path =
        if isWin then rootPath.resolve("c2cpg.bat") else rootPath.resolve("c2cpg.sh")
