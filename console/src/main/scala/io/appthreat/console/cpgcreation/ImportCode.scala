...
class ImportCode[T <: Project](console: Console[T]) extends Reporting:
    ...
    /** This is the `importCode(...)` method exposed on the console. It attempts to find a suitable
      * CPG generator first by looking at the `language` parameter and if no generator is found for
      * the language, looking the contents at `inputPath` to determine heuristically which generator
      * to use.
      */
    def apply(inputPath: String, projectName: String = "", language: String = ""): Cpg =
        var srcPath =
            if
                inputPath.startsWith("http") || inputPath.startsWith(
                  "git://"
                ) || inputPath.startsWith("CVE-") || inputPath
                    .startsWith("GHSA-")
            then importUrl(inputPath)
            else inputPath
        checkInputPath(srcPath)
        if language != "" then
            generatorFactory.forLanguage(language.toUpperCase()) match
                case None =>
                    throw new ConsoleException(s"No Atom generator exists for language: $language")
                case Some(frontend) => apply(frontend, srcPath, projectName)
        else
            generatorFactory.forCodeAt(srcPath) match
                case None =>
                    throw new ConsoleException(s"No suitable Atom generator found for: $srcPath")
                case Some(frontend) => apply(frontend, srcPath, projectName)
    end apply

    def c: SourceBasedFrontend   = new CFrontend("c")
    def cpp: SourceBasedFrontend = new CFrontend("cpp", extension = "cpp")
    def java: SourceBasedFrontend =
        new SourceBasedFrontend("java", Languages.JAVASRC, "Java Source Frontend", "java")
    def jvm: Frontend =
        new BinaryFrontend(
          "jvm",
          Languages.JAVA,
          "Java/Dalvik Bytecode Frontend (based on SOOT's jimple)"
        )
    def ghidra: Frontend =
        new BinaryFrontend("ghidra", Languages.GHIDRA, "ghidra reverse engineering frontend")
    ...
    def golang: SourceBasedFrontend =
        new SourceBasedFrontend("golang", Languages.GOLANG, "Golang Source Frontend", "go")
    def javascript: SourceBasedFrontend =
        new SourceBasedFrontend(
          "javascript",
          Languages.JAVASCRIPT,
          "Javascript Source Frontend",
          "js"
        )
    def jssrc: SourceBasedFrontend =
        new SourceBasedFrontend(
          "jssrc",
          Languages.JSSRC,
          "Javascript/Typescript Source Frontend based on astgen",
          "js"
        )
    def csharp: Frontend =
        new BinaryFrontend("csharp", Languages.CSHARP, "C# Source Frontend (Roslyn)")
    def llvm: Frontend = new BinaryFrontend("llvm", Languages.LLVM, "LLVM Bitcode Frontend")
    def php: SourceBasedFrontend =
        new SourceBasedFrontend("php", Languages.PHP, "PHP source frontend", "php")
    def ruby: SourceBasedFrontend =
        new SourceBasedFrontend("ruby", Languages.RUBYSRC, "Ruby source frontend", "rb")
    ...
    abstract class Frontend(val name: String, val language: String, val description: String = ""):
        def cpgGeneratorForLanguage(
          ...
          args: List[String]
            ...
            io.appthreat.console.cpgcreation.cpgGeneratorForLanguage(
              language,
              config,
              rootPath,
              args
        ...
        def apply(inputPath: String, projectName: String = "", args: List[String] = List()): Cpg =
            val frontend = cpgGeneratorForLanguage(
              ...
              args
                ...
                .getOrElse(throw new ConsoleException(
                  s"no atom generator for language=$language available!"
                ))
      ...
      extension: String
        ...
        def fromString(str: String, args: List[String] = List()): Cpg =
            withCodeInTmpFile(str, "tmp." + extension) { dir =>
                ...
                case Failure(exception) => throw new ConsoleException(
                      s"unable to generate atom from given String",
                      exception
                    )
    ...
    class CFrontend(name: String, extension: String = "c")
        extends SourceBasedFrontend(
          name,
          Languages.NEWC,
          "Eclipse CDT Based Frontend for C/C++",
          extension
        )
    ...
    private def apply(generator: CpgGenerator, inputPath: String, projectName: String): Cpg =
        ...
        val name = Option(projectName).filter(_.nonEmpty).getOrElse(deriveNameFromInputPath(
          inputPath,
          workspace
        ))
        ...
        val cpgMaybe = workspace.createProject(inputPath, name).flatMap { pathToProject =>
            ...
            val frontendAtomPath =
                generatorFactory.runGenerator(generator, inputPath, frontendCpgOutFile.toString)
            frontendAtomPath match
                case Success(_) =>
                    console.open(name).flatMap(_.cpg)
                case Failure(exception) =>
                    throw new ConsoleException(
                      s"Error creating project for input path: `$inputPath`",
                      exception
                    )
        }
        ...
        cpgMaybe
            ...
            .getOrElse(
              throw new ConsoleException(s"Error creating project for input path: `$inputPath`")
            )
        cpgMaybe.get
    end apply
end ImportCode
