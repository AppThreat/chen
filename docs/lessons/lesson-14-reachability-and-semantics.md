...
request readers apply only to JVM-language graphs, in two lists for the two resolution states:
bare names (`getParameter`, `getHeader`, ...) for unresolved calls, and receiver-qualified
REGEX entries (`jakarta.servlet.http.HttpServletRequest.getParameter:.*`, ...) for resolved
ones - both mapping receiver 0 to return -1, because `request.getParameter("q")` returns
request data. The bare list is restricted to request-shaped names: a declared semantic is
authoritative, so a generic name there would also govern an unrelated same-named method. The Java
