...
- **`ChennaiTagsPass(atom: Cpg, externalConfig: Option[String] = None)`** — framework route/input/output and sanitiser tagging. It applies built-in per-language route patterns (Java: Spring MVC/WebFlux, JAX-RS/Jakarta, Micronaut, servlets, Vert.x router DSLs, gRPC `StreamObserver` services, JDBC/JPA/MyBatis `sql`, AI SDK invocations, MCP `@Tool` methods, AWS/GCP/Azure handlers, JNI/FFM natives, Kafka/JMS/RabbitMQ listeners — vocabulary in `x2cpg/.../taggers/java/JavaFrameworks.scala`; Python/Django/Flask; Express/React Router/Vue/Angular/Next.js/Nuxt/SvelteKit; PHP/Laravel/Symfony/WordPress; Rails/Sinatra; C) and then reads a `chennai.json` config to declare additional sources/sinks/sanitisers. The `externalConfig` parameter lets a caller inject the same JSON without embedding it in the graph — this is what `atom --validation-config` uses.
