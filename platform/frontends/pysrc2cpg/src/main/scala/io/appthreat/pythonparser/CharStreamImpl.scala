...
class CharStreamImpl(inputStream: InputStream, inputBufferSize: Int, minimumReadSize: Int)
    ...
    /** Returns the column number of the last character for current token (being matched after the
      * last call to BeginTOken).
      */
    override def getEndColumn: Int =
        posToColumn(lastRedPos)

    /** Returns the line number of the last character for current token (being matched after the
      * last call to BeginTOken).
      */
    ...
    /** Returns the line number of the first character for current token (being matched after the
      * last call to BeginTOken).
      */
    override def getBeginLine: Int =
        posToLine(tokenBeginPos)

    /** Backs up the input stream by amount steps. Lexer calls this method if it had already read
      * some characters, but could not use them to match a (longer) token. So, they will be used
      * again as the prefix of the next token and it is the implementation's responsibility to do
      * this right.
      */
    ...
    /** The lexer calls this function to indicate that it is done with the stream and hence
      * implementations can free any resources held by this class. Again, the body of this function
      * can be just empty and it will not affect the lexer's operation.
      */
