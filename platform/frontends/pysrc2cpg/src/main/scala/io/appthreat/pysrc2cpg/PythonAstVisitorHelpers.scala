...
trait PythonAstVisitorHelpers:
  this: PythonAstVisitor =>
  ...
  protected def getUnusedName(prefix: String = null): String =
    ...
    val result = if prefix != null then
    ...
    val importAssignNodes =
        names.map { alias =>
          ...
          val arguments = Seq(
            ...
            case Some(aliasName) =>
                Seq(nodeBuilder.stringLiteralNode(aliasName, lineAndCol))
            case None => Seq()
          )
  ...
  protected def createComprehensionLowering(
    ...
    blockNode
  end createComprehensionLowering
  ...
  protected def createBlock(
    blockElements: Iterable[NewNode],
    lineAndColumn: LineAndColumn
  ): NewNode =
  ...
  protected def createCall(
    ...
    val code = codeOf(receiverNode) +
        ...
        ")"
    val callNode =
        nodeBuilder.callNode(code, name, DispatchTypes.DYNAMIC_DISPATCH, lineAndColumn)
  ...
  protected def createStaticCall(
    ...
    val code = name +
        ...
        ")"
    val callNode =
        nodeBuilder.callNode(code, methodFullName, DispatchTypes.STATIC_DISPATCH, lineAndColumn)
  ...
  protected def createNAryOperatorCall(
    ...
    val code =
        operands.map(operandNode => codeOf(operandNode)).mkString(" " + operatorCode + " ")
    val callNode =
        nodeBuilder.callNode(code, methodFullName, DispatchTypes.STATIC_DISPATCH, lineAndColumn)
  ...
  protected def createBinaryOperatorCall(
    ...
    val callNode =
        nodeBuilder.callNode(code, opFullName, DispatchTypes.STATIC_DISPATCH, lineAndColumn)
  ...
  protected def createReturn(
    ...
    returnExprOption.foreach { returnExpr =>
        addAstChildrenAsArguments(returnNode, 1, returnExpr)
    }
