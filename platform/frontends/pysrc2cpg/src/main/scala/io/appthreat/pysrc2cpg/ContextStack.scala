...
import ContextStack.transferLineColInfo
import io.appthreat.pysrc2cpg.memop.{Del, Load, MemoryOperation, Store}
...
object ContextStack:
    ...
    def transferLineColInfo(src: NewIdentifier, tgt: NewLocal): Unit =
        src.lineNumber match
            ...
            case Some(srcLineNo)
                if tgt.lineNumber.isEmpty || !tgt.lineNumber.exists(_ < srcLineNo) =>
...
class ContextStack:
    ...
    def pushMethod(
      ...
      methodRefNode: Option[nodes.NewMethodRef]
        ...
        if moduleMethodContext.isEmpty then
    ...
    end createIdentifierLinks

    /** Assignments to variables on the module-level may be exported to other modules and behave as
      * inter-procedurally global variables.
      * @param lhs
      *   the LHS node of an assignment
      */
    def considerAsGlobalVariable(lhs: NewNode): Unit =
        lhs match
            case n: NewIdentifier
                if findEnclosingMethodContext(stack).scopeName.contains("<module>") =>
                addGlobalVariable(n.name)
            case _ =>

    /** For module-methods, the variables of this method can be imported into other modules which
      * resembles behaviour much like fields/members. This inter-procedural accessibility should be
      * marked via the module's type decl node.
      */
    ...
    private def linkLocalOrCapturing(
      ...
      contextStack: List[Context]
        ...
        while stackIt.hasNext && !contextHasVariable do
            ...
            context match
                case methodContext: MethodContext =>
                    ...
                    if !methodContext.isClassBodyMethod || methodContext == startContext then
                        ...
                        if !contextHasVariable then
                            if context != moduleMethodContext.get then
                        ...
                        if !contextHasVariable && context != moduleMethodContext.get then
                            identifierOrClosureBindingToLink =
                                createClosureBinding(closureBindingId, name)
                            createCaptureEdge(
                              identifierOrClosureBindingToLink,
                              methodContext.methodRefNode.get
                            )
    ...
    private def lookupVariableInMethod(name: String, stack: List[Context]): Option[nodes.NewNode] =
        ...
        while stackIt.hasNext && variableNode.isEmpty && !lastContextWasMethod do
            val context = stackIt.next()
