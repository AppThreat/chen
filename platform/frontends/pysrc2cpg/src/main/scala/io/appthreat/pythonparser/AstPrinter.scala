...
class AstPrinter(indentStr: String) extends AstVisitor[String]:
  ...
  override def visit(classDef: ClassDef): String =
    val optionArgEndComma =
        if classDef.bases.nonEmpty && classDef.keywords.nonEmpty then ", " else ""
  ...
  override def visit(forStmt: For): String =
      "for " + print(forStmt.target) + " in " + print(forStmt.iter) + ":" +
          ...
          (if forStmt.orelse.nonEmpty then
             s"${ls}else:" +
  ...
  override def visit(whileStmt: While): String =
      "while " + print(whileStmt.test) + ":" +
          ...
          (if whileStmt.orelse.nonEmpty then
             s"${ls}else:" +
  ...
  override def visit(set: ast.Set): String =
  ...
  override def visit(call: Call): String =
      if call.args.size == 1 && call.args.head.isInstanceOf[GeneratorExp] then
        ...
        val optionArgEndComma =
            if call.args.nonEmpty && call.keywords.nonEmpty then ", " else ""
  ...
  override def visit(arguments: Arguments): String =
    ...
    if arguments.posonlyargs.nonEmpty then
    ...
    if arguments.args.nonEmpty then
    ...
    if arguments.kwonlyargs.nonEmpty then
  ...
  override def visit(matchMapping: MatchMapping): String =
      "{" + matchMapping.keys
          ...
          matchMapping.rest
              .map { r =>
                val separatorString =
                    if matchMapping.keys.nonEmpty then
                      ", "
                    else
                      ""
                separatorString + "**" + r
  ...
  override def visit(matchClass: MatchClass): String =
    val separatorString =
        if matchClass.patterns.nonEmpty && matchClass.kwd_patterns.nonEmpty then
          ", "
        else
          ""
  ...
  override def visit(comprehension: Comprehension): String =
    val prefix =
        if comprehension.is_async then
          " async for "
        else
          " for "
