...
import PythonAstVisitor.{builtinPrefix, metaClassSuffix}
...
class PythonAstVisitor(
  ...
  private def createBuiltinIdentifiers(namesUsedInModule: collection.Set[String])
    ...
    builtinFunctions.distinct.foreach { builtinObjectName =>
        if namesUsedInModule.contains(builtinObjectName) then
  ...
  private def createParameterProcessingFunction(
    ...
    val startIndex =
        if contextStack.isClassContext && !isStatic then
          0
        else
          1
  ...
  private def createMethod(
    ...
    val methodReturnNode =
        nodeBuilder.methodReturnNode(
          nodeBuilder.extractTypesFromHint(returns),
          returnTypeHint,
          lineAndColumn
        )
    ...
    val typeDeclNode =
        nodeBuilder.typeDeclNode(name, fullName, relFileName, Seq(Constants.ANY), lineAndColumn)
    ...
    if name == "<module>" then contextStack.createMemberLinks(typeDeclNode, edgeBuilder.astEdge)
  ...
  def convert(classDef: ast.ClassDef): NewNode =
    ...
    val metaClassCallHandlerMethod =
        createMetaClassCallHandlerMethod(
          initParameters,
          metaTypeDeclName,
          metaTypeDeclFullName,
          instanceTypeDeclFullName
  ...
  private def createMemberBindingsAndAdapter(
    ...
    val methodForMetaClass =
        if isStaticMethod(functionDecoratorList) || isClassMethod(functionDecoratorList) then
    ...
    val memberForMeta = nodeBuilder.memberNode(
      functionName,
      methodForMetaClass.fullName,
      lineAndColOf(function)
    )
    edgeBuilder.astEdge(memberForMeta, metaTypeDecl, contextStack.order.getAndInc)
  end createMemberBindingsAndAdapter

  /** Creates an adapter method which adapts the meta class version of a method to the instance
    * class version. Consider class: class MyClass(): def func(self, p1): pass
    *
    * The syntax to call func via the meta class is: MyClass.func(someInstance, p1), whereas the
    * call via the instance itself is: someInstance.func(p1). To adapt between those two we
    * generate: def func<metaClassAdapter>(cls, self, p1): return STATIC_CALL(MyClass.func(self,
    * p1))
    * @return
    */
  // TODO handle kwArg
  ...
  end createMetaClassAdapterMethod
  ...
  private def stripFirstPositionalParameter(initParameters: ast.Arguments)
    : (ast.Arguments, LineAndColumn) =
      ...
      else if initParameters.args.nonEmpty then
        (
  ...
  /** Creates the method which handles a call to the meta class object. This process is also known
    ...
    * function is to adapt between the special cased instance creation call and a normal call to
    * __new__ (for now <fakeNew>). The adaption is required to in order to provide TYPE_REF(meta
    * class) as instance argument to __new__/<fakeNew>. So the <metaClassCallHandler> looks like:
  ...
  private def createMetaClassCallHandlerMethod(
    ...
    createMethod(
      ...
      bodyProvider = () =>
        val (arguments, keywordArguments) =
            createArguments(parametersWithoutSelf, lineAndColumn)
  ...
  end createMetaClassCallHandlerMethod

  /** Creates a <fakeNew> method which mimics the behaviour of a default __new__ method (the one you
    * would get if no implementation is present). The reason we use a fake version of the __new__
    * method it that we wont be able to correctly track through most custom __new__ implementations
    * as they usually call "super.__init__()" and we cannot yet handle "super". The fake __new__
  ...
  private def createFakeNewMethod(initParameters: ast.Arguments): nodes.NewMethod =
    ...
    createMethod(
      ...
      bodyProvider = () =>
        ...
        argumentWithInstance.append(createIdentifierNode(
          "__newInstance",
          Load,
          lineAndColumn
        ))
  ...
  def convert(assign: ast.Assign): nodes.NewNode =
    val loweredNodes =
        createValueToTargetsDecomposition(
          assign.targets,
          convert(assign.value),
          lineAndColOf(assign)
        )

    if loweredNodes.size == 1 then
    ...
    if ifs.nonEmpty then
      ...
      val ifNotContinueNode = convert(
        ast.If(
          ...
          mutable.ArrayBuffer.empty[ast.istmt].append(
            ast.Continue(ifs.head.attributeProvider)
          ),
  ...
  def convert(astWhile: ast.While): nodes.NewNode =
    ...
    if astWhile.orelse.nonEmpty then
  ...
  def convert(importFrom: ast.ImportFrom): NewNode =
    ...
    for i <- 0 until importFrom.level do
  ...
  def convert(dict: ast.Dict): NewNode =
    ...
    val dictElementAssignNodes = if dict.keys.size > MAX_KV_PAIRS then
      Seq(
        nodeBuilder
            .callNode(
  ...
  /** Lowering of (x for y in l for x in y): { tmp = <operator>.genExp <loweringOf>( for y in l: for
    * x in y: tmp.append(x) ) tmp } This lowering is not quite correct as it ignores the lazy
    * evaluation of the generator expression. Instead it just mimics the list comprehension lowering
  ...
  def convert(compare: ast.Compare): NewNode =
    ...
    if topLevelExprNodes.size > 1 then
  ...
  def lowerComparatorChain(
    ...
    if compOperators.size == 1 then
      ...
      val tmpVariableName = getUnusedName()
      val assignmentNode =
          createAssignmentToIdentifier(tmpVariableName, rhsNode, lineAndColumn)
      ...
      Iterable(
        ...
        createBinaryOperatorCall(
          compareNode,
          andOpCodeAndFullName(),
          blockNode,
          lineAndColumn
        )
      )
    end if
  end lowerComparatorChain

  private def andOpCodeAndFullName(): () => (String, String) = () =>
      ("and", Operators.logicalAnd)

  /** TODO For now this function compromises on the correctness of the lowering in order to get some
    * data flow tracking going.
    ...
    *      bound/captured during function access. This becomes relevant for constructs like: x.func
    *      \= y.func <- y.func is class method object x.func() In this case the instance passed into
    *      func is y and not x. We cannot represent this in th CPG and thus stick to the assumption
    *      that the part before the "." and the bound/captured instance will be the same. For
    *      reference see:
    *      https://docs.python.org/3/reference/datamodel.html#the-standard-type-hierarchy search for
    *      "Instance methods"
    */
  ...
  def convert(formattedValue: ast.FormattedValue): nodes.NewNode =
    ...
    val equalSignStr = if formattedValue.equalSign then "=" else ""
      ...
      constant.value match
        case stringConstant: ast.StringConstant =>
            nodeBuilder.stringLiteralNode(
              stringConstant.prefix + stringConstant.quote + stringConstant
                  .value + stringConstant.quote,
        ...
        case boolConstant: ast.BoolConstant =>
            val boolStr = if boolConstant.value then "True" else "False"
  ...
  /** TODO We currently ignore possible attribute access provider/interception mechanisms like
    * __getattr__, __getattribute__ and __get__.
    */
  ...
  private def createAndRegisterMember(name: String, lineAndCol: LineAndColumn): Unit =
      contextStack.findEnclosingTypeDecl() match
        case Some(typeDecl: NewTypeDecl) =>
            if !members.contains(typeDecl) || !members(typeDecl).contains(name) then
  ...
  def convert(subscript: ast.Subscript): NewNode =
      createIndexAccess(
        convert(subscript.value),
        convert(subscript.slice),
        lineAndColOf(subscript)
      )
  ...
  def convert(tuple: ast.Tuple): NewNode =
    ...
    val code = if tupleElementNodes.size != 1 then
