...
class PythonAstVisitor(
  ...
  version: PythonVersion
    ...
    /** Creates a <fakeNew> method which mimics the behaviour of a default __new__ method (the one
      * you would get if no implementation is present). The reason we use a fake version of the
      * __new__ method it that we wont be able to correctly track through most custom __new__
      * implementations as they usually call "super.__init__()" and we cannot yet handle "super".
      * The fake __new__ looks like: def <fakeNew>(cls, p1): __newInstance =
    ...
    /** Lowering of (x for y in l for x in y): { tmp = <operator>.genExp <loweringOf>( for y in l:
      * for x in y: tmp.append(x) ) tmp } This lowering is not quite correct as it ignores the lazy
      * evaluation of the generator expression. Instead it just mimics the list comprehension
      * lowering but for now this is good enough.
      */
    // TODO test
