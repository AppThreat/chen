...
class DynamicTypeHintFullNamePass(cpg: Cpg) extends ForkJoinParallelCpgPass[CfgNode](cpg):
      ...
      methodReturn.file.foreach { file =>
        ...
        val imports =
            fileToImports.getOrElse(file.name, List.empty) ++ methodReturn.method.typeDecl
                .map(td =>
                    ImportScope(Option(pythonicTypeNameToImport(td.fullName)), Option(td.name))
                )
                .toList
        imports
            .filter { x =>
                ...
                x.alias.exists { imported =>
                    typeHint.matches(Pattern.quote(imported) + "(\\..+)*")
                }
            }
            .flatMap(_.entity)
  ...
  private def runOnMethodParameter(diffGraph: DiffGraphBuilder, param: MethodParameterIn): Unit =
      param.file.foreach { file =>
        ...
        imports
            // TODO: Handle * imports correctly
            .filter(_.alias.exists { imported =>
                typeHint.matches(Pattern.quote(imported) + "(\\..+)*")
            })
            .foreach {
  ...
  private def setTypeHints(
    ...
    val typeFilePath =
        typeHintFullName.replaceAll("\\.", Matcher.quoteReplacement(File.separator))
    val pythonicTypeFullName = importFullPath.split("\\.").lastOption match
      case Some(typeName) =>
          typeFilePath.stripSuffix(s"${File.separator}$typeName").concat(
            s".py:<module>.$typeName"
          )
