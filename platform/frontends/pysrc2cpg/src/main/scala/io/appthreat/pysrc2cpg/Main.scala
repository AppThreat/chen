...
import Frontend.cmdLineParser
import io.appthreat.x2cpg.X2CpgMain
  ...
  val cmdLineParser: OParser[Unit, Py2CpgOnFileSystemConfig] =
    ...
    OParser.sequence(
      ...
      opt[Seq[String]]("ignore-paths")
          .text(
            "Ignores the specified path from analysis. If not absolute it is interpreted relative to input-dir."
          )
