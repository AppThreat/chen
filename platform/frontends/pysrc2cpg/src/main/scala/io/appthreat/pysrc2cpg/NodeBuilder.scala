...
import PythonAstVisitor.{allBuiltinClasses, builtinPrefix, typingClassesV3, typingPrefix}
...
class NodeBuilder(diffGraph: DiffGraphBuilder):
  ...
  def callNode(
    ...
    val callNode = nodes
        ...
        .code(code)
        .name(name)
        .methodFullName(if dispatchType == DispatchTypes.STATIC_DISPATCH then name
        else Defines.DynamicCallUnknownFullName)
  ...
  def namespaceBlockNode(
    name: String,
    fullName: String,
    fileName: String
  ): nodes.NewNamespaceBlock =
