...
/** Call-resolution tests modelled on actual Django source (django/apps/registry.py). These
  * reproduce real-world patterns (self-calls nested in `with`/`for`/`if`, calls made from
  * `__init__`) that the simplified SelfCallResolutionTests did not cover.
  */
class DjangoCallResolutionTests extends PySrc2CpgFixture(withOssDataflow = false):
  ...
  "self.clear_cache() nested inside a with-block" should {
      "resolve to the class method" in {
          ...
          call.methodFullName shouldBe "registry.Apps.clear_cache"
      }
  }
  ...
  "self.check_apps_ready() nested inside with+for" should {
      "resolve to the class method" in {
          ...
          call.methodFullName shouldBe "registry.Apps.check_apps_ready"
      }
  }
  ...
  "self.populate() called from __init__" should {
      "resolve to the class method" in {
          ...
          call.methodFullName shouldBe "registry.Apps.populate"
      }
  }
