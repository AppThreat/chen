...
class ParserTests extends AnyFreeSpec with Matchers:
  ...
  def test(code: String, expected: String, indentStr: String): Unit =
    ...
    val compareTo = if expected != null then expected else code
      ...
      "try statement tests" in {
          ...
          testS(
            """try:
                  ...
                  |except e, f:
                  |  y""".stripMargin,
            // PEP 758 (3.14): unparenthesised multiple exception types are a TUPLE, not the
            // Python 2 `except E, name:` binding the old grammar arm implemented here (a
            // SyntaxError in every 3.x before 3.14 - see Py314GrammarTests for the oracle).
            """try:
                  |  x
                  |except (e,f):
                  |  y""".stripMargin
  ...
  "type statement (type alias) tests" in {
      ...
      testT(
        "type Point[T, *Ts, **P] = tuple[T, Ts, P]",
        "type Point[T, *Ts, **P] = tuple[(T,Ts,P)]"
      )
