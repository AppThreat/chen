...
class VariableReferencingCpgTests extends AnyFreeSpec with Matchers:
  ...
  "reference from class method" - {
      ...
      "test capturing to global x exists" in {
          ...
          moduleLocal._closureBindingViaRefIn.next().closureBindingId shouldBe Some(
            "test.MyClass.f:x"
          )

          val bodyLocal =
              cpg.method.fullName("test.MyClass.<body>").local.name("x").head
...
end VariableReferencingCpgTests
