package io.appthreat.pysrc2cpg.config

import io.appthreat.pysrc2cpg.{NewMain, Py2CpgOnFileSystemConfig}
...
class ConfigTests extends AnyWordSpec with Matchers with Inside:
  ...
  "pysrc2cpg command line args should be parsed correctly" in {
      ...
      inside(X2Cpg.parseCommandLine(args, parser, Py2CpgOnFileSystemConfig())) {
          case Some(config) =>
              config.inputPath.endsWith("INPUT") shouldBe true
              config.outputPath shouldBe "OUTPUT"
              config.ignoredFiles.map(getSuffix(_, 13)).toSet shouldBe Set(
                "1EXCLUDE_FILE",
                "2EXCLUDE_FILE"
              )
