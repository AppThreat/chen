...
/** Task 12 Part A against REAL published distributions - the `flask-3.0.3` wheel and the
  ...
  *
  * Why this exists as a test rather than only as a shell gate: the gate asserted the produced
  * atom's BYTE SIZE (`> 1,000,000`, "was a silent 12,288"). That distinguishes "extracted
  * something" from "extracted nothing" and nothing finer - a size is not a content claim, it drifts
  * with unrelated changes, and when it drifts nobody can tell whether the graph got better or
  * worse. (It did drift: the review refactor moved it ~369 KB with the file and purl content
  * unchanged.) These are the numbers the size was standing in for, asserted directly.
  *
  * Skipped, not failed, where the corpus is absent: the fixtures are 150 KB of third-party archives
  * that do not belong in the repo, and a suite that fails on a machine without them teaches
...
class PythonRealArchiveIngestionTests extends AnyWordSpec with Matchers:
  ...
  private def withRepro(body: io.shiftleft.codepropertygraph.Cpg => Unit): Unit =
    assume(
      ...
      s"corpus packages not present at $packages"
    )
