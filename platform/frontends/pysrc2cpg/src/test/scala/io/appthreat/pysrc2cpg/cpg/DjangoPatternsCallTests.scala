...
/** Broader call-resolution coverage over real-world Django-style patterns. Each `should` block
  * isolates one resolution pattern so failures pinpoint the exact unresolved construct. These are
  * the next robustness targets after self-call resolution.
  */
class DjangoPatternsCallTests extends PySrc2CpgFixture(withOssDataflow = false):
  ...
  "super().__init__() in a subclass" should {
      ...
      "resolve the super().__init__() call to Base.__init__" ignore {
          ...
          call.map(_.methodFullName).toSet should contain("models.Base.__init__")
      }
  }
  ...
  "an inherited method invoked via self" should {
      ...
      "resolve self.helper() to the base class method" in {
          ...
          call.methodFullName shouldBe "models.Base.helper"
      }
  }
  ...
  "a module-level function call within the same module" should {
      ...
      "resolve to the module function" in {
          ...
          call.methodFullName shouldBe "version.get_version"
      }
  }
end DjangoPatternsCallTests
