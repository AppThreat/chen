...
import io.appthreat.x2cpg.passes.frontend.TypeNodePass.fullToShortName
...
/** Creates a `TYPE` node for each type in `usedTypes` as well as all inheritsFrom type names in the
  * CPG
  *
  * Alternatively, set `getTypesFromCpg = true`. If this is set, the `registeredTypes` argument will
  * be ignored. Instead, type nodes will be created for every unique `TYPE_FULL_NAME` value in the
  * CPG.
  */
class TypeNodePass private (
