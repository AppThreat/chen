...
class MethodStubCreator(cpg: Cpg) extends CpgPass(cpg):
    ...
    override def run(dstGraph: BatchedUpdate.DiffGraphBuilder): Unit =
        for method <- cpg.method do
            methodFullNameToNode.put(method.fullName, method)

        for call <- cpg.call if call.methodFullName != Defines.DynamicCallUnknownFullName do
        ...
        for
            (CallSummary(name, signature, fullName, dispatchType), parameterCount) <-
