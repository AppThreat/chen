  ...
  override def run(dstGraph: DiffGraphBuilder): Unit =
    ...
    cpg.typeDecl.whereNot(_.astParent).foreach { typeDecl =>
        addAstParent(
          typeDecl,
          typeDecl.fullName,
          typeDecl.astParentType,
          typeDecl.astParentFullName,
          dstGraph
        )
    }
  end run

  /** For the given method or type declaration, determine its parent in the AST via the
    * AST_PARENT_TYPE and AST_PARENT_FULL_NAME fields and create an AST edge from the parent to it.
    * AST creation to methods and type declarations is deferred in frontends in order to allow them
    * to process methods/type- declarations independently.
    */
    ...
    val astParentOption: Option[StoredNode] =
        astParentType match
          ...
          case NodeTypes.NAMESPACE_BLOCK =>
              namespaceBlockFullNameToNode(cpg, astParentFullName)
          case _ =>
              logger.debug(
