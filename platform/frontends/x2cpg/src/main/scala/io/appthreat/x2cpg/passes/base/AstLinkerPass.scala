...
class AstLinkerPass(cpg: Cpg) extends CpgPass(cpg) with LinkingUtil:
    ...
    /** For the given method or type declaration, determine its parent in the AST via the
      * AST_PARENT_TYPE and AST_PARENT_FULL_NAME fields and create an AST edge from the parent to
      * it. AST creation to methods and type declarations is deferred in frontends in order to allow
      * them to process methods/type- declarations independently.
      */
