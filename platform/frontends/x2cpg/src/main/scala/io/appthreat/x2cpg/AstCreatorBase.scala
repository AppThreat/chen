...
abstract class AstCreatorBase(filename: String)(implicit withSchemaValidation: ValidationMode):
  ...
  /** Creates an AST that represents an annotation, including its content (annotation parameter
    * assignments).
    */
  ...
  /** Creates an AST that represents an annotation assignment with a name for the assigned value,
    * its overall code, and the respective assignment AST.
    */
  def annotationAssignmentAst(
    assignmentValueName: String,
    code: String,
    assignmentAst: Ast
  ): Ast =
  ...
  /** Creates an AST that represents an entire method, including its content and with support for
    * both method and parameter annotations.
    */
  ...
  /** Creates an AST that represents a method stub, containing information about the method, its
  ...
  end staticInitMethodAst

  /** For a given return node and arguments, create an AST that represents the return instruction.
    * The main purpose of this method is to automatically assign the correct argument indices.
    */
  ...
  /** For a given node, condition AST and children ASTs, create an AST that represents the control
    * structure. The main purpose of this method is to automatically assign the correct condition
    * edges.
    */
      ...
      condition match
        case Some(conditionAst) =>
            Ast(controlStructureNode)
                .withChildren(if placeConditionLast then children :+ conditionAst
                else conditionAst +: children)
  ...
  /** For the given try body, catch ASTs and finally AST, create a try-catch-finally AST with orders
    * set correctly for the ossdataflow engine.
    */
  def tryCatchAst(
  ...
  /** For a given block node and statement ASTs, create an AST that represents the block. The main
    * purpose of this method is to increase the readability of the code which creates block asts.
    */
  def blockAst(blockNode: NewBlock, statements: List[Ast] = List()): Ast =
      Ast(blockNode).withChildren(statements)

  /** Create an abstract syntax tree for a call, including CPG-specific edges required for arguments
    * and the receiver.
    *
    * Our call representation is inspired by ECMAScript, that is, in addition to arguments, a call
    * has a base and a receiver. For languages other than Javascript, leave `receiver` empty for
    * now.
    *
    * @param callNode
  ...
  def callAst(
    ...
    val receiverRoot = if receiver.isEmpty && base.nonEmpty then
      baseRoot
    else
