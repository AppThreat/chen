...
abstract class AstCreatorBase(filename: String)(implicit withSchemaValidation: ValidationMode):
    ...
    /** For the given try body, catch ASTs and finally AST, create a try-catch-finally AST with
      * orders set correctly for the ossdataflow engine.
      */
    def tryCatchAst(
    ...
    /** Create an abstract syntax tree for a call, including CPG-specific edges required for
