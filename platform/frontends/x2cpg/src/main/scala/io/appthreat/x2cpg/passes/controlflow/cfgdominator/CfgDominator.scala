...
class CfgDominator[NodeType](adapter: CfgAdapter[NodeType]):
  ...
  /** Calculates the immediate dominators of all CFG nodes reachable from cfgEntry. Since the
    * cfgEntry does not have an immediate dominator, it has no entry in the return map.
    *
    * The algorithm is from: "A Simple, Fast Dominance Algorithm" from "Keith D. Cooper, Timothy J.
  ...
  def calculate(cfgEntry: NodeType): mutable.LinkedHashMap[NodeType, NodeType] =
    ...
    val nodesInReversePostOrder =
        NodeOrdering.reverseNodeList(postOrderNumbering.toList).filterNot(_ == cfgEntry)
    ...
    while changed do
      ...
      nodesInReversePostOrder.foreach { node =>
        ...
        expandBack(node).foreach { predecessor =>
          ...
          if safeDominators(predecessorIndex) != UNDEFINED then
            newImmediateDominator =
                intersect(dominators, predecessorIndex, newImmediateDominator)
        }

        val nodeIndex = indexOf(node)
        if dominators(nodeIndex) != newImmediateDominator then
          dominators(nodeIndex) = newImmediateDominator
          changed = true
      }
    end while

    val postOrderNumberingToNode = postOrderNumbering.map { case (node, index) =>
        (index, node)
    }
  ...
  private def intersect(
    ...
    immediateDomIndex1: Int,
    immediateDomIndex2: Int
  ): Int =
