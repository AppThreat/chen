...
object FileUtil:
  ...
  def deleteOnExit(
    ...
    linkOptions: LinkOption = LinkOption.NOFOLLOW_LINKS
      ...
      try
        if Files.isDirectory(file, linkOptions) then
  ...
  implicit class PathExt(p: Path):
    ...
    def copyTo(
      destination: Path,
      copyOption: StandardCopyOption = StandardCopyOption.COPY_ATTRIBUTES
    ): Unit =
        if Files.isDirectory(p) then // TODO: maxDepth?
          Files.walkFileTree(
            p,
    ...
    def createWithParentsIfNotExists(
      ...
      createParents: Boolean = false
        ...
        if !Files.exists(p) then
          if asDirectory then
            ...
            if createParents then Files.createDirectories(p.getParent)
            try
              Files.createFile(p)
            catch
    ...
    def mergeDirectory(
      directory: Path,
      copyOptions: StandardCopyOption = StandardCopyOption.COPY_ATTRIBUTES
    ): Unit =
