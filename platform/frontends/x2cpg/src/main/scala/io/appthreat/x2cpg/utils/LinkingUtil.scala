...
trait LinkingUtil:
    ...
    /** For all nodes `n` with a label in `srcLabels`, determine the value of `n.\$dstFullNameKey`,
      * use that to lookup the destination node in `dstNodeMap`, and create an edge of type
        ...
        cpg.graph.nodes(srcLabels*).foreach { srcNode =>
            ...
            if srcNode.outE(edgeType).isEmpty then
                ...
                srcNode.out(edgeType).property(Properties.FULL_NAME).nextOption() match
                    ...
                    case None =>
                        logger.debug(s"Missing outgoing edge of type $edgeType from node $srcNode")
                if !loggedDeprecationWarning then
                    logger.debug(
        ...
        cpg.graph.nodes(srcLabels*).asScala.cast[SRC_NODE_TYPE].foreach { srcNode =>
            if !srcNode.outE(edgeType).hasNext then
                ...
                dstGraph.setNodeProperty(
                  srcNode,
                  dstFullNameKey,
                  dstFullNames.map(dereference.dereferenceTypeFullName)
                )
                if !loggedDeprecationWarning then
                    logger.debug(
                      s"Using deprecated CPG format with already existing $edgeType edge between" +
