...
class EasyTagsPass(atom: Cpg) extends CpgPass(atom):
  ...
  private def tagCommonPatterns(dstGraph: DiffGraphBuilder): Unit =
    ...
    atom.method.internal.name(".*(login|authenti).*").newTagNode("authentication").store()(dstGraph)
  ...
  private def tagRubyPatterns(dstGraph: DiffGraphBuilder): Unit =
    ...
    httpLibraries.foreach { httpLib =>
      ...
      atom.call.code(httpClientMethod).newTagNode("http-client").store()(dstGraph)
  ...
  private def tagJavaScriptPatterns(dstGraph: DiffGraphBuilder): Unit =
    ...
    atom.call
        ...
        .foreach { methodName =>
            atom.method.nameExact(methodName).newTagNode("exported").store()(dstGraph)
        }
  ...
  private def tagPythonPatterns(dstGraph: DiffGraphBuilder): Unit =
    ...
    PY_REQUEST_PATTERNS.foreach(p =>
        atom.method.fullName(p).parameter.newTagNode("framework-input").store()(dstGraph)
    )

    PY_RESPONSE_PATTERNS.foreach { p =>
      atom.method.fullName(p).parameter.newTagNode("framework-output").store()(dstGraph)
      atom.call.code(p).newTagNode("framework-output").store()(dstGraph)
    }
    ...
    atom.identifier.typeFullName(cryptoLibs).newTagNode("crypto").store()(dstGraph)
    atom.call.methodFullName(cryptoLibs).newTagNode("crypto").store()(dstGraph)
    ...
    ).newTagNode("crypto-generate").store()(dstGraph)
  ...
  private def tagCPatterns(dstGraph: DiffGraphBuilder): Unit =
    ...
    atom.method.internal.name(eventPattern).parameter.newTagNode("event").store()(dstGraph)
    atom.method.internal.name(eventPattern).callIn(NoResolve).argument.newTagNode("event").store()(
      dstGraph
    )
    ...
    atom.method.internal.name(validationPattern).callIn(NoResolve).argument.newTagNode("validation")
        .store()(dstGraph)

    atom.method.internal.name(".*(parse[_A-Z]).*").parameter.newTagNode("parse").store()(dstGraph)

    // Library calls
    val libraryTags =
        Seq("json", "glibc", "regex", "decode", "wasm", "execution", "unicode", "utf8")
    libraryTags.foreach { tag =>
      atom.method.external.name(s".*$tag.*").callIn(NoResolve).argument.newTagNode(tag).store()(
        dstGraph
      )
      atom.method.external.name(s".*$tag.*").callIn(NoResolve).argument.newTagNode("library-call")
          .store()(dstGraph)
    }
    ...
    // Driver patterns
    atom.method.external.name("DriverEntry").parameter.newTagNode("driver-source").store()(dstGraph)
    atom.method.external.name("WdfDriverCreate").parameter.newTagNode("driver-source").store()(
      dstGraph
    )
    atom.method.external.name("OnDeviceAdd").parameter.newTagNode("driver-source").store()(dstGraph)
    ...
    atom.method.external.fullName("(Windows|WEX|WDMAudio|winrt|wilEx)(::|\\.).*").parameter
        .newTagNode("windows").store()(dstGraph)
    ...
    ).parameter.newTagNode("http").store()(dstGraph)
  ...
  private def tagJdkStandardClasses(dstGraph: DiffGraphBuilder): Unit =
    ...
    fileIoPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("io").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("io").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("io").store()(dstGraph)
    }
    // Network operations
    ...
    networkPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("network").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("network").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("network").store()(dstGraph)
    }
    // SQL operations
    val sqlPatterns = Seq(
      "java.sql.*",
    ...
    sqlPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("sql").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("sql").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("sql").store()(dstGraph)
    }
    // XML operations
    val xmlPatterns = Seq(
      "javax.xml.*",
    ...
    xmlPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("xml").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("xml").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("xml").store()(dstGraph)
    }
    ...
    concurrentPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("concurrent").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("concurrent").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("concurrent").store()(dstGraph)
    }
    // Process operations
    ...
    processPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("process").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("process").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("process").store()(dstGraph)
    }
    ...
    reflectionPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("reflection").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("reflection").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("reflection").store()(dstGraph)
    }
    // System operations
    ...
    systemPatterns.foreach { pattern =>
      atom.call.methodFullName(s"$pattern.*").newTagNode("system").store()(dstGraph)
      atom.identifier.typeFullName(s"$pattern.*").newTagNode("system").store()(dstGraph)
      atom.method.parameter.typeFullName(s"$pattern.*").newTagNode("system").store()(dstGraph)
    }
  ...
  private def tagJavaPatterns(dstGraph: DiffGraphBuilder): Unit =
    ...
    cryptoPatterns.foreach { pattern =>
      atom.identifier.typeFullName(pattern).newTagNode("crypto").store()(dstGraph)
      atom.call.methodFullName(pattern).newTagNode("crypto").store()(dstGraph)
    }
    ...
    cryptoGeneratePatterns.foreach { pattern =>
        atom.call.methodFullName(pattern).newTagNode("crypto-generate").store()(dstGraph)
    }
