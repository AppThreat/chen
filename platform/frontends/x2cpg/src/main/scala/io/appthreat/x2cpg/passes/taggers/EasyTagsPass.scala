...
class EasyTagsPass(atom: Cpg) extends CpgPass(atom):
  ...
  private def tagJavaSinkFamilies(dstGraph: DiffGraphBuilder): Unit =
    ...
    // SSRF vs plain client usage is decided at TARGET CONSTRUCTION, not at the request call:
    // the receiver of `openStream`/`send`/`execute` is essentially always an identifier or
    // call, so an argument-label test at the request site can never distinguish a constant URL
    // from an attacker-steerable one. A `URL`/`URI` built from a NON-LITERAL argument is the
    // ssrf shape; a literal target is inventory only (`http-client`). The request calls
    // themselves are always `http-client`.
    val urlConstructionPatterns = Seq(
      "java.net.URL.<init>.*",
      "java.net.URI.create.*"
    )
    urlConstructionPatterns.foreach { pattern =>
      val constructions = atom.call.methodFullName(pattern).l
      constructions.iterator
          .filter(c =>
              // Argument 1 is the target string/URI (argument 0 is the receiver); a literal
              // there is a fixed endpoint, anything else may be steerable.
              c.argument.argumentIndex(1).exists(a => a.label != "LITERAL")
          )
          .newTagNode("ssrf")
          .store()(using dstGraph)
      constructions.iterator.newTagNode("http-client").store()(using dstGraph)
    }

    val httpClientPatterns = Seq(
    ...
    httpClientPatterns.foreach { pattern =>
        atom.call
