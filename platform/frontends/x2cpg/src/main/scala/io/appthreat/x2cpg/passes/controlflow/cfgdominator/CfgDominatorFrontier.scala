...
/** Calculates the dominator frontier for a set of CFG nodes. The returned multimap associates the
  * frontier nodes to each CFG node.
  *
  * The used algorithm is from: "A Simple, Fast Dominance Algorithm" from "Keith D. Cooper, Timothy
  * J. Harvey, and Ken Kennedy".
  */
class CfgDominatorFrontier[NodeType](
  cfgAdapter: CfgAdapter[NodeType],
  domTreeAdapter: DomTreeAdapter[NodeType]
):
    ...
    def calculate(cfgNodes: Seq[NodeType]): mutable.Map[NodeType, mutable.Set[NodeType]] =
        ...
        do
            preds.foreach { p =>
                ...
                while currentPred.isDefined && currentPred.get != joinNodeIDom do
                    val frontierNodes =
                        domFrontier.getOrElseUpdate(currentPred.get, mutable.Set.empty)
