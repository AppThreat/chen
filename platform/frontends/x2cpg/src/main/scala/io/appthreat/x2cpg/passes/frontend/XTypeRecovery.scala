...
import io.appthreat.x2cpg.{Defines, X2CpgConfig}
...
/** In order to propagate types across compilation units, but avoid the poor scalability of a
  ...
  * parameter. Note that iterations < 2 will not provide any interprocedural type recovery
  * capabilities.
  * @param cpg
            ...
            try
                ...
                if stopEarly.get() && config.enabledDummyTypes then
                    generateRecoveryPass(
                      state.copy(currentIteration = config.iterations - 1)
                    ).createAndApply()
            finally
                state.clear()

    protected def generateRecoveryPass(state: XTypeRecoveryState)
      : XTypeRecovery[CompilationUnitType]
end XTypeRecoveryPass
...
/** Based on a flow-insensitive static single-assignment symbol-table-style approach. This pass aims
  * to be fast and deterministic and does not try to converge to some fixed point but rather
  * iterates a fixed number of times. This will help recover: <ol><li>Imported call signatures from
  * external dependencies</li><li>Dynamic type hints for mutable variables in a compilation
  * unit.</ol>
  *
  * The algorithm flows roughly as follows: <ol> <li> Scan for method signatures of methods for each
  * compilation unit, either by internally defined methods or by reading import signatures. This
  * includes looking for aliases, e.g. import foo as bar.</li><li>(Optionally) Prune these method
  * signatures by checking their validity against the CPG.</li><li>Visit assignments to populate
  * where variables are assigned a value to extrapolate its type. Store these values in a local
  * symbol table. If a field is assigned a value, store this in the global table</li><li>Find
  * instances of where these fields and variables are used and update their type
  * information.</li><li>If this variable is the receiver of a call, make sure to set the type of
  * the call accordingly.</li></ol>
  *
  * The symbol tables use the [[SymbolTable]] class to track possible type information. <br>
  * <strong>Note: Local symbols are cleared once a compilation unit is complete. This is to keep
  * memory usage down while maximizing concurrency.</strong>
  *
  * @param cpg
  *   the CPG to recovery types for.
  * @tparam CompilationUnitType
...
abstract class XTypeRecovery[CompilationUnitType <: AstNode](cpg: Cpg, state: XTypeRecoveryState)
    ...
    override def run(builder: DiffGraphBuilder): Unit =
        val changesWereMade = compilationUnit
            ...
            .getOrElse(false)
        if !changesWereMade then state.stopEarly.set(true)
    ...
    /** A factory method to generate a [[RecoverForXCompilationUnit]] task with the given
      * parameters.
      *
      * @param unit
...
object XTypeRecovery:
    ...
    private lazy val DummyTokens: Set[String] =
        Set(DummyReturnType, DummyMemberLoad, DummyIndexAccess)
        ...
        OParser.sequence(
          ...
          opt[Int]("type-prop-iterations")
              ...
              .validate { x =>
                  if x <= 0 then
                      logger.debug(
                        "Disabling type propagation as the given iteration count is <= 0"
                      )
                  else if x == 1 then
...
abstract class RecoverForXCompilationUnit[CompilationUnitType <: AstNode](
  ...
  state: XTypeRecoveryState
    ...
    protected val codeRoot: String =
        cpg.metaData.root.headOption.getOrElse("") + java.io.File.separator
    ...
    /** For tracking members and the type operations that need to be performed. Since these are
      * mostly out of scope locally it helps to track these separately.
      *
    ...
    protected def prepopulateSymbolTableEntry(x: AstNode): Unit = x match
        case x @ (_: Identifier | _: Local | _: MethodParameterIn) =>
            symbolTable.append(x, x.getKnownTypes)
    ...
    protected def assignments: Iterator[Assignment] = cu match
        case x: File =>
            x.method.flatMap(_._callViaContainsOut).nameExact(Operators.assignment).map(
              new OpNodes.Assignment(_)
            )
        case x: Method => x.flatMap(_._callViaContainsOut).nameExact(Operators.assignment).map(
              new OpNodes.Assignment(_)
            )
    ...
    /** The initial import setting is over-approximated, so this step checks the CPG for any matches
      * and prunes against these findings. If there are no findings, it will leave the table as is.
      * The latter is significant for external types or methods.
      */
    protected def postVisitImports(): Unit = {}

    /** Using assignment and import information (in the global symbol table), will propagate these
    ...
    protected def visitAssignments(a: Assignment): Set[String] =
        a.argumentOut.l match
            ...
            case List(i: Identifier, l: Literal) if state.isFirstIteration =>
            ...
            case List(c: Call, l: Literal) if state.isFirstIteration =>
    ...
    protected def visitIdentifierAssignedToBlock(i: Identifier, b: Block): Set[String] =
        ...
        if blockTypes.nonEmpty then associateTypes(i, blockTypes)
        else Set.empty
    ...
    /** Process each statement but only assign the type of the last statement to the identifier
      */
    protected def visitStatementsInBlock(
      b: Block,
      assignmentTarget: Option[Identifier] = None
    ): Set[String] =
        b.astChildren
            .map {
                case x: Call if x.name.startsWith(Operators.assignment) =>
                    visitAssignments(new Assignment(x))
                ...
                case x =>
                    logger.debug(
                      s"Unhandled block element ${x.label}:${x.code} @ ${debugLocation(x)}"
                    ); Set.empty[String]
            }
            .lastOption
            .getOrElse(Set.empty[String])

    /** Visits an identifier being assigned to a call. This call could be an operation, function
    ...
    protected def visitIdentifierAssignedToCall(i: Identifier, c: Call): Set[String] =
        ...
        else if symbolTable.contains(c) && isConstructor(c) then
    ...
    /** Visits an identifier being assigned to the value held by another identifier. This is a weak
      * copy.
      */
    protected def visitIdentifierAssignedToIdentifier(x: Identifier, y: Identifier): Set[String] =
        if symbolTable.contains(y) then associateTypes(x, symbolTable.get(y))
        else Set.empty

    /** Will build a call full path using the call base node. This method assumes the base node is
      * in the symbol table.
      */
    protected def setCallMethodFullNameFromBase(c: Call): Set[String] =
        val recTypes = c.argument.headOption
            .map {
                ...
                case x: Call =>
                    cpg.method.fullNameExact(c.methodFullName).methodReturn.typeFullNameNot(
                      "ANY"
                    ).typeFullName.toSet match
                        case xs if xs.nonEmpty => xs
                        case _ => symbolTable.get(x).map(t =>
                                Seq(t, XTypeRecovery.DummyReturnType).mkString(pathSep.toString)
                            )
    ...
    /** A heuristic method to determine if an identifier may be a field or not. The result means
      * that it would be stored in the global symbol table. By default this checks if the identifier
      * name matches a member name.
      *
    ...
    protected def associateInterproceduralTypes(
      ...
      baseTypes: Set[String]
        ...
        if globalTypes.nonEmpty then
            // We have been able to resolve the type inter-procedurally
            associateTypes(i, globalTypes)
        else if baseTypes.nonEmpty then
            if baseTypes.equals(symbolTable.get(LocalVar(fieldFullName))) then
                associateTypes(i, baseTypes)
            else
                // If not available, use a dummy variable that can be useful for call matching
                associateTypes(
                  i,
                  baseTypes.map(t => XTypeRecovery.dummyMemberType(t, fieldName, pathSep))
                )
        else
            // Assign dummy
            val dummyTypes = Set(
              XTypeRecovery.dummyMemberType(
                fieldFullName.stripSuffix(s"$pathSep$fieldName"),
                fieldName,
                pathSep
              )
            )
    ...
    /** Visits an identifier being assigned to an operator call.
      */
    protected def visitIdentifierAssignedToOperator(
      i: Identifier,
      c: Call,
      operation: String
    ): Set[String] =
        operation match
            ...
            case x =>
                logger.debug(s"Unhandled operation $x (${c.code}) @ ${debugLocation(c)}"); Set.empty

    /** Visits an identifier being assigned to a constructor and attempts to speculate the
      * constructor path.
      */
    protected def visitIdentifierAssignedToConstructor(i: Identifier, c: Call): Set[String] =
        val constructorPaths =
            symbolTable.get(c).map(t => t.concat(s"$pathSep${Defines.ConstructorMethodName}"))
    ...
    protected def visitIdentifierAssignedToCallRetVal(i: Identifier, c: Call): Set[String] =
        if symbolTable.contains(c) then
            val callReturns = methodReturnValues(symbolTable.get(c).toSeq)
            associateTypes(i, callReturns)
        else if c.argument.exists(_.argumentIndex == 0) then
            val callFullNames = (c.argument(0) match
                case i: Identifier if symbolTable.contains(LocalVar(i.name)) =>
                    symbolTable.get(LocalVar(i.name))
                case i: Identifier if symbolTable.contains(CallAlias(i.name)) =>
                    symbolTable.get(CallAlias(i.name))
                case _ => Set.empty
    ...
    /** Will attempt to find the return values of a method if in the CPG, otherwise will give a
      * dummy value.
      */
    protected def methodReturnValues(methodFullNames: Seq[String]): Set[String] =
        val rs = cpg.method
            ...
            .toSet
        if rs.isEmpty then
            methodFullNames.map(_.concat(s"$pathSep${XTypeRecovery.DummyReturnType}")).toSet
        else rs
    ...
    /** Not all frontends populate <code>typeFullName</code> for literals so we allow this to be
      * overridden.
      */
            ...
            getSymbolFromCall(x) match
                case (lhs, globalKeys) if globalKeys.nonEmpty =>
                    globalKeys.foreach { (fieldVar: FieldPath) =>
                        persistMemberWithTypeDecl(
                          fieldVar.compUnitFullName,
                          fieldVar.identifier,
                          types
                        )
                    }
    ...
    protected def getIndexAccessTypes(ia: Call): Set[String] = indexAccessToCollectionVar(ia) match
        ...
        case Some(cVar) if symbolTable.contains(LocalVar(cVar.identifier)) =>
            symbolTable.get(LocalVar(cVar.identifier)).map(x =>
                s"$x$pathSep${XTypeRecovery.DummyIndexAccess}"
            )
        case _ => Set.empty
    ...
    /** Tries to identify the underlying symbol from the call operation as it is used on the LHS of
      * an assignment. The second element is a list of any associated global keys if applicable.
      */
    protected def getSymbolFromCall(c: Call): (LocalKey, Set[FieldPath]) = c.name match
        ...
        case Operators.indexAccess =>
            (indexAccessToCollectionVar(c).getOrElse(LocalVar(c.name)), Set.empty)
        case x =>
    ...
    protected def getFieldName(fa: FieldAccess, prefix: String = "", suffix: String = ""): String =
        def wrapName(n: String) =
            ...
            if prefix.nonEmpty then sb.append(s"$prefix$pathSep")
            sb.append(n)
            if suffix.nonEmpty then sb.append(s"$pathSep$suffix")
            sb.toString()
        ...
        fa.argumentOut.l match
            case ::(i: Identifier, ::(f: FieldIdentifier, _)) if i.name.matches("(self|this)") =>
                wrapName(f.canonicalName)
            case ::(i: Identifier, ::(f: FieldIdentifier, _)) =>
                wrapName(s"${i.name}$pathSep${f.canonicalName}")
            ...
            case ::(c: Call, ::(f: FieldIdentifier, _)) =>
                // TODO: Handle this case better
                val callCode =
                    if c.code.contains("(") then c.code.substring(c.code.indexOf("(")) else c.code
    ...
    protected def visitCallAssignedToLiteral(c: Call, l: Literal): Set[String] =
        if c.name.equals(Operators.indexAccess) then
            ...
            c.argumentOut.l match
                ...
                case xs =>
                    logger.debug(
                      s"Unhandled index access point assigned to literal ${xs.map(x =>
                              (x.label, x.code)
                          ).mkString(",")} @ ${debugLocation(c)}"
                    )
                    Set.empty
        else if c.name.equals(Operators.fieldAccess) then
            val fa        = new FieldAccess(c)
            val fieldName = getFieldName(fa)
            associateTypes(LocalVar(fieldName), fa, getLiteralType(l))
        else
            logger.debug(
              s"Unhandled call assigned to literal point ${c.name} @ ${debugLocation(c)}"
            )
            Set.empty
    ...
    protected def indexAccessToCollectionVar(c: Call): Option[CollectionVar] =
        def callName(x: Call) =
            if x.name.equals(Operators.fieldAccess) then
                getFieldName(new FieldAccess(x))
            else if x.name.equals(Operators.indexAccess) then
        ...
        Option(c.argumentOut.l match
            ...
            case xs =>
                logger.debug(
                  s"Unhandled index access ${xs.map(x => (x.label, x.code)).mkString(",")} @ ${debugLocation(c)}"
                )
                null
        )
    end indexAccessToCollectionVar
    ...
    protected def visitIdentifierAssignedToFieldLoad(i: Identifier, fa: FieldAccess): Set[String] =
        ...
        fa.argumentOut.l match
            case ::(base: Identifier, ::(fi: FieldIdentifier, _))
                if symbolTable.contains(LocalVar(base.name)) =>
            ...
            case ::(c: Call, ::(fi: FieldIdentifier, _)) if c.name.equals(Operators.fieldAccess) =>
                ...
                for segment <- baseName.split(pathSep) ++ Array(fi.canonicalName) do
                    val types =
                        if buf.isEmpty then symbolTable.get(LocalVar(segment))
                        else
                            buf.flatMap(t => symbolTable.get(LocalVar(s"$t$pathSep$segment"))).toSet
                    if types.nonEmpty then
                        buf.clear()
                        buf.addAll(types)
                    else
            ...
            case ::(call: Call, ::(fi: FieldIdentifier, _)) =>
                assignTypesToCall(
                  call,
                  Set(fieldName.stripSuffix(
                    s"${XTypeRecovery.DummyMemberLoad}$pathSep${fi.canonicalName}"
                  ))
                )
            case _ =>
                logger.debug(
                  s"Unable to assign identifier '${i.name}' to field load '$fieldName' @ ${debugLocation(i)}"
                )
                Set.empty
        end match
    end visitIdentifierAssignedToFieldLoad
    ...
    protected def visitReturns(ret: Return): Unit =
        ...
        def extractTypes(xs: List[CfgNode]): Set[String] = xs match
            ...
            case ::(head: Call, Nil) if head.name == Operators.fieldAccess =>
                ...
                val cpgTypes = cpg.typeDecl
                    ...
                    .toSet
                if cpgTypes.nonEmpty then cpgTypes
            ...
            case ::(head: Call, Nil) if head.argumentOut.headOption.exists(symbolTable.contains) =>
                symbolTable
                    ...
                    .map(t =>
                        Seq(t, head.name, XTypeRecovery.DummyReturnType).mkString(pathSep.toString)
                    )
        ...
        builder.setNodeProperty(
          ret.method.methodReturn,
          PropertyNames.DYNAMIC_TYPE_HINT_FULL_NAME,
          existingTypes
        )
    end visitReturns

    /** Using an entry from the symbol table, will queue the CPG modification to persist the
        ...
        cu.ast
            ...
            .foreach {
                case x: Local if symbolTable.contains(x) =>
                    storeNodeTypeInfo(x, symbolTable.get(x).toSeq)
    ...
    end setTypeInformation

    protected def createCallFromIdentifierTypeFullName(
      typeFullName: String,
      callName: String
    ): String =
    ...
    private def setTypeInformationForRecCall(x: AstNode, n: Option[Call], ms: List[AstNode]): Unit =
        (n, ms) match
            ...
            case (Some(call: Call), ::(i: Identifier, ::(c: Call, _)))
                if call.name == Operators.assignment =>
    ...
    end setTypeInformationForRecCall
    ...
    protected def setTypeForFieldAccess(
      ...
      f: FieldIdentifier
        ...
        val idHints = if symbolTable.contains(i) then symbolTable.get(i)
        else symbolTable.get(CallAlias(i.name))
    ...
    protected def setTypeForDynamicDispatchCall(call: Call, i: Identifier): Unit =
        ...
        persistType(i, idHints)
        if callTypes.isEmpty && !call.name.startsWith("<operator>") then
    ...
    protected def setTypeForIdentifierAssignedToCall(call: Call, i: Identifier, c: Call): Unit =
        val idTypes = if symbolTable.contains(i) then symbolTable.get(i)
        else symbolTable.get(CallAlias(i.name))
        val callTypes = symbolTable.get(c)
        persistType(call, callTypes)
        if idTypes.nonEmpty || callTypes.nonEmpty then
            if idTypes.equals(callTypes) then
    ...
    protected def setTypeFromTypeHints(n: StoredNode): Unit =
        ...
        if types.nonEmpty then setTypes(n, types.toSeq)

    /** In the case this field access is a function pointer, we would want to make sure this has a
      * method ref.
      */
    private def handlePotentialFunctionPointer(
      ...
      baseName: Option[String] = None
        ...
        if !funcPtr.astParent.iterator.collectAll[Call].exists(_.name == funcName) then
            baseTypes
                .map(t => if t.endsWith(funcName) then t else s"$t$pathSep$funcName")
                .flatMap(cpg.method.fullNameExact)
                .filterNot(m =>
                    addedNodes.contains(
                      s"${funcPtr.id()}${NodeTypes.METHOD_REF}$pathSep${m.fullName}"
                    )
                )
                .map(m =>
                    m -> createMethodRef(
                      ...
                      m.fullName,
                      funcPtr.lineNumber,
                      funcPtr.columnNumber
                    )
                )
                .foreach { case (m, mRef) =>
                    funcPtr.astParent
                        .filterNot(
                          _.astChildren.isMethodRef.exists(_.methodFullName == mRef.methodFullName)
                        )
                        .foreach { inCall =>
    ...
    /** Integrate this method ref node into the CPG according to schema rules. Since we're adding
      * this after the base passes, we need to add the necessary linking manually.
      */
    private def integrateMethodRef(
      funcPtr: Expression,
      m: Method,
      mRef: NewMethodRef,
      inCall: AstNode
    ...
    protected def persistType(x: StoredNode, types: Set[String]): Unit =
        val filteredTypes = if state.config.enabledDummyTypes then types
        else types.filterNot(XTypeRecovery.isDummyType)
        if filteredTypes.nonEmpty then
            storeNodeTypeInfo(x, filteredTypes.toSeq)
            x match
                case i: Identifier if symbolTable.contains(i) =>
                    if isField(i) then persistMemberType(i, filteredTypes)
    ...
    /** Type decls where member access are required need to point to the correct type decl that
      * holds said members. This allows implementations to use the type names to find the correct
      ...
      */
    protected def typeDeclIterator(typeFullName: String): Iterator[TypeDecl] =
        cpg.typeDecl.fullNameExact(typeFullName)
    ...
    /** Given a type full name and member name, will persist the given types to the member.
      ...
      */
    protected def persistMemberWithTypeDecl(
      typeFullName: String,
      memberName: String,
      types: Set[String]
    ): Unit =
    ...
    /** Given an identifier that has been determined to be a field, an attempt is made to get the
      * corresponding member. This implementation follows more the way dynamic languages define
      * method/type relations.
      * @param i
      *   the identifier.
      * @return
    ...
    private def storeNodeTypeInfo(storedNode: StoredNode, types: Seq[String]): Unit =
        ...
        if types.nonEmpty && types.toSet != existingTypes then
            storedNode match
                case m: Member =>
    ...
    protected def storeDefaultTypeInfo(n: StoredNode, types: Seq[String]): Unit =
        if types.toSet != n.getKnownTypes then
            state.changesWereMade.compareAndSet(false, true)
            setTypes(
              n,
              (n.property(PropertyNames.DYNAMIC_TYPE_HINT_FULL_NAME, Seq.empty) ++ types).distinct
            )

    /** If there is only 1 type hint then this is set to the `typeFullName` property and
      * `dynamicTypeHintFullName` is cleared. If not then `dynamicTypeHintFullName` is set to the
      * types.
      */
    ...
    protected def storeLocalTypeInfo(l: Local, types: Seq[String]): Unit =
        storeDefaultTypeInfo(
          l,
          if state.config.enabledDummyTypes then types
          else types.filterNot(XTypeRecovery.isDummyType)
        )
    ...
    implicit class AllNodeTypesFromIteratorExt(x: Iterator[StoredNode]):
        ...
        def getKnownTypes: Set[String] =
            x.allTypes.toSet.filterNot(unknownTypePattern.matches)
end RecoverForXCompilationUnit
