...
abstract class RecoverForXCompilationUnit[CompilationUnitType <: AstNode](
  ...
  /** For tracking members and the type operations that need to be performed. Since these are mostly
    * out of scope locally it helps to track these separately.
    *
  ...
  /** Will build a call full path using the call base node. This method assumes the base node is in
    * the symbol table.
    */
  ...
  /** A heuristic method to determine if an identifier may be a field or not. The result means that
    * it would be stored in the global symbol table. By default this checks if the identifier name
    * matches a member name.
    *
  ...
  /** Visits an identifier being assigned to a constructor and attempts to speculate the constructor
    * path.
    */
  ...
  /** Will attempt to find the return values of a method if in the CPG, otherwise will give a dummy
    * value.
    */
  ...
  /** Tries to identify the underlying symbol from the call operation as it is used on the LHS of an
    * assignment. The second element is a list of any associated global keys if applicable.
    */
  ...
  /** Using an entry from the symbol table, will queue the CPG modification to persist the recovered
    * type information.
    */
  ...
  /** Integrate this method ref node into the CPG according to schema rules. Since we're adding this
    * after the base passes, we need to add the necessary linking manually.
    */
  ...
  /** Type decls where member access are required need to point to the correct type decl that holds
    * said members. This allows implementations to use the type names to find the correct type
    * holding members.
    * @param typeFullName
    *   the type full name.
    * @return
