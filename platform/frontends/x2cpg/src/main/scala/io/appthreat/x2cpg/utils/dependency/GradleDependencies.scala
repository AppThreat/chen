...
case class GradleProjectInfo(
  ...
  def gradleVersionMajorMinor(): (Int, Int) =
    ...
    if parts.length == 1 && isValidPart(parts(0)) then
      (parts(0).toInt, 0)
    else if parts.length >= 2 && isValidPart(parts(0)) && isValidPart(parts(1)) then
...
object GradleDependencies:
      ...
      Try(makeConnection(projectDir.toFile)) match
        case Success(gradleConnection) =>
            Using.resource(gradleConnection) { connection =>
                try
                  val buildEnv =
                      connection.getModel[BuildEnvironment](classOf[BuildEnvironment])
                  ...
                  val hasAndroidPrefixGradleProperty =
                      runGradleTask(connection, Constants.gradlePropertiesTaskName) match
                        case Some(out) =>
                            out.split('\n').exists(
                              _.startsWith(Constants.gradleAndroidPropertyPrefix)
                            )
                        case None => false
                  ...
                  if hasAndroidPrefixGradleProperty then
                    val validProjectNames = List(
                      project.getName
                    ) ++ project.getChildren.getAll.asScala.map(_.getName)
                    logger.debug(
                      s"Found Gradle projects: ${validProjectNames.mkString(",")}"
                    )
                    if !validProjectNames.contains(projectName) then
                  ...
                  case t: Throwable =>
                      logger.debug(
                        s"Caught exception while trying use Gradle connection: ${t.getMessage}"
                      )
        ...
        case Failure(t) =>
            logger.debug(
              s"Caught exception while trying fetch Gradle project information: ${t.getMessage}"
            )
            logger.debug(s"Full exception: ", t)
            None
  ...
  private def runGradleTask(connection: ProjectConnection, taskName: String): Option[String] =
      Using.resource(new ByteArrayOutputStream()) { out =>
          Try(
            ...
            case Failure(ex) =>
                logger.debug(
                  s"Caught exception while executing Gradle task named `$taskName`:",
                  ex.getMessage
                )
  ...
  private def runGradleTask(
    ...
    initScriptPath: String
      ...
      Using.resources(new ByteArrayOutputStream, new ByteArrayOutputStream) {
          case (stdoutStream, stderrStream) =>
              logger.debug(s"Executing gradle task '${initScript.taskName}'...")
              Try(
                connection
                    .newBuild()
                ...
                case Failure(ex) =>
                    logger.debug(
                      s"Caught exception while executing Gradle task: ${ex.getMessage}"
                    )
  ...
  private def extractClassesJarFromAar(aar: File): Option[Path] =
    ...
    val classesJarEntries =
        outDir.listRecursively
            ...
            .toList
    if classesJarEntries.size != 1 then
  ...
  private[dependency] def get(
    ...
    logger.debug(
      s"Fetching Gradle project information at path `$projectDir` with project name `$projectName`."
    )
                ...
                Try(File.newTemporaryFile(initScriptPrefix).deleteOnExit()) match
                  case Success(initScriptFile) =>
                      ...
                      Try(makeConnection(projectDir.toFile)) match
                        case Success(connection) =>
                            Using.resource(connection) { c =>
                                runGradleTask(
                                  ...
                                  case Some(deps) =>
                                      Some(deps.map { d =>
                                          if !d.endsWith(Constants.aarFileExtension)
                                          then d
                                          else
                        ...
                        case Failure(ex) =>
                            logger.debug(
                              s"Caught exception while trying to establish a Gradle connection: ${ex.getMessage}"
                            )
                            logger.debug(s"Full exception: ", ex)
                            None
                      end match
                  case Failure(ex) =>
                      logger.debug(
                        s"Could not create temporary file for Gradle init script: ${ex.getMessage}"
                      )
                      logger.debug(s"Full exception: ", ex)
                      None
            case Failure(ex) =>
                logger.debug(
                  s"Could not create temporary directory for saving dependency files: ${ex.getMessage}"
                )
