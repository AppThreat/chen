...
import io.appthreat.x2cpg.frontendspecific.ruby2atom.Constants.*
...
/** In some Ruby frameworks, it is common to have an autoloader library that implicitly loads
  * requirements onto the stack. This pass makes these imports explicit. The most popular one is <a
  * href="https://github.com/fxn/zeitwerk">Zeitwerk</a> which we check in `Gemsfile.lock` to enable
  * this pass.
  *
  * @param externalTypes
...
class ImplicitRequirePass(cpg: Cpg, externalTypes: Seq[TypeImportInfo] = Nil)
  ...
  /** A tuple holding information about the type import info, additionally with a boolean indicating
    * if it is external or not.
    */
  ...
  override def init(): Unit =
    val importableTypeInfo = cpg.typeDecl
        ...
        .map { typeDecl =>
          val typeImportInfo =
              TypeImportInfo(typeDecl.name, ImplicitRequirePass.normalizePath(typeDecl.filename))
    ...
    typeNameToImportInfo.addAll(importableTypeInfo.groupBy {
  ...
  override def generateParts(): Array[Method] =
      cpg.method.whereNot(_.astChildren.isCall.nameExact(Require)).toArray
        ...
        .flatMap { identifierName =>
            typeNameToImportInfo
                ...
                .collectFirst {
                    ...
                    case TypeImportInfoWithProvidence(TypeImportInfo(_, importPath), _)
                        if importPath != currPath => importPath
                }
        }
        .distinct
        .foreach { importPath =>
  ...
  private def createRequireCall(builder: DiffGraphBuilder, path: String): NewCall =
    ...
    val pathLiteralNode =
        NewLiteral().code(s"'$path'").typeFullName(s"$kernelPrefix.String").argumentIndex(1).order(
          2
        )
  ...
  private def fieldAccessBase(fa: FieldAccess): String =
      fieldAccessParts(fa).headOption.getOrElse(fa.argument(1).code)
...
object ImplicitRequirePass:
  ...
  /** Determines if the given type name and its corresponding parent file name allow for the type to
    * be autoloaded by zeitwerk.
    * @return
