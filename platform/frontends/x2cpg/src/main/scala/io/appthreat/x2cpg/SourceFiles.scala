...
object SourceFiles:
    ...
    /** For given input paths, determine all source files by inspecting filename extensions and
      * filter the result according to the given config (by its ignoredFilesRegex and ignoredFiles).
      */
    def determine(
    ...
    /** Constructs an absolute path against rootPath. If the given path is already absolute this
      * path is returned unaltered. Otherwise, "rootPath / path" is returned.
      */
    ...
    /** Constructs a relative path against rootPath. If the given path is not inside rootPath, path
      * is returned unaltered. Otherwise, the path relative to rootPath is returned.
      */
