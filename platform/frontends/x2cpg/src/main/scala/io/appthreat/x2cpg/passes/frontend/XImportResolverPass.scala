...
import io.appthreat.x2cpg.passes.frontend.ImportsPass.ResolvedImport
import io.appthreat.x2cpg.passes.frontend.ImportsPass.ResolvedImport.*
...
abstract class XImportResolverPass(cpg: Cpg) extends ConcurrentWriterCpgPass[Import](cpg):
    ...
    protected def resolvedImportToTag(
      x: ResolvedImport,
      importCall: Call,
      diffGraph: DiffGraphBuilder
    ): Unit =
...
object ImportsPass:
    ...
    case class ResolvedMethod(
      ...
      override val label: String = RESOLVED_METHOD
        ...
        override def serialize: String =
            s"$OPT_FULL_NAME,$fullName,$OPT_ALIAS,$alias" + receiver.map(r =>
                s",$OPT_RECEIVER,$r"
            ).getOrElse("")
    ...
    case class UnknownImport(path: String, override val label: String = UNKNOWN_IMPORT)
        extends ResolvedImport:
