...
object Ast:
    ...
    /** For all `order` fields that are unset, derive the `order` field automatically by determining
      * the position of the child among its siblings.
      */
    private def setOrderWhereNotSet(ast: Ast): Unit =
        ast.root.collect { case r: AstNodeNew =>
            if r.order == PropertyDefaults.Order then
                r.order = 1
        }
        val siblings = ast.edges.groupBy(_.src).map { case (_, edgeToChild) =>
            edgeToChild.map(_.dst)
        }
        siblings.foreach { children =>
            children.zipWithIndex.collect { case (c: AstNodeNew, i) =>
                if c.order == PropertyDefaults.Order then
                    c.order = i + 1
            }
        }
end Ast
...
case class Ast(
  ...
  argEdges: collection.Seq[AstEdge] = Vector.empty
    ...
    /** AST that results when adding `other` as a child to this AST. `other` is connected to this
      * AST's root node.
      */
    ...
    /** AST that results when adding all ASTs in `asts` as children, that is, connecting them to the
    ...
    /** Returns a deep copy of the sub tree rooted in `node`. If `order` is set, then the `order`
      ...
      * set, then this replaces `node` in the new copy.
      */
    def subTreeCopy(
      node: AstNodeNew,
      argIndex: Int = -1,
      replacementNode: Option[AstNodeNew] = None
        ...
        val newConditionEdges =
            conditionEdges.filter(_.src == node).map(x => AstEdge(newNode, newIfExists(x.dst)))
        val newRefEdges =
            refEdges.filter(_.src == node).map(x => AstEdge(newNode, newIfExists(x.dst)))
        val newBindsEdges =
