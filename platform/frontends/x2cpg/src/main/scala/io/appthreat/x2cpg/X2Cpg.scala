...
trait X2CpgConfig[R <: X2CpgConfig[R]]:
  ...
  def createPathForIgnore(ignore: String): String =
    ...
    if path.isAbsolute then path.toString
...
/** Base class for `Main` classes of CPG frontends.
  *
  * Main classes that inherit from this base class parse the command line, exiting with an error
  * code if this does not succeed. On success, the method `run` is called, which evaluates, given a
  * frontend and a configuration, creates the CPG and stores it on disk.
  *
  * @param cmdLineParser
...
trait X2CpgFrontend[T <: X2CpgConfig[?]]:
  ...
  /** Create CPG according to given configuration, printing errors to the console if they occur. The
    * CPG is closed and not returned.
    */
  ...
  /** Create a CPG for code at `inputName` (a single location) with default frontend configuration.
    * If `outputName` exists, it is the file name of the resulting CPG. Otherwise, the CPG is held
    * in memory.
    */
  def createCpg(inputName: String, outputName: Option[String])(implicit
    defaultConfig: T
  ): Try[Cpg] =
    val defaultWithInputPath = defaultConfig.withInputPath(inputName).asInstanceOf[T]
    val config = if !outputName.contains(X2CpgConfig.defaultOutputPath) then
      if outputName.isEmpty then
...
object X2Cpg:
  ...
  /** Parse commands line arguments in `args` using an X2Cpg command line parser, extended with the
    * frontend specific options in `frontendSpecific` with the initial configuration set to
    * `initialConf`. On success, the configuration is returned wrapped into an Option. On failure,
    * error messages are printed and, `None` is returned.
    */
  ...
  /** Create a command line parser that can be extended to add options specific for the frontend.
    */
  private def commandLineParser[R <: X2CpgConfig[R]](frontendSpecific: OParser[?, R])
    : OParser[?, R] =
    ...
    OParser.sequence(
      ...
      opt[Seq[String]]("exclude")
          ...
          .text(
            "files or folders to exclude during CPG generation (paths relative to <input-dir> or absolute paths)"
          ),
  ...
  /** Create an empty CPG, backed by the file at `optionalOutputPath` or in-memory if
    * `optionalOutputPath` is empty.
    */
  def newEmptyCpg(optionalOutputPath: Option[String] = None): Cpg =
    val odbConfig = optionalOutputPath
        .map { outputPath =>
          val outFile = File(outputPath)
          if outputPath != "" && outFile.exists then
            logger.debug("Output file exists, removing: " + outputPath)
            outFile.delete()
  ...
  def withNewEmptyCpg[T <: X2CpgConfig[?]](
    ...
    val outputPath = if outPath != "" then Some(outPath) else None
    Try {
  ...
  /** Given a function that receives a configuration and returns an arbitrary result type wrapped in
    * a `Try`, evaluate the function, printing errors to the console.
    */
  ...
  /** For a CPG generated by a frontend, run the default passes that turn a frontend-CPG into a
    * complete CPG.
    */
  ...
  /** Write `sourceCode` to a temporary file inside a temporary directory. The prefix for the
    * temporary directory is given by `tmpDirPrefix`. The suffix for the temporary file is given by
    * `suffix`. Both file and directory are deleted on exit.
    */
