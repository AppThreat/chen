...
trait X2CpgFrontend[T <: X2CpgConfig[_]]:
    ...
    /** Create CPG according to given configuration, printing errors to the console if they occur.
      * The CPG is closed and not returned.
      */
    ...
    /** Create a CPG for code at `inputName` (a single location) with default frontend
      * configuration. If `outputName` exists, it is the file name of the resulting CPG. Otherwise,
      * the CPG is held in memory.
      */
...
object X2Cpg:
    ...
    /** Parse commands line arguments in `args` using an X2Cpg command line parser, extended with
      * the frontend specific options in `frontendSpecific` with the initial configuration set to
    ...
    /** Write `sourceCode` to a temporary file inside a temporary directory. The prefix for the
      * temporary directory is given by `tmpDirPrefix`. The suffix for the temporary file is given
      * by `suffix`. Both file and directory are deleted on exit.
      */
