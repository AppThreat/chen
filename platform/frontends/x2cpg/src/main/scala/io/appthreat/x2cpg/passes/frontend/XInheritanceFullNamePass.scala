...
abstract class XInheritanceFullNamePass(cpg: Cpg) extends ForkJoinParallelCpgPass[TypeDecl](cpg):
    ...
    override def runOnPart(builder: DiffGraphBuilder, source: TypeDecl): Unit =
        ...
        if resolvedTypeDecls.nonEmpty then
            ...
            cpg.typ.fullNameExact(fullNames*).foreach(tgt =>
                builder.addEdge(source, tgt, EdgeTypes.INHERITS_FROM)
            )
            ...
            x.isCallForImportOut.importedEntity.map {
                case imp if relativePathPattern.matcher(imp).matches() =>
                    imp.split(pathSep).toList match
                        case head :: next =>
                            (Paths.get(head).normalize().toString +: next).mkString(
                              pathSep.toString
                            )
                        case Nil =>
    ...
    protected def resolveInheritedTypeFullName(
      td: TypeDecl,
      builder: DiffGraphBuilder
        ...
        if filteredTypes.isEmpty then
            // Usually in the case of inheriting external types
            qualifiedNamesInScope
                .flatMap { qn =>
                    td.inheritsFromTypeFullName.find(t =>
                        ImportStringHandling.namesIntersect(qn, t, pathSep)
                    ) match
                        case Some(path) => Option((qn, path))
                        case None       => None
                }
                .map { case (qualifiedName, inheritedNames) =>
                    xTypeFullName(qualifiedName, inheritedNames)
                }
                .map { case (name, fullName) => createTypeStub(name, fullName, builder) }
        else
            filteredTypes
    end resolveInheritedTypeFullName

    private def createTypeStub(
      name: String,
      fullName: String,
      builder: DiffGraphBuilder
    ): TypeDeclBase =
    ...
    /** Converts types in the form `foo.bar.Baz` to `foo/bar.js::program:Baz` and will result in a
      * tuple of name and full name.
      */
...
object ImportStringHandling:
    ...
    def combinedPath(importedType: String, importedPath: String, pathSep: Char = '.'): String =
        val (a, b) =
            if importedType.length > importedPath.length then
                (importedType, importedPath)
            else
                (importedPath, importedType)
        val (as, bs, intersect) = splitAndIntersect(a, b, pathSep)

        if a == importedPath then bs.diff(intersect).concat(as).mkString(pathSep.toString)
        else as.diff(intersect).concat(bs).mkString(pathSep.toString)
end ImportStringHandling
