package io.appthreat.x2cpg.passes.controlflow
...
/** A pass that creates control flow graphs from abstract syntax trees.
  ...
  *
  * Note: the version of OverflowDB that we currently use as a storage backend does not assign ids
  * to edges and this pass only creates edges at the moment. Therefore, we currently do without key
  * pools.
  */
