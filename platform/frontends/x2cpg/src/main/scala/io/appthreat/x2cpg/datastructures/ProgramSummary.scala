...
/** A hierarchical data-structure that stores the result of types and their respective members.
  * These types can be sourced from pre-parsing the application, or pre-computed stubs of common
  * libraries.
  *
...
trait ProgramSummary[T <: TypeLike[M, F], M <: MethodLike, F <: FieldLike]:
  ...
  def typesUnderNamespace(namespace: String): Set[T] =
      namespaceToType.getOrElse(namespace, Set.empty).toSet

  /** For a type, will search for the associated namespace.
    */
  def namespaceFor(clazz: T): Option[String] = namespaceToType.find { case (_, v) =>
      v.contains(clazz)
  }.map(_._1)
...
/** Extends the capability of the scope object to track types in scope as provide type resolution.
  ...
  *   the type/class meta data class.
  */
trait TypedScope[M <: MethodLike, F <: FieldLike, T <: TypeLike[M, F]](summary: ProgramSummary[
  T,
  M,
  F
]):
  ...
  /** Tracks the members visible to this scope. In languages like JavaScript or Python, where
    * members can be directly imported and accessed without an explicit base, they are kept here.
    */
  ...
  /** Given a type name or alias, attempts to resolve its full name using the types currently in
    * scope.
    *
    * @param typeName
    *   the shorthand name.
  ...
  protected def matchingM(callName: String)(implicit
    tag: ClassTag[M]
  ): PartialFunction[MemberLike, M] = {
  ...
  /** Given the type full name and field name, will attempt to find the matching entry.
    ...
    */
  def tryResolveFieldAccess(fieldName: String, typeFullName: Option[String] = None): Option[F] =
      typeFullName match
        case None => membersInScope.collectFirst {
                case f: FieldLike if f.name == fieldName => f.asInstanceOf[F]
            }
        case Some(tfn) =>
            tryResolveTypeReference(tfn).flatMap { t =>
  ...
  /** Given a method, will attempt to find the associated type with preference to the types in
    * scope.
    * @param m
...
trait OverloadableScope[M <: OverloadableMethod]:
  ...
  override def tryResolveMethodInvocation(
    ...
    case Some(tfn) =>
        val methodsWithEqualArgs = tryResolveTypeReference(tfn).flatMap { t =>
            ...
            Option(
              t.methods.filter(m =>
                  m.name == callName && m.parameterTypes.filterNot(_._1 == "this").size == argTypes.size
              )
            )
        }
  ...
  /** Determines if, by observing the given argument types, that the method's signature is a
    * plausible match to the observed arguments.
    *
    * The default implementation only considers that the same number of arguments are added and does
    * not account for variadic arguments nor polymorphism.
    *
    * @param method
    *   the method meta data.
    * @param argTypes
...
end OverloadableScope

/** An implementation of combining the typed scoping structures to manage the available type
  * information at namespace levels.
  *
  * @tparam M
  *   the method/function meta data class.
  * @tparam F
  ...
  *   the program summary.
  */
class DefaultTypedScope[M <: MethodLike, F <: FieldLike, T <: TypeLike[M, F]](
  summary: ProgramSummary[T, M, F]
...
trait TypeLike[M <: MethodLike, F <: FieldLike]:
  ...
  /** Adds the contents of the two types and produces a new type.
    ...
    *   the other type-like.
    * @return
    *   a type-like that is the combination of the two, with precedence to colliding contents to
    *   this type (LHS).
    */
  @targetName("add")
  def +(o: TypeLike[M, F]): TypeLike[M, F]

  /** Helper method for creating the sum of two type-like's methods, while preferring this types'
    * methods on collisions.
    * @param o
    *   the other type-like.
    * @return
  ...
  /** Helper method for creating the sum of two type-like's fields, while preferring this types'
    * fields on collisions.
    *
    * @param o
    *   the other type-like.
    * @return
