...
abstract class XTypeHintCallLinker(cpg: Cpg) extends CpgPass(cpg):
  ...
  /** Once we have connected methods that were speculatively generated and managed to correctly link
    * to methods already in the CPG, we link the rest to the "speculativeMethods" namespace as a way
    * to show that these may not actually exist.
    */
...
object XTypeHintCallLinker:
  ...
  /** The shared namespace for all methods generated from the type recovery that may not exist with
    * this exact full name in reality.
    */
