...
/** Attempts to set the <code>methodFullName</code> and link to callees using the recovered type
  * information from [[XTypeRecovery]]. Note that some methods may not be present as they could be
  * external and have been dynamically discovered, thus the
  * [[io.appthreat.x2cpg.passes.base.MethodStubCreator]] would have missed it.
  *
  * @param cpg
...
abstract class XTypeHintCallLinker(cpg: Cpg) extends CpgPass(cpg):
    ...
    protected def callees(names: Seq[String]): List[Method] =
        cpg.method.fullNameExact(names*).toList
        ...
        callerAndCallees.foreach { case (call, methodNames) =>
            ...
            if ms.nonEmpty then
                ...
                methodNames
                    ...
                    .map(fullName =>
                        newMethods.getOrElseUpdate(
                          fullName,
                          createMethodStub(fullName, call, builder)
                        )
                    )
    ...
    protected def setCallees(
      ...
      builder: DiffGraphBuilder
        ...
        else if methodNames.sizeIs > 1 && methodNames != nonDummyTypes then
    ...
    protected def createMethodStub(
      ...
      builder: DiffGraphBuilder
        ...
        val isExternal = if matcher.matches() then
            val fileName = matcher.group(1)
            cpg.file.nameExact(s"$basePath$fileName").isEmpty
        else
            true
        val name =
            if methodName.contains(pathSep) && methodName.length > methodName.lastIndexOf(
                  pathSep
                ) + 1
            then
    ...
    end createMethodStub

    /** Try to extract a type full name from the method full name, if one exists in the CPG then we
      * are lucky and we use it, else we ignore for now.
      */
    protected def createMethodStub(
      name: String,
      fullName: String,
      ...
      builder: DiffGraphBuilder
        ...
        val (astParentType, astParentFullName) =
            if !fullName.isBlank && !fullName.startsWith("<operator") && nameIdx > 0 then
                cpg.typeDecl
    ...
    /** Once we have connected methods that were speculatively generated and managed to correctly
      * link to methods already in the CPG, we link the rest to the "speculativeMethods" namespace
      * as a way to show that these may not actually exist.
      */
    protected def linkSpeculativeNamespaceNodes(
      newMethods: IterableOnce[NewMethod],
      builder: DiffGraphBuilder
    ): Unit =
        val speculativeNamespace =
            NewNamespaceBlock().name(XTypeHintCallLinker.namespace).fullName(
              XTypeHintCallLinker.namespace
            )
...
end XTypeHintCallLinker

object XTypeHintCallLinker:

    /** The shared namespace for all methods generated from the type recovery that may not exist
      * with this exact full name in reality.
      */
