...
/** A control flow graph that is under construction, consisting of:
  *
  * @param entryNode
  *   the control flow graph's first node, that is, the node to which a CFG that appends this CFG
  *   should attach itself to.
  * @param edges
  *   control flow edges between nodes of the code property graph.
  * @param fringe
  *   nodes of the CFG for which an outgoing edge type is already known but the destination node is
  *   not. These nodes are connected when another CFG is appended to this CFG.
  *
  * In addition to these three core building blocks, we store labels and jump statements that have
  * not been resolved and may be resolvable as parent sub trees or sibblings are translated.
  *
  * @param labeledNodes
  ...
  * @param breaks
  *   unresolved breaks collected along the way together with an integer value which indicates the
  *   number of loop/switch levels to break
  * @param continues
  *   unresolved continues collected along the way together with an integer value which indicates
  *   the number of loop/switch levels after which to continue
  * @param jumpsToLabel
...
case class Cfg(
  ...
  jumpsToLabel: List[(CfgNode, String)] = List()
    ...
    /** Create a new CFG in which `other` is appended to this CFG. All nodes of the fringe are
      * connected to `other`'s entry node and the new fringe is `other`'s fringe. The diffgraphs,
      * jumps, and labels are the sum of those present in `this` and `other`.
      */
    def ++(other: Cfg): Cfg =
        if other == Cfg.empty then
            this
        else if this == Cfg.empty then
            other
        else
            this.copy(
              fringe = other.fringe,
    ...
    /** Upon completing traversal of the abstract syntax tree, this method creates CFG edges between
      * jumps like gotos, labeled breaks, labeled continues and respective labels.
      */
        ...
        val edges = jumpsToLabel.flatMap {
            case (jumpToLabel, label) if label != "*" =>
                labeledNodes.get(label) match
                    ...
                    case None =>
                        logger.debug("Unable to wire jump statement. Missing label {}.", label)
                        None
            case (jumpToLabel, _) =>
...
object Cfg:
    ...
    /** Create edges from all nodes of cfg's fringe to `node`, ignoring fringe edge types and using
      * `cfgEdgeType` instead.
      */
    def edgesFromFringeTo(
      cfg: Cfg,
      node: Option[CfgNode],
      cfgEdgeType: CfgEdgeType
    ): List[CfgEdge] =
    ...
    /** Create edges from a list (node, cfgEdgeType) pairs to `node`
      */
    def edgesFromFringeTo(
      fringeElems: List[(CfgNode, CfgEdgeType)],
      node: Option[CfgNode]
    ): List[CfgEdge] =
