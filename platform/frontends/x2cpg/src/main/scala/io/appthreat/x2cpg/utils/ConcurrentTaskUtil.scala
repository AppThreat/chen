...
/** A utility for providing out-of-the-box basic concurrent execution for a collection of Scala
  * functions.
  */
object ConcurrentTaskUtil:
  ...
  /** Uses a thread pool with a limited number of active threads executing a task at any given
    * point. This is effective when tasks may require large amounts of memory, or single tasks are
    * too short lived.
    *
    * @param tasks
  ...
  /** Uses a Spliterator to run a number of tasks in parallel, where any number of threads may be
    * alive at any point. This is useful for running a large number of tasks with low memory
    * consumption. Spliterator's default thread pool is ForkJoinPool.commonPool().
    *
    * @param tasks
    *   the tasks to parallelize.
    * @tparam V
