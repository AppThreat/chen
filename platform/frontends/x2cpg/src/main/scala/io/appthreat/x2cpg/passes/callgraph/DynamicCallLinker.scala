...
/** We compute the set of possible call-targets for each dynamic call, and add them as CALL edges to
  * the graph, based on call.methodFullName, method.name and method.signature, the inheritance
  * hierarchy and the AST of typedecls and methods.
  *
  * This pass intentionally ignores the vtable mechanism based on BINDING nodes but does check for
  * an existing call edge before adding one. It assumes non-circular inheritance, on pain of endless
...
class DynamicCallLinker(cpg: Cpg) extends CpgPass(cpg):
  ...
  override def run(dstGraph: DiffGraphBuilder): Unit =
    ...
    if !cpg.call.exists(_.dispatchType == DispatchTypes.DYNAMIC_DISPATCH) then
      return
    initMaps()
  ...
  /** Recursively returns all the sub-types of the given type declaration. Does account for circular
    * hierarchies.
    */
  ...
  /** Recursively returns all the super-types of the given type declaration. Does account for
  ...
  private def inheritTraversal(
    ...
    if visitedNodes.contains(cur) then return visitedNodes
    visitedNodes.addOne(cur)

    (if inSuperDirection then
       cpg.typeDecl.fullNameExact(cur.fullName).flatMap(_.inheritsFromOut.referencedTypeDecl)
  ...
  private def resolveCallInSuperClasses(call: Call): Boolean =
    ...
    val candidateInheritedMethods =
        cpg.typeDecl
            ...
            .fullName
            .l
    if candidateInheritedMethods.nonEmpty then
      validM.put(
        call.methodFullName,
        validM.getOrElse(
          call.methodFullName,
  ...
  private def linkDynamicCall(call: Call, dstGraph: DiffGraphBuilder): Unit =
    ...
    if call.methodFullName.equals("<empty>") || call.methodFullName.equals(
    ...
    validM.get(methodNameToUse) match
      case Some(tgts) =>
          ...
          (if externalMs.nonEmpty && internalMs.nonEmpty then internalMs else tgtMs)
              .foreach { tgtM =>
                  if !callsOut.contains(tgtM.fullName) then
  ...
  /** In the case where the method isn't an internal method and cannot be resolved by crawling
    * TYPE_DECL nodes it can be resolved from the map of external methods.
    */
