...
class CfgCreator(entryNode: Method, diffGraph: DiffGraphBuilder):
  ...
  /** Control flow graph definitions often feature a designated entry and exit node for each method.
    * While these nodes are no-ops from a computational point of view, they are useful to guarantee
    * that a method has exactly one entry and one exit.
    *
  ...
  private def isLogicalOperator(node: AstNode): Boolean = node match
    case call: Call =>
        call.name == Operators.conditional || call.name == Operators.logicalOr || call
  ...
  /** Only include block nodes that do not describe the entire method body or the bodies of control
    * structures or inlined calls or logical operators.
    */
  ...
  /** The CFG for a break/continue statements contains only the break/continue statement as a single
    * entry node. The fringe is empty, that is, appending another CFG to the break statement will
    * not result in the creation of an edge from the break statement to the entry point of the other
    * CFG. Labeled breaks are treated like gotos and are added to "jumpsToLabel".
    */
  ...
  /** Jump targets ("labels") are included in the CFG. As these should be connected to the next
    ...
    * fringe. This is achieved by calling `cfgForSingleNode` on the label node. Just like for breaks
    * and continues, we record labels. We store case/default labels separately from other labels,
  ...
  /** The right hand side of a logical AND expression is only evaluated if the left hand side is
    ...
    * in the corresponding control flow graph by creating control flow graphs for the left and right
    * hand expressions and appending the two, where the fringe edge type of the left CFG is
    * `TrueEdge`.
    */
  ...
  /** Same construction recipe as for the AND expression, just that the fringe edge type of the left
    * CFG is `FalseEdge`.
    */
  ...
  /** A conditional expression is of the form `condition ? trueExpr ; falseExpr` where both
    * `trueExpr` and `falseExpr` are optional. We create the corresponding CFGs by creating CFGs for
    * the three expressions and adding edges between them. The new entry node is the condition entry
    * node.
    */
  ...
  /** For macros, the AST contains a CALL node, along with child sub trees for all arguments, and a
    * final sub tree that contains the inlined code. The corresponding CFG consists of the CFG for
    * the call, an edge to the exit and an edge to the CFG of the inlined code. We choose this
  ...
  /** A for statement is of the form `for(initExpr; condition; loopExpr) body` and all four
    * components may be empty. The sequence (condition - body - loopExpr) form the inner part of the
    * loop and we calculate the corresponding CFG `innerCfg` so that it is no longer relevant which
    * of these three actually exist and we still have an entry node for the loop and a fringe.
    */
  ...
  /** CFG creation for while statements of the form `while(condition) body1 else body2` where body1
    * and the else block are optional.
    */
  ...
  /** CFG creation for if statements of the form `if(condition) body`, optionally followed by `else
    * body2`.
    */
