package io.appthreat.x2cpg.passes.controlflow.cfgcreation
...
/** Translation of abstract syntax trees into control flow graphs
  *
  * The problem of translating an abstract syntax tree into a corresponding control flow graph can
  * be formulated as a recursive problem in which sub trees of the syntax tree are translated and
  * their corresponding control flow graphs are connected according to the control flow semantics of
  * the root node. For example, consider the abstract syntax tree for an if-statement:
  * {{{
  *               (  if )
  *              /       \
  *          (x < 10)  (x += 1)
  ...
  *           x  10     x   1
  * }}}
  * This tree can be translated into a control flow graph, by translating the sub tree rooted in `x
  * < 10` and that of `x+= 1` and connecting their control flow graphs according to the semantics of
  * `if`:
  * {{{
  *            [x < 10]----
  *               |t     f|
  ...
  * }}}
  *
  * The semantics of if dictate that the first sub tree to the left is a condition, which is
  * connected to the CFG of the second sub tree - the body of the if statement - via a control flow
  * edge with the `true` label (indicated in the illustration by `t`), and to the CFG of any
  * follow-up code via a `false` edge (indicated by `f`).
  *
  * A problem that becomes immediately apparent in the illustration is that the result of
  * translating a sub tree may leave us with edges for which a source node is known but the
  * destination node depends on parents or siblings that were not considered in the translation. For
  * example, we know that an outgoing edge from [x<10] must exist, but we do not yet know where it
  * should lead. We refer to the set of nodes of the control flow graph with outgoing edges for
...
class CfgCreator(entryNode: Method, diffGraph: DiffGraphBuilder):
    ...
    import io.appthreat.x2cpg.passes.controlflow.cfgcreation.Cfg.*
    import io.appthreat.x2cpg.passes.controlflow.cfgcreation.CfgCreator.*

    /** Control flow graph definitions often feature a designated entry and exit node for each
      * method. While these nodes are no-ops from a computational point of view, they are useful to
      * guarantee that a method has exactly one entry and one exit.
      *
      * For the CPG-based control flow graph, we do not need to introduce fake entry and exit node.
      * Instead, we can use the METHOD and METHOD_RETURN nodes as entry and exit nodes respectively.
      * Note that METHOD_RETURN nodes are the nodes representing formal return parameters, of which
      * there exists exactly one per method.
      */
    private val exitNode: MethodReturn = entryNode.methodReturn

    /** We return the CFG as a sequence of Diff Graphs that is calculated by first obtaining the CFG
      * for the method and then resolving gotos.
      */
    def run(): Unit =
    ...
    /** Conversion of a method to a CFG, showing the decomposition of the control flow graph
      * generation problem into that of translating sub trees according to the node type. In the
      * particular case of a method, the CFG is obtained by creating a CFG containing the single
      * method node and a fringe containing the node and an outgoing AlwaysEdge, to the CFG obtained
      * by translating child CFGs one by one and appending them.
      */
    private def cfgForMethod(node: Method): Cfg =
        cfgForSingleNode(node) ++ cfgForChildren(node)

    /** For any single AST node, we can construct a CFG containing that single node by setting it as
    ...
    /** The CFG for all children is obtained by translating child ASTs one by one from left to right
      * and appending them.
      */
    ...
    /** Only include block nodes that do not describe the entire method body or the bodies of
      * control structures or inlined calls or logical operators.
      */
    private def blockMatches(block: Block): Boolean =
        if block._astIn.hasNext then
            val parentNode = block.astParent
            parentNode.isMethod || parentNode.isControlStructure || isLogicalOperator(
              parentNode
            ) || isInlinedCall(parentNode)
        else false

    /** A second layer of dispatching for control structures. This could as well be part of `cfgFor`
      * and has only been placed into a separate function to increase readability.
      */
    ...
    /** The CFG for a break/continue statements contains only the break/continue statement as a
      * single entry node. The fringe is empty, that is, appending another CFG to the break
      * statement will not result in the creation of an edge from the break statement to the entry
      * point of the other CFG. Labeled breaks are treated like gotos and are added to
      * "jumpsToLabel".
      */
    ...
    /** Jump targets ("labels") are included in the CFG. As these should be connected to the next
      * appended CFG, we specify that the label node is both the entry node and the only node in the
      * fringe. This is achieved by calling `cfgForSingleNode` on the label node. Just like for
      * breaks and continues, we record labels. We store case/default labels separately from other
      * labels, but that is not a relevant implementation detail.
      */
    protected def cfgForJumpTarget(n: JumpTarget): Cfg =
        val labelName = n.name
        val cfg       = cfgForSingleNode(n)
        if labelName.startsWith("case") || labelName.startsWith("default") then
            cfg.copy(caseLabels = List(n))
        else
            cfg.copy(labeledNodes = Map(labelName -> n))

    /** A CFG for a goto statement is one containing the goto node as an entry node and an empty
      * fringe. Moreover, we store the goto for dispatching with `withResolvedJumpToLabel` once the
      * CFG for the entire method has been calculated.
      */
    protected def cfgForGotoStatement(node: ControlStructure): Cfg =
        node.astChildren.find(_.order == 1) match
            ...
            case None =>
                ...
                target.map(t =>
                    Cfg(entryNode = Some(node), jumpsToLabel = List((node, t)))
                ).getOrElse(Cfg.empty)

    /** Return statements may contain expressions as return values, and therefore, the CFG for a
      * return statement consists of the CFG for calculation of that expression, appended to a CFG
      * containing only the return node, connected with a single edge to the method exit node. The
      * fringe is empty.
      *
      * @param inheritFringe
      *   indicates if the resulting Cfg object must contain the fringe value of the return value's
      *   children.
      */
    protected def cfgForReturn(actualRet: Return, inheritFringe: Boolean = false): Cfg =
        ...
        childrenCfg ++
            Cfg(
              ...
              if inheritFringe then childrenCfg.fringe else List()
            )

    /** The right hand side of a logical AND expression is only evaluated if the left hand side is
      * true as the entire expression can only be true if both expressions are true. This is encoded
      * in the corresponding control flow graph by creating control flow graphs for the left and
      * right hand expressions and appending the two, where the fringe edge type of the left CFG is
      * `TrueEdge`.
      */
    ...
    /** A conditional expression is of the form `condition ? trueExpr ; falseExpr` where both
      * `trueExpr` and `falseExpr` are optional. We create the corresponding CFGs by creating CFGs
      * for the three expressions and adding edges between them. The new entry node is the condition
      * entry node.
      */
    protected def cfgForConditionalExpression(call: Call): Cfg =
        ...
        val trueFridge = if trueCfg.entryNode.isDefined then
            trueCfg.fringe
        else
            conditionCfg.fringe.withEdgeType(TrueEdge)
        val falseFridge = if falseCfg.entryNode.isDefined then
            falseCfg.fringe
        else
    ...
    end cfgForConditionalExpression

    /** For macros, the AST contains a CALL node, along with child sub trees for all arguments, and
      * a final sub tree that contains the inlined code. The corresponding CFG consists of the CFG
      * for the call, an edge to the exit and an edge to the CFG of the inlined code. We choose this
      * representation because it allows both queries that use the macro reference as well as
      * queries that reference the inline code to be chosen as sources/sinks in data flow queries.
      */
    ...
    /** A for statement is of the form `for(initExpr; condition; loopExpr) body` and all four
      ...
      * the loop and we calculate the corresponding CFG `innerCfg` so that it is no longer relevant
      * which of these three actually exist and we still have an entry node for the loop and a
      * fringe.
      */
    protected def cfgForForStatement(node: ControlStructure): Cfg =
        ...
        val newEdges = edgesFromFringeTo(initExprCfg, innerCfg.entryNode) ++
            ...
            edgesFromFringeTo(conditionCfg, bodyCfg.entryNode, TrueEdge) ++ {
                if loopExprCfg.entryNode.isDefined then
        ...
        Cfg
            ...
            .copy(
              ...
              fringe = conditionCfg.fringe.withEdgeType(FalseEdge) ++ takeCurrentLevel(
                bodyCfg.breaks
              ).map((_, AlwaysEdge)),
              breaks = reduceAndFilterLevel(bodyCfg.breaks),
              continues = reduceAndFilterLevel(bodyCfg.continues)
            )
    end cfgForForStatement

    /** A Do-Statement is of the form `do body while(condition)` where body may be empty. We again
      * first calculate the inner CFG as bodyCfg ++ conditionCfg and then connect edges according to
    ...
    protected def cfgForDoStatement(node: ControlStructure): Cfg =
        ...
        Cfg
            ...
            .copy(
              entryNode = if bodyCfg != Cfg.empty then bodyCfg.entryNode
    ...
    end cfgForDoStatement

    /** CFG creation for while statements of the form `while(condition) body1 else body2` where
      * body1 and the else block are optional.
      */
    protected def cfgForWhileStatement(node: ControlStructure): Cfg =
        ...
        Cfg
            ...
            .copy(
              ...
              fringe =
                  conditionCfg.fringe.withEdgeType(FalseEdge) ++ takeCurrentLevel(trueCfg.breaks)
    ...
    /** CFG creation for if statements of the form `if(condition) body`, optionally followed by
      * `else body2`.
      */
        ...
        Cfg
            ...
            .copy(
              ...
              fringe = trueCfg.fringe ++ {
                  if falseCfg.entryNode.isDefined then
                      falseCfg.fringe
                  else
                      conditionCfg.fringe.withEdgeType(FalseEdge)
              }
            )
    end cfgForIfStatement

    /** CFG creation for try statements of the form `try { tryBody ] catch { catchBody } `,
      * optionally followed by `finally { finallyBody }`.
      *
      * To avoid very large CFGs for try statements, only edges from the last statement in the `try`
      * block to each `catch` block (and optionally the `finally` block) are created. The last
      * statement in each `catch` block should then have an outgoing edge to the `finally` block if
      * it exists (and not to any subsequent catch blocks), or otherwise * be part of the fringe.
      *
      * By default, the first child of the `TRY` node is treated as the try body, while every
      * subsequent node is treated as a `catch`, with no `finally` present. To treat the last child
      * of the node as the `finally` block, the `code` field of the `Block` node must be set to
      * `finally`.
      */
        ...
        if maybeTryBlock.isEmpty then
            ...
            Cfg
                ...
                .copy(
                  ...
                  edges =
                      diffGraphs ++ tryBodyCfg.edges ++ catchBodyCfgs.flatMap(
                        _.edges
                      ) ++ maybeFinallyBodyCfg.flatMap(_.edges),
                  fringe = if maybeFinallyBodyCfg.flatMap(_.entryNode).nonEmpty then
    ...
    /** The CFGs for match cases are modeled after PHP match expressions and assumes that a case
      * will always consist of one or more JumpTargets followed by a single expression. The CFG also
      * assumes an implicit `break` at the end of each match case.
      */
    ...
    protected def cfgForSwitchLike(conditionCfg: Cfg, bodyCfgs: List[Cfg]): Cfg =
        val hasDefaultCase =
            bodyCfgs.flatMap(_.caseLabels).exists(x => x.asInstanceOf[JumpTarget].name == "default")
        val caseEdges =
            edgesToMultiple(conditionCfg.fringe.map(_._1), bodyCfgs.flatMap(_.caseLabels), CaseEdge)
        ...
        Cfg
            ...
            .copy(
              ...
              fringe = {
                  if !hasDefaultCase then conditionCfg.fringe.withEdgeType(FalseEdge)
                  else Nil
