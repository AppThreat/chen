...
import io.appthreat.x2cpg.passes.controlflow.cfgdominator.{
    CfgAdapter,
    CfgDominator,
...
class CfgDominatorFrontierTests extends AnyWordSpec with Matchers:
  ...
  private class TestDomTreeAdapter(immediateDominators: scala.collection.Map[Node, Node])
  ...
  "Cfg domiance frontier with dead code test" in {
      ...
      val v0 = graph + "UNKNOWN"
      val v1 =
          graph + "UNKNOWN" // This node simulates dead code as it is not reachable from the entry v0.
      val v2 = graph + "UNKNOWN"
