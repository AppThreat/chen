  ...
  "AndroidServicesTagsPass" should {
      ...
      "tag a call into an AI/LLM SDK and its arguments" in
          Fixture(
            Languages.JAVASRC,
            callWithArg =
                Some(("com.openai.client.OpenAiService.createChatCompletion", "\"prompt text\""))
          ) { cpg =>
      ...
      "not tag a non-egress http client call" in
          Fixture(
            Languages.JAVASRC,
            callWithArg = Some(("okhttp3.OkHttpClient.connectTimeout", "\"30\""))
          ) { cpg =>
      ...
      "tag a newly-added ML/AI cloud SDK call" in
          Fixture(
            Languages.JAVASRC,
            callWithArg = Some(("com.google.mlkit.vision.text.TextRecognizer.process", "image"))
          ) { cpg =>
              cpg.call.methodFullName("com.google.mlkit.*").tag.name.toSet should contain(
                "service-ai-llm"
              )
          }
      ...
      "tag a local/on-device AI runtime call with the on-device-ai umbrella" in
          Fixture(
            Languages.JAVASRC,
            callWithArg = Some(("de.kherud.llama.LlamaModel.complete", "\"prompt\""))
          ) { cpg =>
...
end AndroidServicesTagsPassTests
