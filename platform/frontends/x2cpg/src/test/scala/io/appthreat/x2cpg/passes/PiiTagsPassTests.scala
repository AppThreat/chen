...
class PiiTagsPassTests extends AnyWordSpec with Matchers:
  ...
  "PiiTagsPass" should {
      ...
      "tag a Luhn-valid credit card literal as PCI data" in Fixture(literals =
          Seq("\"4111111111111111\"")
      ) { cpg =>
      ...
      "not tag a digit string that fails the Luhn check" in Fixture(literals =
          Seq("\"4111111111111112\"")
      ) { cpg =>
