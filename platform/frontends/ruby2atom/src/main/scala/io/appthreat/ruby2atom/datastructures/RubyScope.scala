...
import io.appthreat.x2cpg.datastructures.{TypedScopeElement, *}
...
class RubyScope(summary: RubyProgramSummary, projectRoot: Option[String])
  ...
  def newProgramScope: Option[ProgramScope] =
      surroundingScopeFullName.map(_.stripSuffix(NamespaceTraversal.globalNamespaceName)).map(
        ProgramScope.apply
      )
  ...
  /** Variables entering children scope persist into parent scopes, so the variables should be
    * transferred to the top-level method, returning the next block so that locals can be attached.
    */
  ...
  /** Get the name of the implicit or explict proc param and mark the method scope as using the proc
    * param
    */
  def useProcParam: Option[String] = updateSurrounding {
      ...
      case ScopeElement(ConstructorScope(fullName, param, _), variables) =>
          (
            ScopeElement(ConstructorScope(fullName, param, true), variables),
            param.fold(x => x, x => x)
          )
  }
  ...
  def setProcParam(param: String, paramNode: NewMethodParameterIn): Unit = updateSurrounding {
      case ScopeElement(MethodScope(fullName, _, _), variables) =>
          (
            ScopeElement(
              MethodScope(fullName, Right(param), true),
              variables ++ Map(paramNode.name -> paramNode)
            ),
            ()
          )
      case ScopeElement(ConstructorScope(fullName, _, _), variables) =>
          (
            ScopeElement(
              ConstructorScope(fullName, Right(param), true),
              variables ++ Map(paramNode.name -> paramNode)
            ),
            ()
          )
  ...
  def surroundingTypeFullName: Option[String] =
      stack.collectFirst { case ScopeElement(x: TypeLikeScope, _) =>
          x.fullName
      }

  /** Searches the surrounding classes for a class that matches the given value. Returns it if
    * found.
    */
  ...
  def shouldGenerateDefaultConstructor: Boolean = stack
      .collectFirst {
          ...
          case ScopeElement(x: TypeLikeScope, _) =>
              !typesInScope.find(_.name == x.fullName).exists(_.hasConstructor)
          case _ => false
      }
      .getOrElse(false)

  /** When a singleton class is introduced into the scope, the base variable will now have the
    * singleton's functionality mixed in. This method finds base variable and appends the singleton
    * type.
    *
  ...
  override def tryResolveTypeReference(typeName: String): Option[RubyType] =
    ...
    def tryResolveInternalTypeReference(typeName: String): Option[RubyType] =
        typesInScope.collectFirst {
            case typ
                if !typ.isInstanceOf[RubyStubbedType] && typ.name.split("[.]").endsWith(
                  typeName.split("[.]")
                ) => typ
        }
        ...
        .orElse {
            super.tryResolveTypeReference(normalizedTypeName) match
              case None if GlobalTypes.kernelFunctions.contains(normalizedTypeName) =>
                  Option(RubyType(
                    s"${GlobalTypes.kernelPrefix}.$normalizedTypeName",
                    List.empty,
                    List.empty
                  ))
              case None if GlobalTypes.bundledClasses.contains(normalizedTypeName) =>
                  Option(RubyType(
                    s"${GlobalTypes.builtinPrefix}.$normalizedTypeName",
                    List.empty,
                    List.empty
                  ))
              case None =>
