...
import io.appthreat.ruby2atom.astcreation.RubyIntermediateAst.{Unknown, Block as RubyBlock, *}
import io.appthreat.ruby2atom.datastructures.BlockScope
import io.appthreat.ruby2atom.parser.RubyJsonHelpers
import io.appthreat.ruby2atom.passes.Defines
import io.appthreat.ruby2atom.passes.GlobalTypes
import io.appthreat.ruby2atom.passes.Defines.{RubyOperators, getBuiltInType}
import io.appthreat.x2cpg.{Ast, ValidationMode, Defines as XDefines}
...
trait AstForExpressionsCreator(implicit withSchemaValidation: ValidationMode):
  ...
  protected def astForNilLiteral: Ast =
      Ast(NewLiteral().code("nil").typeFullName(getBuiltInType(Defines.NilClass)))
      ...
      baseNode match
        ...
        case _ =>
            scope.lookupVariable(baseNode.text) match
              ...
              case Some(decl: NewLocal) if decl.typeFullName != Defines.Any =>
                  Option(decl.typeFullName)
              case Some(decl: NewMethodParameterIn) if decl.typeFullName != Defines.Any =>
                  Option(decl.typeFullName)
  ...
  protected def astForMemberCall(node: MemberCall, isStatic: Boolean = false): Ast =
    ...
    def createMemberCall(n: MemberCall): Ast =
      val receiverAst = astForFieldAccess(
        MemberAccess(n.target, ".", n.methodName)(n.span),
        stripLeadingAt = true
      )
              ...
              case _ =>
                  (n.target match
                    case ma: MemberAccess =>
                        scope.tryResolveTypeReference(ma.memberName).map(_.name)
      ...
      val dispatchType =
          if isStatic then DispatchTypes.STATIC_DISPATCH else DispatchTypes.DYNAMIC_DISPATCH

      val callCode = if baseCode.contains("<tmp-") then
        val rhsCode =
            if n.methodName == "new" then n.methodName
      ...
      if methodFullName != XDefines.DynamicCallUnknownFullName then
    ...
    def determineMemberAccessBase(target: RubyExpression): RubyExpression = target match
      ...
      case x: SimpleIdentifier =>
          scope.getSurroundingType(x.text).map(_.fullName) match
            ...
            case None => MemberCall(
                  SelfIdentifier()(x.span.spanStart(Defines.Self)),
                  ".",
                  x.text,
                  Nil
                )(x.span)
    ...
    node.target match
      ...
      case x: SimpleIdentifier if isBundledClass(x.text) =>
          createMemberCall(
            node.copy(target = TypeIdentifier(prefixAsBundledType(x.text))(x.span))(node.span)
          )
      case x: SimpleIdentifier =>
  ...
  private def astForMemberAccessTarget(target: RubyExpression): (Ast, String) =
      target match
        ...
        case target: MemberAccess =>
            handleTmpGen(target, astForFieldAccess(target, stripLeadingAt = true))
  ...
  private def handleTmpGen(target: RubyExpression, rhs: Ast): (Ast, String) =
    ...
    val tmpIdenAst =
        scope.lookupVariable(tmpName).map(x => Ast(tmpIden).withRefEdge(tmpIden, x)).getOrElse(Ast(
          tmpIden
        ))
    val code = s"$tmpName = ${target.text}"
    if createAssignmentToTmp then
          ...
          scope
              ...
              .map { m =>
                ...
                expr.root.collect { case x: NewCall =>
                    ...
                    scope.tryResolveTypeReference(m.returnType).map(_.name).foreach(
                      x.typeFullName(_)
                    )
                }
                expr
   ...
   * ```
   */
  protected def astForCallWithBlock[C <: RubyCall](node: RubyExpression & RubyCallWithBlock[C])
    : Ast =
    ...
    val callWithLambdaArg = node.withoutBlock match
      case x: SimpleCall =>
          astForSimpleCall(x.copy(arguments = x.arguments ++ typeRefDummyNode)(x.span))
      case x: MemberCall =>
          astForMemberCall(x.copy(arguments = x.arguments ++ typeRefDummyNode)(x.span))
      case x =>
          Ast()
  ...
  protected def astForObjectInstantiation(node: RubyExpression & ObjectInstantiation): Ast =
      ...
      case x: (SimpleIdentifier | MemberAccess) =>
          scope.tryResolveTypeReference(x.text) match
            case Some(typeMetaData) =>
                s"${typeMetaData.name}<class>" -> s"${typeMetaData.name}.${Defines.Initialize}"
    ...
    // Assign tmp to <alloc>
    val allocCall =
        callNode(node, code(node), Operators.alloc, Operators.alloc, DispatchTypes.STATIC_DISPATCH)
    ...
    if fullName != XDefines.DynamicCallUnknownFullName then
      constructorCall.dynamicTypeHintFullName(Seq(fullName))
    val constructorRecv =
        astForExpression(MemberAccess(node.target, ".", Defines.Initialize)(node.span))
    val constructorCallAst =
        callAst(constructorCall, argumentAsts, Option(tmpIdentifier), Option(constructorRecv))
    val retIdentifierAst = tmpIdentifier
    scope.popScope()

    // Assemble statements
    blockAst(block, Ast(tmpLocal) :: tmpAssignment :: constructorCallAst :: retIdentifierAst :: Nil)
  end astForObjectInstantiation
              ...
              case Some(op) =>
                  node.rhs match
                            ...
                            ElseClause(
                              StatementList(
                                SingleAssignment(
                                  node.lhs,
                                  node.op,
                                  StaticLiteral(getBuiltInType(Defines.NilClass))(
                                    span.spanStart("nil")
                                  )
                    ...
                    case _ =>
                        ...
                        val lhsAst = node.lhs match
                          case x: SimpleIdentifier if scope.lookupVariable(code(x)).isEmpty =>
                              ...
                              scope.addToScope(name, local) match
                                case BlockScope(block) =>
                                    diffGraph.addEdge(block, local, EdgeTypes.AST)
                                case _ =>
                              astForExpression(node.lhs)
                          case SplattingRubyNode(nameNode: SimpleIdentifier)
                              if scope.lookupVariable(code(nameNode)).isEmpty =>
                          ...
                          case x: GroupedParameter =>
                              ...
                              val call =
                                  callNode(node, code(node), op, op, DispatchTypes.STATIC_DISPATCH)
                        ...
                        if node.rhs.isInstanceOf[ObjectInstantiation] && lhsAst.root.exists(
                            _.isInstanceOf[NewIdentifier]
                          )
                        then
                          rhsAst.nodes.collectFirst {
                              case tmp: NewIdentifier
                                  if tmp.name.startsWith("<tmp") && tmp.typeFullName != Defines.Any =>
                                  lhsAst.root.collectFirst { case i: NewIdentifier =>
                                      scope.lookupVariable(i.name).foreach {
                                          case x: NewLocal =>
                                              x.dynamicTypeHintFullName(
                                                x.dynamicTypeHintFullName :+ tmp.typeFullName
                                              )
                                      ...
                                      i.dynamicTypeHintFullName(
                                        i.dynamicTypeHintFullName :+ tmp.typeFullName
                                      )
                                  }
                          }
                        end if
  ...
  private def reassign(
    ...
    def stmtListAssigningLastExpression(stmts: List[RubyExpression]): List[RubyExpression] =
        stmts match
          ...
          case head :: Nil =>
              SingleAssignment(lhs, op, head)(
                rhs.span.spanStart(s"${lhs.span.text} $op ${head.span.text}")
              ) :: Nil
          case Nil          => List.empty
          case head :: tail => head :: stmtListAssigningLastExpression(tail)

    def clauseAssigningLastExpression(x: RubyExpression & ControlFlowClause): RubyExpression =
        x match
          ...
          case EnsureClause(thenClause) =>
              EnsureClause(reassign(lhs, op, thenClause, transform))(x.span)
          case ElsIfClause(condition, thenClause) =>
              ElsIfClause(condition, reassign(lhs, op, thenClause, transform))(x.span)
          case ElseClause(thenClause) =>
              ElseClause(reassign(lhs, op, thenClause, transform))(x.span)
    ...
    rhs match
      case StatementList(statements) =>
          StatementList(stmtListAssigningLastExpression(statements))(rhs.span)
      case clause: ControlFlowClause  => clauseAssigningLastExpression(clause)
      case expr: ControlFlowStatement => transform(expr)
      case _ =>
          SingleAssignment(lhs, op, rhs)(
            rhs.span.spanStart(s"${lhs.span.text} $op ${rhs.span.text}")
          )
  end reassign
  ...
  protected def astForSimpleIdentifier(node: RubyExpression & RubyIdentifier): Ast =
    ...
    if isBundledClass(name) then
      ...
      scope.lookupVariable(name) match
        ...
        case None =>
            astForMemberAccess(
              MemberAccess(SelfIdentifier()(node.span.spanStart(Defines.Self)), ".", node.text)(
                node.span
              )
            )
  ...
  protected def astForSimpleCall(node: SimpleCall): Ast =
      node.target match
        ...
        case targetNode: RubyFieldIdentifier =>
            astForMemberCallWithoutBlock(node, targetNode.toMemberAccess)
  ...
  protected def astForRequireCall(node: RequireCall): Ast =
    ...
    pathOpt.foreach(path =>
        scope.addRequire(projectRoot.get, fileName, path, node.isRelative, node.isWildCard)
    )
  ...
  /** A yield in Ruby calls an explicit (or implicit) proc parameter and returns its value. This can
    * be lowered as block.call(), which is effectively how one invokes a proc parameter in any case.
    */
  protected def astForYield(node: YieldExpr): Ast =
      scope.useProcParam match
        case Some(param) =>
            ...
            if scope.lookupVariable(param).isEmpty then
            ...
            val loweredCall =
                MemberCall(
                  SimpleIdentifier()(node.span.spanStart(param)),
                  ".",
                  "call",
                  node.arguments
                )(node.span)
  ...
  protected def astForRange(node: RangeExpression): Ast =
    ...
    val call =
        callNode(node, code(node), Operators.range, Operators.range, DispatchTypes.STATIC_DISPATCH)
    callAst(call, Seq(lbAst, ubAst))

  protected def astForArrayLiteral(node: ArrayLiteral): Ast =
    val arguments = if node.text.startsWith("%") then
      val argumentsType =
          if node.isStringArray then getBuiltInType(Defines.String)
          else getBuiltInType(Defines.Symbol)
      node.elements.map {
          case element @ StaticLiteral(_) => StaticLiteral(argumentsType)(element.span)
          case element @ DynamicLiteral(_, expressions) =>
              DynamicLiteral(argumentsType, expressions)(element.span)
  ...
  protected def astForHashLiteral(node: HashLike): Ast =
    ...
    val argumentAsts = node.elements.flatMap(elem =>
        elem match
          ...
          case splattingRubyNode: SplattingRubyNode =>
              astForSplattingRubyNode(splattingRubyNode) :: Nil
          case node =>
  ...
  protected def astForAssociationHash(node: Association, tmp: String): List[Ast] =
      node.key match
        ...
        case iden: SimpleIdentifier =>
            ...
            val sym =
                StaticLiteral(getBuiltInType(Defines.Symbol))(iden.span.spanStart(s":${iden.text}"))
      ...
      (node.lowerBound, node.upperBound) match
        case (lb: StaticLiteral, ub: StaticLiteral) =>
            (lb.typeFullName, ub.typeFullName) match
              case (
                    s"${GlobalTypes.`kernelPrefix`}.Integer",
                    s"${GlobalTypes.`kernelPrefix`}.Integer"
                  ) =>
                  generateRange(
                    lb.span.text.toInt,
                    ub.span.text.toInt,
                    node.rangeOperator.exclusive
                  )
                      .map(x =>
                          StaticLiteral(lb.typeFullName)(TextSpan(
                            lb.line,
                            lb.column,
                            lb.lineEnd,
                            lb.columnEnd,
                      ...
                      .toList
              case (
                    s"${GlobalTypes.`kernelPrefix`}.String",
                    s"${GlobalTypes.`kernelPrefix`}.String"
                  ) =>
                  ...
                  if lbVal.length > 1 || ubVal.length > 1 then
  ...
  protected def astForSingleKeyValue(
    keyNode: RubyExpression,
    valueNode: RubyExpression,
    tmp: String
  ): Ast =
      astForExpression(
        SingleAssignment(
          IndexAccess(
            SimpleIdentifier()(TextSpan(
              keyNode.line,
              keyNode.column,
              keyNode.lineEnd,
              keyNode.columnEnd,
  ...
  protected def astForRescueExpression(node: RescueExpression): Ast =
    ...
    val rescueAsts = node.rescueClauses
        .map { x =>
          val classes =
              x.exceptionClassList.map(e =>
                  scope.tryResolveTypeReference(e.text).map(_.name).getOrElse(e.text)
              ).toSeq
          val variables = x.variables
              ...
              .toList
          val rescueNode = controlStructureNode(x.thenClause.asStatementList, "CATCH", "catch")
          Ast(rescueNode).withChild(
            astForStatementList(x.thenClause.asStatementList).withChildren(variables)
          )
        }
    val elseAst = node.elseClause.map { x =>
      val astForClause =
          controlStructureNode(x.thenClause.asStatementList, ControlStructureTypes.ELSE, "else")
    ...
    val ensureAst = node.ensureClause.map { x =>
      val astForEnsureClause =
          controlStructureNode(x.thenClause.asStatementList, "FINALLY", "finally")
  ...
  private def astForSelfIdentifier(node: SelfIdentifier): Ast =
    val thisIdentifier =
        identifierNode(
          ...
          Defines.Self,
          code(node),
          scope.surroundingTypeFullName.getOrElse(Defines.Any)
        )
  ...
  private def astForMemberCallWithoutBlock(node: SimpleCall, memberAccess: MemberAccess): Ast =
    ...
    val call =
        callNode(node, code(node), methodName, methodFullName, DispatchTypes.DYNAMIC_DISPATCH)

    callAst(call, argumentAsts, Some(receiverAst))

  private def astForMethodCallWithoutBlock(
    node: SimpleCall,
    methodIdentifier: SimpleIdentifier
  ): Ast =
    ...
    val (receiverType, methodFullNameHint) =
        ...
        match
          case Some(m) =>
              scope.typeForMethod(m).map(t => t.name -> s"${t.name}.${m.name}").getOrElse(
                defaultResult
              )
    ...
    val (dispatchType, methodFullName) =
        if receiverType.startsWith(GlobalTypes.builtinPrefix) then
  ...
  end astForMethodCallWithoutBlock
        ...
        case selfMethod: SingletonMethodDeclaration =>
            ...
            scope.surroundingScopeFullName
                .map(s =>
                    Ast(methodRefNode(
                      node,
                      selfMethod.span.text,
                      s"$s.${selfMethod.methodName}",
                      Defines.Any
                    ))
                )
                .getOrElse(Ast())
  ...
  private def astForKeywordArgument(assoc: Association): Ast =
    ...
    assoc.key match
      ...
      case _: (LiteralExpr | RubyCall | ProcOrLambdaExpr | MemberAccess | IndexAccess) =>
  ...
  private def getAssignmentOperatorName(op: String): Option[String] =
      AssignmentOperatorNames.get(op)
