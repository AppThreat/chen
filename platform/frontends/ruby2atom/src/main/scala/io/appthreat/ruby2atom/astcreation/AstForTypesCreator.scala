...
import io.appthreat.ruby2atom.astcreation.RubyIntermediateAst.{TypeDeclaration, *}
...
trait AstForTypesCreator(implicit withSchemaValidation: ValidationMode):
  ...
  private def astForSimpleNamedClassDeclaration(
    ...
    /** Pushes new NamespaceScope onto scope stack and populates AST_PARENT_FULL_NAME and
      * AST_PARENT_TYPE for TypeDecls that are declared in a namespace
      * @param typeDecl
      *   \- TypeDecl node
    ...
    def populateAstParentValues(typeDecl: NewTypeDecl, astParentFullName: String): NewTypeDecl =
      val namespaceBlockFullName =
          s"${scope.surroundingScopeFullName.getOrElse("")}.$astParentFullName"
      scope.pushNewScope(NamespaceScope(namespaceBlockFullName))

      val namespaceBlock =
          NewNamespaceBlock().name(astParentFullName).fullName(astParentFullName).filename(
            relativeFileName
          )
    ...
    def separateStatementsFromBody(ss: List[RubyExpression]) =
        ...
        ss.flatMap {
            case t: TypeDeclaration =>
                val (typeDeclAsts, other) =
                    astsForStatement(t).partition(_.root.exists(_.isInstanceOf[NewTypeDecl]))
    ...
    val classBodyAsts =
      ...
      if scope.shouldGenerateDefaultConstructor && this.parseLevel == AstParseLevel.FULL_AST then
        val bodyStart = classBody.span.spanStart()
        val initBody  = StatementList(List())(bodyStart)
        val methodDecl = astForMethodDeclaration(MethodDeclaration(
          Defines.Initialize,
          List(),
          initBody
        )(bodyStart))
  ...
  end astForSimpleNamedClassDeclaration
  ...
  private def astForTypeDeclBodyCall(node: TypeDeclBodyCall, typeFullName: String): Ast =
    ...
    callAst.nodes.collectFirst {
        case c: NewCall if c.name == Defines.TypeDeclBody =>
            c.methodFullName(s"$typeFullName.${Defines.TypeDeclBody}")
    }
    callAst
      ...
      if scope.isSurroundedByProgramScope then
        ...
        val typeRefNode = Ast(
          NewTypeRef()
              .code(typeRefCode)
              .typeFullName(
                s"${typeDecl.fullName}<class>"
              ) // Everything will be dispatched on the singleton
  ...
  private def astsForSingleFieldDeclaration(
    node: FieldsDeclaration,
    nameNode: RubyExpression
  ): Seq[Ast] =
      nameNode match
        case nameAsSymbol: StaticLiteral if nameAsSymbol.isSymbol =>
            ...
            val getterAst =
                Option.when(node.hasGetter)(astForGetterMethod(node, fieldName)).getOrElse(Nil)
            val setterAst =
                Option.when(node.hasSetter)(astForSetterMethod(node, fieldName)).getOrElse(Nil)
