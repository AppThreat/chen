...
import io.appthreat.ruby2atom.datastructures.{ConstructorScope, MethodScope}
...
import io.appthreat.x2cpg.{Ast, AstEdge, ValidationMode, Defines as XDefines}
...
trait AstForFunctionsCreator(implicit withSchemaValidation: ValidationMode):
  ...
  /** As expressions may be discarded, we cannot store closure ASTs in the diffgraph at the point of
    * creation. So we assume every reference to this map means that the closure AST was successfully
    * propagated.
    */
  ...
  /** Creates method declaration related structures.
    ...
    * @param isClosure
    *   if true, will generate a type decl, type ref, and method ref, as well as add the `c`
    *   modifier.
    * @return
  ...
  protected def astForMethodDeclaration(
    ...
    val fullName =
        node match
          ...
          case _ =>
              computeFullName(methodName, useSurroundingTypeFullName = useSurroundingTypeFullName)

    val astParentType =
        if useSurroundingTypeFullName || shouldUseSurroundingTypeFullName then
    ...
    if isConstructor then scope.pushNewScope(ConstructorScope(fullName, this.procParamGen.fresh))
    ...
    val stmtBlockAst = if isClosure || isSingletonObjectMethod then
      val baseStmtBlockAst = astForMethodBody(node.body, optionalStatementList)
      transformAsClosureBody(refs, baseStmtBlockAst)
    else if methodName == Defines.TypeDeclBody then
      val stmtList = node.body.asInstanceOf[StatementList]
      astForStatementList(
        StatementList(stmtList.statements ++ optionalStatementList.statements)(stmtList.span)
      )
    else if methodName != Defines.Initialize then
    ...
    val anonProcParam = scope.procParamName.map { p =>
      val nextIndex =
          parameterAsts.flatMap(_.root).lastOption.map { case m: NewMethodParameterIn =>
              m.index + 1
          }.getOrElse(0)
    ...
    val methodTypeDeclAst =
      ...
      if isClosure then Ast(typeDeclNode_).withChild(Ast(newModifierNode("LAMBDA")))
    ...
    val lambdaTypeDeclAst = if isClosure then
      val typeDeclNode_ =
          typeDeclNode(node, s"$methodName&Proc", s"$fullName&Proc", relativeFileName, code(node))
      astParentType.foreach(typeDeclNode_.astParentType(_))
      astParentFullName.foreach(typeDeclNode_.astParentFullName(_))
      Ast(typeDeclNode_)
          .withChild(
            // This member refers back to itself, as itself is the type decl bound to the respective method
            Ast(NewMember().name("call").code("call").dynamicTypeHintFullName(Seq(fullName))
    ...
    val accessModifier =
        ...
        if methodName == Defines.Initialize || methodName == Defines.TypeDeclBody then
          ModifierTypes.PRIVATE
        // <main> functions are private functions on the Object class
        else if isSurroundedByProgramScope then ModifierTypes.PRIVATE
    ...
    if isClosure then modifiers.addOne("LAMBDA")
    if isConstructor then modifiers.addOne(ModifierTypes.CONSTRUCTOR)
    ...
    val prefixMemberAst =
        if isClosure || isSingletonObjectMethod || isSurroundedByProgramScope then
          ...
          val methodMember = scope.surroundingTypeFullName match
            case Some(astParentTfn) =>
                memberForMethod(method, Option(NodeTypes.TYPE_DECL), Option(astParentTfn))
            case None =>
                memberForMethod(method, scope.surroundingAstLabel, scope.surroundingScopeFullName)
  ...
  protected def astForMethodAccessModifier(node: MethodAccessModifier): Seq[Ast] =
    ...
    methodAst
  end astForMethodAccessModifier
  ...
  private def transformAsClosureBody(refs: List[Ast], baseStmtBlockAst: Ast) =
    ...
    val capturedLocalNodes = baseStmtBlockAst.nodes
        .collect {
            case x: NewIdentifier if x.name != Defines.Self => x
        } // Self identifiers are handled separately
        .distinctBy(_.name)
    ...
    capturedLocalNodes
        .collect {
            case local: NewLocal =>
                val closureBindingId =
                    scope.variableScopeFullName(local.name).map(x => s"$x.${local.name}")
                (local, local.name, local.code, closureBindingId)
            case param: NewMethodParameterIn =>
                val closureBindingId =
                    scope.variableScopeFullName(param.name).map(x => s"$x.${param.name}")
        ...
        .collect { case (capturedLocal, name, code, Some(closureBindingId)) =>
            ...
            val _refEdges =
                capturedIdentifiers.filter(_.name == name).map(i => i -> capturingLocal) :+ (
                  closureBinding,
                  capturedLocal
                )
    ...
    val astWithAstChildren = astChildren.foldLeft(capturedBlockAst) { case (ast, child) =>
        ast.withChild(Ast(child))
    }
    val astWithRefEdges = refEdges.foldLeft(astWithAstChildren) { case (ast, (src, dst)) =>
        ast.withRefEdge(src, dst)
    }
    captureEdges.foldLeft(astWithRefEdges) { case (ast, (src, dst)) =>
        ast.withCaptureEdge(src, dst)
    }
  end transformAsClosureBody

  /** Creates the bindings between the method and its types. This is useful for resolving function
    * pointers and imports.
    */
  ...
  protected def astForAnonymousTypeDeclaration(node: AnonymousTypeDeclaration): Ast =
    ...
    typeRefAst.nodes
        .collectFirst { case typRef: NewTypeRef =>
            ...
            astForMemberCall(
              MemberCall(typeIdentifier, ".", "new", List.empty)(node.span.spanStart(newSpanText))
            )
        }
        .getOrElse(Ast())
  ...
  protected def astForSingletonMethodDeclaration(node: SingletonMethodDeclaration): Seq[Ast] =
      node.target match
        case targetNode: SingletonMethodIdentifier =>
            ...
            val methodTypeDecl_ =
                typeDeclNode(node, node.methodName, fullName, relativeFileName, code(node))
            ...
            val member = memberForMethod(
              method,
              Option(NodeTypes.TYPE_DECL),
              astParentFullName.map(x => s"$x<class>")
            )
            ...
            val _methodAst =
                methodAst(
                  ...
                  newModifierNode(ModifierTypes.VIRTUAL) :: newModifierNode(
  ...
  private def createMethodRefPointer(method: NewMethod): Ast =
      if scope.isSurroundedByProgramScope then
  ...
  private def astForMethodBody(
    ...
    returnLastExpression: Boolean = true
      ...
      if this.parseLevel == AstParseLevel.SIGNATURES then
        Ast()
      else
        body match
          case stmtList: StatementList =>
              val combinedStmtList =
                  StatementList(optionalStatementList.statements ++ stmtList.statements)(
                    stmtList.span
                  )
              if returnLastExpression then
                astForStatementListReturningLastExpression(combinedStmtList)
  ...
  private def astForConstructorMethodBody(
    body: RubyExpression,
    optionalStatementList: StatementList
  ): Ast =
      if this.parseLevel == AstParseLevel.SIGNATURES then
        ...
        body match
          case stmtList: StatementList =>
              astForStatementList(StatementList(
                optionalStatementList.statements ++ stmtList.statements
              )(stmtList.span))
          case _: (StaticLiteral | BinaryExpression | SingleAssignment | SimpleIdentifier | ArrayLiteral | HashLiteral |
                  SimpleCall | MemberAccess | MemberCall) =>
              astForStatementList(
  ...
  protected def popAccessModifier(): Unit =
      if accessModifierStack.nonEmpty then accessModifierStack.pop()

  private def shouldUseSurroundingTypeFullName: Boolean =
    val inBodyMethodScope =
        scope.surroundingScopeFullName.exists(x =>
            x.split("[.]").takeRight(1).contains(Defines.TypeDeclBody)
        )
