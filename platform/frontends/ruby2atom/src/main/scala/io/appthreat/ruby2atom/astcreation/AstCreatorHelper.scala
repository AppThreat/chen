...
import io.appthreat.ruby2atom.datastructures.{BlockScope, FieldDecl}
import io.appthreat.ruby2atom.passes.Defines
import io.appthreat.ruby2atom.passes.GlobalTypes
import io.appthreat.ruby2atom.passes.GlobalTypes.{kernelFunctions, kernelPrefix}
...
trait AstCreatorHelper(implicit withSchemaValidation: ValidationMode):
  ...
  protected def computeFullName(
    ...
    val surroundingName =
        if useSurroundingTypeFullName then scope.surroundingTypeFullName.head
        else scope.surroundingScopeFullName.head
    val candidate = counter match
      case Some(cnt) => s"$surroundingName.$name$cnt"
      case None      => s"$surroundingName.$name"
    if usedFullNames.contains(candidate) then
      computeFullName(name, counter.map(_ + 1).orElse(Option(0)), useSurroundingTypeFullName)
    else
      usedFullNames.add(candidate)
      candidate

  override def column(node: RubyExpression): Option[Integer]    = node.column
  override def columnEnd(node: RubyExpression): Option[Integer] = node.columnEnd
  override def line(node: RubyExpression): Option[Integer]      = node.line
  override def lineEnd(node: RubyExpression): Option[Integer]   = node.lineEnd
  ...
  private def astForFieldInstance(name: String, node: RubyExpression & RubyFieldIdentifier): Ast =
    val identName = node match
      ...
      case _: ClassFieldIdentifier =>
          scope.surroundingTypeFullName.map(_.split("[.]").last).getOrElse(Defines.Any)

    astForFieldAccess(
      MemberAccess(
        DummyNode(identifierNode(node, identName, identName, Defines.Any))(
          node.span.spanStart(identName)
        ),
        ".",
        name
      )(node.span)
            ...
            case Some(local) =>
                local match
                  ...
                  case x: NewMethodParameterIn =>
                      identifier.dynamicTypeHintFullName(x.dynamicTypeHintFullName)
                Ast(identifier).withRefEdge(identifier, local)
    end match
  end handleVariableOccurrence
  ...
  protected def astForAssignment(
    ...
    val _code =
        code.getOrElse(
          Seq(lhs, rhs).flatMap(_.root).collect { case x: ExpressionNew => x.code }.mkString(" = ")
        )
  ...
  private def nilCheckCondition(
    ...
    if op == "||=" then memberAccess
      ...
      UnaryExpression(op = "!", expression = memberAccess)(
        span.spanStart(s"!${memberAccess.span.text}")
      )

  /** Generates the assignment and the `thenClause` used in the lowering of `||=` and `&&=`
    */
  private def nilCheckThenClause(
    lhs: RubyExpression,
    rhs: RubyExpression,
    span: TextSpan
  ): RubyExpression =
      StatementList(List(
        SingleAssignment(lhs, "=", rhs)(span.spanStart(s"${lhs.span.text} = ${rhs.span.text}"))
      ))(
