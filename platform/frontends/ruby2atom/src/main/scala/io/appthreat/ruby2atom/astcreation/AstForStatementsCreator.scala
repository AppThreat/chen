...
import io.appthreat.ruby2atom.astcreation.RubyIntermediateAst.{RubyStatement, *}
...
import io.appthreat.x2cpg.datastructures.MethodLike
...
trait AstForStatementsCreator(implicit withSchemaValidation: ValidationMode):
  ...
  protected def foldIfExpression(builder: (IfExpression, Ast, Ast, List[Ast]) => Ast)(
    node: IfExpression
  ): Ast =
    val conditionAst = astForExpression(node.condition)
    val thenAst      = astForThenClause(node.thenClause)
  ...
  protected def astForThenClause(node: RubyExpression): Ast =
      astForStatementList(node.asStatementList)
        ...
        case elsIfNode :: rest =>
            elsIfNode match
              case elsIfNode: ElsIfClause =>
                  val newIf = IfExpression(
                    elsIfNode.condition,
                    elsIfNode.thenClause,
                    rest,
                    elseClause
                  )(elsIfNode.span)
  ...
  protected def astForDoBlock(block: Block & RubyExpression): Seq[Ast] =
      if closureToRefs.contains(block) then
        ...
        val methodRefAsts = block.body match
          case x: Block =>
              astForMethodDeclaration(
                x.toMethodDeclaration(methodName, Option(block.parameters)),
                isClosure = true
              )
          case _ =>
              astForMethodDeclaration(
                block.toMethodDeclaration(methodName, Option(block.parameters)),
                isClosure = true
              )
  ...
  private def astsForImplicitReturnStatement(node: RubyExpression): Seq[Ast] =
    def elseReturnNil(span: TextSpan) = Option {
        ElseClause(
          StatementList(
            ReturnExpression(
              StaticLiteral(getBuiltInType(Defines.NilClass))(span.spanStart("nil")) :: Nil
            )(
    ...
    node match
      ...
      case node: SingleAssignment =>
          astForSingleAssignment(node) :: List(
            astForReturnExpression(ReturnExpression(List(node.lhs))(node.span))
          )
      case node: DefaultMultipleAssignment =>
          astsForStatement(node) ++ astsForImplicitReturnStatement(
            ArrayLiteral(node.assignments.map(_.lhs))(node.span)
          )
      ...
      case node: AttributeAssignment =>
          List(
            ...
            astForReturnFieldAccess(
              MemberAccess(node.target, node.op, node.attributeName)(node.span)
            )
          )
      ...
      case stmtList: StatementList
          if stmtList.statements.lastOption.exists(_.isInstanceOf[ReturnExpression]) =>
  ...
  end astsForImplicitReturnStatement

  private def returnAstForRubyCall[C <: RubyCall](node: RubyExpression & RubyCallWithBlock[C])
    : Seq[Ast] =
  ...
  // E.g. `def f = 0` ===> `:f`
  private def astForReturnMethodDeclarationSymbolName(node: RubyExpression & ProcedureDeclaration)
    : Ast =
  ...
  /** Wraps the last RubyNode with a ReturnExpression.
    * @param x
    *   the node to wrap a return around. If a StatementList is given, then the ReturnExpression
    *   will wrap around the final element.
    * @return
  ...
  private def returnLastNode(
    ...
    def statementListReturningLastExpression(stmts: List[RubyExpression]): List[RubyExpression] =
        stmts match
    ...
    def clauseReturningLastExpression(x: RubyExpression & ControlFlowClause): RubyExpression =
        x match
          case RescueClause(exceptionClassList, assignment, thenClause) =>
              RescueClause(exceptionClassList, assignment, returnLastNode(thenClause, transform))(
                x.span
              )
          case EnsureClause(thenClause) =>
              EnsureClause(returnLastNode(thenClause, transform))(x.span)
          case ElsIfClause(condition, thenClause) =>
              ElsIfClause(condition, returnLastNode(thenClause, transform))(x.span)
    ...
    x match
      case StatementList(statements) =>
          StatementList(statementListReturningLastExpression(statements))(x.span)
  ...
  /** @param node
    ...
    * @param transform
    *   \- RubyNode => RubyNode function for transformation on the clauses of the
      ...
      node match
        case RescueExpression(body, rescueClauses, elseClause, ensureClause) =>
            ...
            RescueExpression(
              ...
              elseClause.map(transform).orElse(defaultElseBranch(node.span)).collect {
                  case x: ElseClause => x
              },
              ensureClause
            )(node.span)
        case WhileExpression(condition, body) =>
            WhileExpression(condition, transform(body))(node.span)
        case DoWhileExpression(condition, body) =>
            DoWhileExpression(condition, transform(body))(node.span)
        case UntilExpression(condition, body) =>
            UntilExpression(condition, transform(body))(node.span)
  ...
  private def generateAliasMethodDecl(alias: AliasStatement): MethodDeclaration =
    ...
    val forwardedArgs =
        SplattingRubyNode(SimpleIdentifier()(span.spanStart("args")))(span.spanStart("*args"))
    ...
    val aliasingMethodParams =
        ArrayParameter("*args")(span.spanStart("*args")) :: ProcParameter("&block")(
          span.spanStart("&block")
        ) :: Nil
