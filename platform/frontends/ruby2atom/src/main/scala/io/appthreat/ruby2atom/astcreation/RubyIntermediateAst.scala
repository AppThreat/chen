...
import io.appthreat.ruby2atom.passes.{Defines, GlobalTypes}
...
object RubyIntermediateAst:
  ...
  case class TextSpan(
    ...
    offset: Option[(Integer, Integer)],
    text: String
  ):
    def spanStart(newText: String = ""): TextSpan =
        TextSpan(line, column, line, column, offset, newText)

  /** Most-if-not-all constructs in Ruby evaluate to some value, so we name the base class
    * `RubyExpression`.
    */
  sealed class RubyExpression(val span: TextSpan):
    ...
    def offset: Option[(Integer, Integer)] = span.offset
  ...
  /** Ruby statements evaluate to some value (and thus are expressions), but also perform some
    * operation, e.g., assignments, method definitions, etc.
    */
  ...
  final case class FieldsDeclaration(fieldNames: List[RubyExpression], accessType: String)(
    span: TextSpan
  ) extends RubyExpression(span)
    ...
    def isSplattingFieldDecl: Boolean =
        fieldNames.length == 1 && fieldNames.head.isInstanceOf[SplattingRubyNode]
  ...
  final case class MandatoryParameter(name: String)(span: TextSpan) extends RubyExpression(span)
      with MethodParameter:
    def toSimpleIdentifier: SimpleIdentifier = SimpleIdentifier()(span)

  final case class OptionalParameter(name: String, defaultExpression: RubyExpression)(
    span: TextSpan
  ) extends RubyExpression(span)
      with MethodParameter
  ...
  final case class ArrayParameter(name: String)(span: TextSpan) extends RubyExpression(span)
      with CollectionParameter

  final case class HashParameter(name: String)(span: TextSpan) extends RubyExpression(span)
      with CollectionParameter

  final case class ProcParameter(name: String)(span: TextSpan) extends RubyExpression(span)
      with MethodParameter

  final case class SingleAssignment(lhs: RubyExpression, op: String, rhs: RubyExpression)(
    span: TextSpan
  ) extends RubyExpression(span)
      with RubyStatement
  ...
  final case class OperatorAssignment(lhs: RubyExpression, op: String, rhs: RubyExpression)(
    span: TextSpan
  ) extends RubyExpression(span)
      with RubyStatement
  ...
  final case class SplattingRubyNode(target: RubyExpression)(span: TextSpan)
      extends RubyExpression(span)
  ...
  /** Any structure that conditionally modifies the control flow of the program. These also behave
    * as statements.
    */
  ...
  final case class DoWhileExpression(condition: RubyExpression, body: RubyExpression)(
    span: TextSpan
  ) extends RubyExpression(span)
      with ControlFlowStatement
  ...
  final case class ElsIfClause(condition: RubyExpression, thenClause: RubyExpression)(
    span: TextSpan
  ) extends RubyExpression(span)
      with ControlFlowClause
  ...
  final case class BreakExpression()(span: TextSpan) extends RubyExpression(span)
      with ControlFlowStatement
  ...
  final case class InstanceFieldIdentifier()(span: TextSpan) extends RubyExpression(span)
      with RubyFieldIdentifier

  /** Represents a ClassFieldIdentifier e.g `@@x` */
  final case class ClassFieldIdentifier()(span: TextSpan) extends RubyExpression(span)
      with RubyFieldIdentifier

  final case class SelfIdentifier()(span: TextSpan) extends RubyExpression(span)
      with SingletonMethodIdentifier
  ...
  final case class StaticLiteral(typeFullName: String)(span: TextSpan) extends RubyExpression(span)
      with LiteralExpr:
  ...
  final case class DynamicLiteral(typeFullName: String, expressions: List[RubyExpression])(
    span: TextSpan
  ) extends RubyExpression(span)
      with LiteralExpr
  ...
  final case class Association(key: RubyExpression, value: RubyExpression)(span: TextSpan)
      extends RubyExpression(span)
  ...
  sealed trait RubyCall extends RubyExpression:
    ...
    def withBlock(block: Block): RubyCallWithBlock[?] =
        SimpleCallWithBlock(target, arguments, block)(span)

  /** Represents traditional calls, e.g. `foo`, `foo x, y`, `foo(x,y)` */
  final case class SimpleCall(target: RubyExpression, arguments: List[RubyExpression])(
    span: TextSpan
  ) extends RubyExpression(span)
      with RubyCall
  ...
  final case class RaiseCall(target: RubyExpression, arguments: List[RubyExpression])(
    span: TextSpan
  ) extends RubyExpression(span)
      with RubyCall
  ...
  final case class PublicModifier()(span: TextSpan) extends RubyExpression(span)
      with AccessModifier:
    override def toSimpleIdentifier: SimpleIdentifier = SimpleIdentifier(None)(span)

  final case class PrivateModifier()(span: TextSpan) extends RubyExpression(span)
      with AccessModifier:
    override def toSimpleIdentifier: SimpleIdentifier = SimpleIdentifier(None)(span)

  final case class ProtectedModifier()(span: TextSpan) extends RubyExpression(span)
      with AccessModifier:
    override def toSimpleIdentifier: SimpleIdentifier = SimpleIdentifier(None)(span)
  ...
  final case class PrivateMethodModifier(method: RubyExpression)(span: TextSpan)
    ...
    override def toSimpleIdentifier: SimpleIdentifier =
        SimpleIdentifier(None)(span.spanStart("private_class_method"))

  final case class PublicMethodModifier(method: RubyExpression)(span: TextSpan)
      extends RubyExpression(span)
      with MethodAccessModifier:
    override def toSimpleIdentifier: SimpleIdentifier =
        SimpleIdentifier(None)(span.spanStart("public_class_method"))
  ...
  final case class YieldExpr(arguments: List[RubyExpression])(span: TextSpan)
      extends RubyExpression(span)
  ...
  final case class IndexAccess(target: RubyExpression, indices: List[RubyExpression])(
    span: TextSpan
  ) extends RubyExpression(span)

  final case class MemberAccess(target: RubyExpression, op: String, memberName: String)(
    span: TextSpan
  ) extends RubyExpression(span):
  ...
  final case class SimpleObjectInstantiation(
    target: RubyExpression,
    arguments: List[RubyExpression]
  )(span: TextSpan)
      extends RubyExpression(span)
  ...
  final case class ObjectInstantiationWithBlock(
    target: RubyExpression,
    arguments: List[RubyExpression],
    block: Block
  )(
  ...
  final case class Block(parameters: List[RubyExpression], body: RubyExpression)(span: TextSpan)
    ...
    def toMethodDeclaration(
      name: String,
      parameters: Option[List[RubyExpression]]
    ): MethodDeclaration =
        parameters match
  ...
  /** A dummy class for wrapping around `NewNode` and allowing it to integrate with RubyNode
    * classes.
    */
  final case class DummyNode(node: NewNode)(span: TextSpan) extends RubyExpression(span)

  final case class UnaryExpression(op: String, expression: RubyExpression)(span: TextSpan)
      extends RubyExpression(span)

  final case class BinaryExpression(lhs: RubyExpression, op: String, rhs: RubyExpression)(
    span: TextSpan
  ) extends RubyExpression(span)
