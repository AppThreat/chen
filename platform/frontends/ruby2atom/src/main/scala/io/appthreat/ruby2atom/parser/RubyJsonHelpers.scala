...
import io.appthreat.ruby2atom.passes.Defines.getBuiltInType
import upickle.core.*
...
object RubyJsonHelpers:
  ...
  implicit class JsonObjHelper(o: ujson.Obj):
    ...
    def toTextSpan: TextSpan =
      val metaData =
          if o.obj.contains(ParserKeys.MetaData) then read[MetaData](o(ParserKeys.MetaData))
    ...
    def visitOption(key: String)(implicit
      visit: ujson.Value => RubyExpression
    ): Option[RubyExpression] =
        if contains(key) then Option(visit(o(key))) else None

    def visitArray(key: String)(implicit
      visit: ujson.Value => RubyExpression
    ): List[RubyExpression] =
  ...
  def createClassBodyAndFields(
    ...
    visit: ujson.Value => RubyExpression
  ): (StatementList, List[RubyExpression & RubyFieldIdentifier]) =
    ...
    def bodyMethod(fieldStatements: List[RubyExpression]): MethodDeclaration =
      val body = fieldStatements
          .map {
              case field: SimpleIdentifier =>
                  ...
                  SingleAssignment(ClassFieldIdentifier()(field.span), "=", nilLiteral(field.span))(
                    assignmentSpan
                  )
      ...
      MethodDeclaration(
        ...
        Nil,
        StatementList(body)(obj.toTextSpan.spanStart(s"(...)"))
      )(
    ...
    def getFields(
      ...
      typeDeclChildStatements: Boolean = true
        ...
        expr match
          case field: SimpleIdentifier if typeDeclChildStatements =>
              ClassFieldIdentifier()(field.span) :: Nil
          ...
          case _ @StatementList(stmts) =>
              stmts.flatMap(x => getFields(x, typeDeclChildStatements)).distinctBy(_.text)
          case _ => Nil

    /** Attempts to evaluate and parse the collection associated with the splattingField, generating
      * FieldDeclarations for each of the elements.
      * @param fieldStmts
      ...
      * @return
      *   List of:
      *   - Some(_) => if splattingField either is evaluated to a list of FieldDeclarations,
      *     otherwise a SimpleCall
    ...
    def lowerSplattingFieldDecl(
      ...
      splattingFields: List[RubyExpression]
        ...
        splattingFields.flatMap {
            case x @ FieldsDeclaration(fieldName :: Nil, accessType)
                if fieldName.isInstanceOf[SplattingRubyNode] =>
                fieldStmts.map {
                    case _ @SingleAssignment(lhs: SimpleIdentifier, _, rhs: MemberAccess)
                        if rhs.memberName == "freeze" && lhs.span.text == fieldName.span.text
                            .stripPrefix("*") =>
                        rhs.target match
                          case y: ArrayLiteral =>
                    ...
                    case _ =>
                        Some(
                          SimpleCall(
                            SimpleIdentifier()(x.span.spanStart(accessType)),
                            List(fieldName)
                          )(
    ...
    obj.visitOption(ParserKeys.Body).map(lowerSingletonClassDecls) match
      case Some(stmtList @ StatementList(expression :: Nil))
          if expression.isInstanceOf[AllowedTypeDeclarationChild] =>
          if isSplattingField(expression) then
            ...
            splattingField.fieldNames.headOption match
              case Some(splattingFieldName) =>
                  val nonExpandedSplattingFieldCall =
                      ...
                      )(expression.span.spanStart(
                        s"${splattingField.accessType}(${splattingFieldName.span.text})"
                      ))
                  (
                    StatementList(bodyMethod(List(nonExpandedSplattingFieldCall)) :: Nil)(
                      stmtList.span
                    ),
              ...
              case None =>
                  (
                    StatementList(bodyMethod(Nil) :: expression :: Nil)(stmtList.span),
                    getFields(expression)
                  )
            end match
          else
      ...
      case Some(stmtList @ StatementList(expression :: Nil)) if isFieldStmt(expression) =>
          (
            StatementList(bodyMethod(expression :: Nil) :: Nil)(stmtList.span),
            getFields(expression)
          )
      case Some(stmtList: StatementList) =>
          val (fieldStmts, otherStmts) = stmtList.statements.partition(isFieldStmt)
          val (typeDeclStmts, bodyStmts) =
              otherStmts.partition(_.isInstanceOf[AllowedTypeDeclarationChild])
          ...
          val fields =
              (fieldStmts.flatMap(x => getFields(x)) ++ otherTypeDeclStmts.flatMap(x =>
                  getFields(x)
              ))
                  .distinctBy(_.text)
          val body = stmtList.copy(statements =
              bodyMethod(
                fieldStmts ++ otherTypeDeclStmts.flatMap(x =>
                    getFields(x)
                ) ++ bodyStmts ++ nonExpandedSplattingFieldsCalls
  ...
  def getParts(memberAccess: MemberAccess): List[String] =
      memberAccess.target match
        case targetMemberAccess: MemberAccess =>
            getParts(targetMemberAccess) :+ memberAccess.memberName
  ...
  def lowerMultipleAssignment(
    ...
    /** Recursively expand and duplicate splatting nodes so that they line up with what they
      * consume.
      *
      * @param nodes
      *   the splat nodes.
    ...
    val assignments = if (lhsNodes ++ rhsNodes).exists(_.isInstanceOf[SplattingRubyNode]) then
      rhsNodes.size - lhsNodes.size match
        ...
        case x if x > 0 =>
            ...
            slurpedLhs
                ...
                .sortBy { x =>
                    slurpedLhs.indexOf(x._1)
                } // groupBy produces a map which discards insertion order
                .map {
                    case (SplattingRubyNode(lhs), rhss) =>
                        SingleAssignment(lhs, op, ArrayLiteral(rhss)(obj.toTextSpan))(
                          obj.toTextSpan
                        )
                    case (lhs, rhs :: Nil) => SingleAssignment(lhs, op, rhs)(obj.toTextSpan)
                    case (lhs, rhss) => SingleAssignment(
                          lhs,
                          op,
                          ArrayLiteral(rhss)(obj.toTextSpan)
                        )(obj.toTextSpan)
                }
                .toList
        ...
        case x if x < 0 =>
            ...
            lhsNodes
                ...
                .sortBy { x =>
                    slurpedRhs.indexOf(x._1)
                } // groupBy produces a map which discards insertion order
                .flatMap {
                    case (SplattingRubyNode(rhs), lhss) =>
                        lhss.map(
                          SingleAssignment(_, op, SplattingRubyNode(rhs)(rhs.span))(obj.toTextSpan)
                        )
                    case (rhs, lhs :: Nil) => Seq(SingleAssignment(lhs, op, rhs)(obj.toTextSpan))
                    case (rhs, lhss) => lhss.map(SingleAssignment(
                          _,
                          op,
                          SplattingRubyNode(rhs)(rhs.span)
                        )(obj.toTextSpan))
                }
                .toList
      ...
      UnaryExpression(
        ...
        MemberAccess(
          SimpleIdentifier(Option(getBuiltInType(Defines.Float)))(
            obj.toTextSpan.spanStart("Float")
          ),
          "::",
          "INFINITY"
        )(obj.toTextSpan.spanStart("Float::INFINITY"))
      ...
      classBody match
        case stmtList: StatementList =>
            StatementList(stmtList.statements.flatMap {
                case _ @SingletonClassDeclaration(
                      _,
                      baseClass: Some[RubyExpression],
                      body: StatementList,
                      _
                    ) =>
                    body.statements.map {
                        case method @ MethodDeclaration(methodName, parameters, body) =>
                            SingletonMethodDeclaration(baseClass.get, methodName, parameters, body)(
                              method.span
                            )
  ...
  private case class MetaData(
    code: String,
    @upickle.implicits.key("start_line") lineNumber: Integer,
    @upickle.implicits.key("start_column") columnNumber: Integer,
    @upickle.implicits.key("end_line") lineNumberEnd: Integer,
    @upickle.implicits.key("end_column") columnNumberEnd: Integer,
    @upickle.implicits.key("offset_start") offsetStart: Integer,
    @upickle.implicits.key("offset_end") offsetEnd: Integer
  ) derives ReadWriter
end RubyJsonHelpers
