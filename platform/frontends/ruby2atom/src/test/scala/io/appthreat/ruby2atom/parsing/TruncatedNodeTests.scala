...
/** A file whose JSON contains depth-truncated nodes (generated with `rbastgen --max-depth 3`, see
  * `resources/ruby/nested/truncated.rb.json`) must degrade one node per truncation point — not lose
  * the whole file. Before the truncated-node guard this fixture threw in `visitArray` (missing
  * `children` key) and the exception escaped as a swallowed `Failure`, silently dropping the file
  * from the CPG.
  */
