...
class TypeInferencePass(cpg: Cpg) extends ConcurrentWriterCpgPass[Call](cpg):
  ...
  private def isMatchingMethod(method: Method, call: Call, callNameParts: NameParts): Boolean =
    ...
    val argSizeMod =
        if method.modifier.modifierType.iterator.contains(ModifierTypes.STATIC) then 1 else 0
    ...
    lazy val argTypesMatch =
        doArgumentTypesMatch(method: Method, call: Call, skipCallThis = argSizeMod == 1)
  ...
  /** Check if argument types match by comparing exact full names. An argument type of `ANY` always
    * matches.
    *
  ...
  private def doArgumentTypesMatch(method: Method, call: Call, skipCallThis: Boolean): Boolean =
    val callArgs = if skipCallThis then call.argument.toList.tail else call.argument.toList
    ...
    val hasDifferingArg = method.parameter.zip(callArgs).exists { case (parameter, argument) =>
        ...
        val argMatches =
            maybeArgumentType == TypeConstants.Any || maybeArgumentType == parameter.typeFullName
  ...
  private def getReplacementMethod(call: Call): Option[Method] =
    val argTypes =
        call.argument.flatMap(arg =>
            Option(arg.property(PropertyNames.TypeFullName)).map(_.toString)
        ).mkString(":")
    val callKey =
    ...
    cache.get(callKey).toScala.getOrElse {
        ...
        resolvedMethodIndex.get(call.name).flatMap { candidateMethods =>
          ...
          val uniqueMatchingMethod =
              candidateMethodsIter.find(isMatchingMethod(_, call, callNameParts)).flatMap {
                  method =>
                    val otherMatchingMethod =
                        candidateMethodsIter.find(isMatchingMethod(_, call, callNameParts))
