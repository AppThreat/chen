...
object BindingTable:
  ...
  def createBindingTable[InputTypeDecl, AstTypeDecl, AstMethodDecl, TypeMap](
    ...
    adapter.directParents(typeDecl).filterNot(adapter.typeDeclEquals(_, typeDecl)).foreach {
        parentTypeDecl =>
              ...
              try
                ...
                case e: StackOverflowError =>
                    throw new RuntimeException(
                      s"SOE getting binding table for $typeDeclFullName"
                    )
    ...
    adapter.allParentsWithTypeMap(typeDecl).foreach {
        case (parentTypeDecl, typeParameterInDerivedContext) =>
            directTableEntries.foreach { directTableEntry =>
              ...
              parentMethods.foreach { case (parentName, parentMethodDecl) =>
                  if directTableEntry.name == parentName then
                    val parentSigInDerivedContext = adapter.getMethodSignature(
                      parentMethodDecl,
                      typeParameterInDerivedContext
                    )
                    if directTableEntry.signature == parentSigInDerivedContext then
                      val erasedParentMethodSig =
                          adapter.getMethodSignatureForEmptyTypeMap(parentMethodDecl)
