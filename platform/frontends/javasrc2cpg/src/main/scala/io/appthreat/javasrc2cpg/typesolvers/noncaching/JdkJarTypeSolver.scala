...
object JdkJarTypeSolver:
    ...
    /** Convert Jar entry name foo/bar/qux/Baz.class to package prefix foo.bar Only use first 2
      * parts since this is sufficient to deterimine whether a class has been registered in most
      * cases and, if not, the failure is just a slow lookup.
      */
    ...
    /** Convert jmod entry name classes/foo/bar/qux/Baz.class to package prefix foo.bar Only use
      * first 2 parts since this is sufficient to deterimine whether a class has been registered in
      * most cases and, if not, the failure is just a slow lookup.
      */
    ...
    /** A name is assumed to contain at least one subclass (e.g. ...Foo$Bar) if the last name part
      * starts with a digit, or if the last 2 name parts start with capital letters. This heuristic
      * is based on the class name format in the JDK jars, where names with subclasses have one of
      * the forms:
    ...
    /** JavaParser replaces the `$` in nested class names with a `.`. This method converts the
      ...
      * with a capital letter or a digit with a `$` since the jdk classes follow the standard
      * practice of capitalising the first letter in class names but not package names.
      */
