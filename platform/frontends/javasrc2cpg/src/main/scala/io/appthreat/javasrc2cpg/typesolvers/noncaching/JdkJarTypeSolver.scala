...
class JdkJarTypeSolver extends TypeSolver:
  ...
  override def tryToSolveType(javaParserName: String)
    : SymbolReference[ResolvedReferenceTypeDeclaration] =
    val packagePrefix = packagePrefixForJavaParserName(javaParserName)
    if knownPackagePrefixes.contains(packagePrefix) then
  ...
  private def addPathToClassPool(archivePath: String): Try[ClassPath] =
      if archivePath.isJarPath then
        Try(classPool.appendClassPath(archivePath))
      else if archivePath.isJmodPath then
  ...
  private def registerPackagesForJar(archivePath: String): Unit =
    val entryNameConverter =
        if archivePath.isJarPath then packagePrefixForJarEntry else packagePrefixForJmodEntry
    try
        ...
        def jarPackages = jarFile
            ...
            .filter(entry =>
                !entry.isDirectory && !entry.getName
                    .startsWith("module-info") && (entry.getName.endsWith(
                  ClassExtension
                ) || entry.getName
...
object JdkJarTypeSolver:
  ...
  /** Convert JavaParser class name foo.bar.qux.Baz to package prefix foo.bar Only use first 2 parts
    * since this is sufficient to deterimine whether a class has been registered in most cases and,
    * if not, the failure is just a slow lookup.
    */
  def packagePrefixForJavaParserName(className: String): String =
      className.split("\\.").take(2).mkString(".")

  /** Convert Jar entry name foo/bar/qux/Baz.class to package prefix foo.bar Only use first 2 parts
    * since this is sufficient to deterimine whether a class has been registered in most cases and,
    * if not, the failure is just a slow lookup.
    */
  ...
  /** Convert jmod entry name classes/foo/bar/qux/Baz.class to package prefix foo.bar Only use first
    * 2 parts since this is sufficient to deterimine whether a class has been registered in most
    * cases and, if not, the failure is just a slow lookup.
    */
  def packagePrefixForJmodEntry(entryName: String): String =
      packagePrefixForJarEntry(entryName.stripPrefix(JmodClassPrefix))

  /** A name is assumed to contain at least one subclass (e.g. ...Foo$Bar) if the last name part
    * starts with a digit, or if the last 2 name parts start with capital letters. This heuristic is
    * based on the class name format in the JDK jars, where names with subclasses have one of the
    * forms:
  ...
  /** JavaParser replaces the `$` in nested class names with a `.`. This method converts the
    * JavaParser names to the standard format by replacing the `.` between name parts that start
    * with a capital letter or a digit with a `$` since the jdk classes follow the standard practice
    * of capitalising the first letter in class names but not package names.
    */
