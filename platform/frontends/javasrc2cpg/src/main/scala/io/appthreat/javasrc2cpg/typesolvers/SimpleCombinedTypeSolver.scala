        ...
        typeCache.get(name).toScala match
            ...
            case None =>
                findSolvedTypeWithSolvers(cachingTypeSolvers, name)
                    .getOrElse {
                        val result = findSolvedTypeWithSolvers(
                          nonCachingTypeSolvers,
                          name
    ...
    private def findSolvedTypeWithSolvers(
      ...
      className: String
        ...
        typeSolvers.iterator
            .map { typeSolver =>
                try
                    val result = typeSolver.tryToSolveType(className): SymbolReference[
