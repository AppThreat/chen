package io.appthreat.javasrc2cpg

import io.appthreat.javasrc2cpg.jpastprinter.JavaParserAstPrinter
import io.appthreat.javasrc2cpg.typesolvers.SimpleCombinedTypeSolver
import Frontend.*
  ...
  val cmdLineParser: OParser[Unit, Config] =
    ...
    OParser.sequence(
      ...
      opt[String]("delombok-java-home")
          .text(
            "Optional override to set java home used to run Delombok. Java 17 is recommended for the best results."
          )
      ...
      opt[String]("jdk-path")
          ...
          .text(
            "JDK used for resolving builtin Java types. If not set, current classpath will be used"
          ),
      opt[Unit]("show-env")
      ...
      opt[Unit]("dump-javaparser-asts")
          ...
          .text(
            "Dump the javaparser asts for the given input files and terminate (for debugging)."
          )
    )
  end cmdLineParser
end Frontend
