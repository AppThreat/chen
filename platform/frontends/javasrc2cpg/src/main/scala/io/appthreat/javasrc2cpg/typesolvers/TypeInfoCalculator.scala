...
import TypeInfoCalculator.{TypeConstants, TypeNameConstants}
...
class TypeInfoCalculator(global: Global, symbolResolver: SymbolResolver):
      ...
      trySubstitutedType
          .map { substitutedType =>
            ...
            val isSimilarWildcardSubstition = substitutedType match
              case wc: ResolvedWildcard =>
                  Try(wc.getBoundedType.asTypeParameter()).toOption.contains(typeParamDecl)
              case _ => false
    ...
    fullyQualified: Boolean
      ...
      typ match
        ...
        case arrayType: ResolvedArrayType =>
            nameOrFullName(arrayType.getComponentType, typeParamValues, fullyQualified).map(
              _ + "[]"
            )
        ...
        case typeVariable: ResolvedTypeVariable =>
            ...
            if typesSubstituted(substitutedType, typeParamDecl) then
              nameOrFullName(substitutedType.get, typeParamValues, fullyQualified)
            else
              val extendsBoundOption =
                  Try(typeParamDecl.getBounds.asScala.find(_.isExtends)).toOption.flatten
              extendsBoundOption
                  .flatMap(bound =>
                      nameOrFullName(bound.getType, typeParamValues, fullyQualified)
                  )
        ...
        case wildcardType: ResolvedWildcard =>
            if wildcardType.isBounded then
        ...
        case _ =>
            ...
            Try(symbolResolver.toResolvedType(typ, classOf[ResolvedType])).toOption
                .flatMap(resolvedType =>
                    nameOrFullName(resolvedType, emptyTypeParamValues, fullyQualified)
                )
  ...
  private def nameOrFullName(
    typeDecl: ResolvedTypeDeclaration,
    fullyQualified: Boolean
  ): Option[String] =
      typeDecl match
        case typeParamDecl: ResolvedTypeParameterDeclaration =>
            if fullyQualified then
              val containFullName =
                  nameOrFullName(
                    typeParamDecl.getContainer.asInstanceOf[ResolvedDeclaration],
                    fullyQualified = true
                  )
        ...
        case _ =>
            val typeName = Option(typeDecl.getName).getOrElse(
            ...
            if isInnerTypeDecl then
              nameOrFullName(typeDecl.containerType().get, fullyQualified).map(
                _ + "$" + typeName
              )
            else if fullyQualified then
              val packageName = typeDecl.getPackageName

              if packageName == null || packageName == "" then
                Some(typeName)
              else
  ...
  /** Add `typeName` to a global map and return it. The map is later passed to a pass that creates
    * TYPE nodes for each key in the map. Skip the `ANY` type, since this is created by default.
    * TODO: I want the type registration not in here but for now it is the easiest.
    */
  def registerType(typeName: String): String =
    if typeName != "ANY" then
      global.usedTypes.putIfAbsent(typeName, true)
    typeName
end TypeInfoCalculator
