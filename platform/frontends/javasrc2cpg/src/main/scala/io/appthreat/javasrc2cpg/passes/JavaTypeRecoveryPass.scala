...
private class JavaTypeRecovery(cpg: Cpg, state: XTypeRecoveryState)
    extends XTypeRecovery[Method](cpg, state):
...
private class RecoverForJavaFile(
  ...
  state: XTypeRecoveryState
    ...
    // and code properties.
    override protected def storeIdentifierTypeInfo(i: Identifier, types: Seq[String]): Unit =
        if i.name != "this" then
