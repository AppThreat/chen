package io.appthreat.javasrc2cpg.jartypereader.descriptorparser

import io.appthreat.javasrc2cpg.jartypereader.model
import io.appthreat.javasrc2cpg.jartypereader.model.Bound.{BoundAbove, BoundBelow}
import io.appthreat.javasrc2cpg.jartypereader.model.{
    ArrayTypeSignature,
    BoundWildcard,
    ClassSignature,
...
trait TypeParser extends TokenParser:
    ...
    def typeVarSignature: Parser[TypeVariableSignature] =
        (typeVarStart ~ identifier ~ semicolon) ^^ { case _ ~ name ~ _ =>
    ...
    def simpleClassTypeSignature: Parser[NameWithTypeArgs] =
        (identifier ~ opt(typeArguments)) ^^ { case name ~ maybeTypes =>
            model.NameWithTypeArgs(name, maybeTypes.getOrElse(Nil))
        }
    ...
    def classTypeSignature: Parser[ClassTypeSignature] =
        (classTypeStart ~ opt(packageSpecifier) ~ simpleClassTypeSignature ~ rep(
    ...
    def methodSignature: Parser[MethodSignature] =
        (opt(typeParameters) ~ openParen ~ rep(javaTypeSignature) ~ closeParen ~ returnType ~ rep(
          javaTypeSignature
        )) ^^ {
