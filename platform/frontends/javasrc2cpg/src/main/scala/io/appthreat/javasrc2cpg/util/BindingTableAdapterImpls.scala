...
import Util.{composeMethodFullName, getAllParents, safeGetAncestors}
...
object Shared:
    def getDeclaredMethods(typeDecl: ResolvedReferenceTypeDeclaration)
      : Iterable[ResolvedMethodDeclaration] =
        typeDecl match
...
class BindingTableAdapterForJavaparser(
  methodSignatureImpl: (ResolvedMethodDeclaration, ResolvedTypeParametersMap) => String
    ...
    override def getMethodSignature(
      methodDecl: ResolvedMethodDeclaration,
      typeMap: ResolvedTypeParametersMap
    ): String =
...
end BindingTableAdapterForJavaparser
...
class BindingTableAdapterForLambdas(
  methodSignatureImpl: (ResolvedMethodDeclaration, ResolvedTypeParametersMap) => String
    ...
    override def directParents(lambdaBindingInfo: LambdaBindingInfo)
      : collection.Seq[ResolvedReferenceTypeDeclaration] =
    ...
    override def allParentsWithTypeMap(
      lambdaBindingInfo: LambdaBindingInfo
        ...
        val nonDirectParents =
            lambdaBindingInfo.implementedType.flatMap(_.getTypeDeclaration.toScala).toList.flatMap(
              getAllParents
            )
...
end BindingTableAdapterForLambdas
