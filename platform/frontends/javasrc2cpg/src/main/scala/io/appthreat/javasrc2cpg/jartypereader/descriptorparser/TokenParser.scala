...
import io.appthreat.javasrc2cpg.jartypereader.model.PrimitiveType
import io.appthreat.javasrc2cpg.jartypereader.model.Model.TypeConstants.*
...
trait TokenParser extends RegexParsers:
  ...
  private def translateBaseType(descriptor: String): String =
      descriptor match
        ...
        case "V" => Void
        case unk =>
            throw new IllegalArgumentException(s"$unk is not a valid base type descriptor.")

  def baseType: Parser[PrimitiveType] = "[BCDFIJSZ]".r ^^ { t =>
      PrimitiveType(translateBaseType(t))
  }
