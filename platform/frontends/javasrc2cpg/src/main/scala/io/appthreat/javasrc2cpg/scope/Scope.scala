package io.appthreat.javasrc2cpg.scope

import io.appthreat.javasrc2cpg.passes.ExpectedType
...
import io.appthreat.x2cpg.utils.ListUtils.*
import io.shiftleft.codepropertygraph.generated.nodes.NewNamespaceBlock
import io.appthreat.x2cpg.Ast
...
class Scope:
    ...
    private def lookupResultFromFound(
      ...
      variable: ScopeVariable
        ...
        val isCaptureChain =
            typeDecls.size > 1 || (typeDecls.nonEmpty && !foundSubstack.last.isInstanceOf[
              TypeDeclScope
            ])

        if isCaptureChain then
            CapturedVariable(typeDecls.map(_.typeDecl), variable)
        else
            SimpleVariable(variable)

    def enclosingTypeDecl: Option[NewTypeDecl] =
        scopeStack.collectFirst { case typeDeclScope: TypeDeclScope =>
    ...
    def addMemberInitializers(initializers: Seq[Ast]): Unit =
        scopeStack.collectFirst { case typeDeclScope: TypeDeclScope =>
            typeDeclScope.addMemberInitializers(initializers)
        }

    // TODO: Refactor and remove this
    def memberInitializers: List[Ast] =
        scopeStack
            .collectFirst { case typeDeclScope: TypeDeclScope =>
                typeDeclScope.getMemberInitializers()
            }
            .getOrElse(Nil)
...
object Scope:
    ...
    final case class ScopeParameter(override val node: NewMethodParameterIn) extends ScopeVariable:
        val typeFullName: String = node.typeFullName
        val name                 = node.name
    final case class ScopeMember(override val node: NewMember, isStatic: Boolean)
            ...
            val nodeTypeInfo = variable match
                case ScopeMember(memberNode, isStatic) =>
                    NodeTypeInfo(
                      memberNode,
                      memberNode.name,
                      Some(memberNode.typeFullName),
                      true,
                      isStatic
                    )

                case variable => NodeTypeInfo(
                      variable.node,
                      variable.name,
