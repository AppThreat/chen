package io.appthreat.javasrc2cpg.testfixtures
...
class JavaSrcTestCpg(enableTypeRecovery: Boolean = false) extends TestCpg with JavaSrcFrontend:
  ...
  override def applyPasses(): Unit =
    ...
    if enableTypeRecovery then JavaSrc2Cpg.typeRecoveryPasses(this).foreach(_.createAndApply())
...
class JavaSrcCode2CpgFixture(withOssDataflow: Boolean = false, enableTypeRecovery: Boolean = false)
    extends Code2CpgFixture(() =>
        new JavaSrcTestCpg(enableTypeRecovery).withOssDataflow(withOssDataflow)
    ):
