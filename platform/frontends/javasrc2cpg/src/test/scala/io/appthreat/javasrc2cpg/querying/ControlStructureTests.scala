...
import io.appthreat.javasrc2cpg.util.NameConstants
...
class NewControlStructureTests extends JavaSrcCode2CpgFixture:
      ...
      "create nodes for resources" in {
          cpg.method.name("foo").body.astChildren.l match
            case List(
                ...
                val List(frAssignLhs: Identifier, frAssignRhs: Call) =
                    frAssign.argument.l: @unchecked
                ...
                frInit.name shouldBe io.appthreat.x2cpg.Defines.ConstructorMethodName
                val List(frInitThis: Identifier, frInitArg: Identifier) =
                    frInit.argument.l: @unchecked
                ...
                val List(brAssignLhs: Identifier, brAssignRhs: Call) =
                    brAssign.argument.l: @unchecked
                ...
                brInit.name shouldBe io.appthreat.x2cpg.Defines.ConstructorMethodName
                val List(brInitThis: Identifier, brInitArg: Identifier) =
                    brInit.argument.l: @unchecked
  ...
  "foreach loops over native array initialization expressions" should {
      ...
      "assign the array to the created local" in {
          val assignment = cpg.method
              ...
              .find { assignment =>
                  assignment.argument.l match
                    case List(identifier: Identifier, _: Call)
                        if identifier.name == "$iterLocal0" => true
                    case _ => false
              } match
          ...
          arrayInitializer.astChildren.zip(expectedLiterals).foreach {
              case (initChild, expectedCode) =>
            ...
            case List(idx: Identifier, arraySize: Call) =>
                ...
                arraySize.argument.l match
                  case List(items: Identifier, length: FieldIdentifier) =>
                      ...
                      items.refOut.toSet should contain(
                        cpg.method.name("foo").local.nameExact("$iterLocal0").head
                      )
      ...
      "create an assignment to the `item` local in the FOR body" in {
          ...
          val (itemLocal: Local, itemAssign: Call, sink: Call) = body.astChildren.l match
            case List(itemLocal: Local, itemAssign: Call, sink: Call) =>
                (itemLocal, itemAssign, sink)
  ...
  "foreach loops over collections" should {
      ...
      "create a local for the iterator as a child of the FOR block" in {
          val iterLocal = cpg.method.name("foo").body.astChildren.l match
            ...
            case result =>
                fail(s"Expected iterLocal as first of 3 foo body children but got $result")
      ...
      "create an item local and assignment in the body of the FOR loop" in {
          ...
          val (itemLocal, itemAssign, sinkCall) = body.astChildren.l match
            case List(itemLocal: Local, itemAssign: Call, sinkCall: Call) =>
                (itemLocal, itemAssign, sinkCall)
...
class ControlStructureTests extends JavaSrcCode2CpgFixture:
  ...
  "should identify an else block" in {
      ...
      val List(condition: Identifier, thenBlock: Block, elseBlock: ControlStructure) =
          ifBlock.astChildren.l: @unchecked
