...
class InferenceJarTests extends AnyFreeSpec with Matchers:
  ...
  "CPG for code where inference jar for dependencies is provided" - {
      val inferenceJarPath =
          ProjectRoot.relativise("platform/frontends/javasrc2cpg/src/test/resources/Deps.jar")
      lazy val cpg =
          JavaSrc2CpgTestContext.buildCpg(code, inferenceJarPaths = Set(inferenceJarPath))
