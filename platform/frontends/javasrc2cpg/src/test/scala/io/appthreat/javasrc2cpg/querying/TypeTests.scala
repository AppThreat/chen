          ...
          call.argument.l match
            case List(_: Identifier, varargs: Call) =>
                ...
                varargs.argument.l match
                  ...
                  case result =>
                      fail(s"Expected array initializer args matching 2 items but got $result")

            case result =>
                fail(s"Expected identifier and arrayInitializer arguments but got $result")
      }
...
class TypeTests extends JavaSrcCode2CpgFixture:
  ...
  "should use correct type for super calls in the constructor of foo.Foo" in {
      val List(call) = cpg.call.name(io.appthreat.x2cpg.Defines.ConstructorMethodName).l
      call.methodFullName shouldBe s"java.lang.Object.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void()"
