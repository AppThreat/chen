...
class ConfigFileCreationPassTests extends JavaSrcCode2CpgFixture:
  ...
  private val testConfigDir: String =
      ProjectRoot.relativise("platform/frontends/javasrc2cpg/src/test/resources/config_tests")

  "it should find the correct config files" in {
      val cpg = new Cpg()
      BatchedUpdate.applyDiff(
        cpg.graph,
        new DiffGraphBuilder().addNode(NewMetaData().root(testConfigDir)).build()
      )
...
end ConfigFileCreationPassTests
