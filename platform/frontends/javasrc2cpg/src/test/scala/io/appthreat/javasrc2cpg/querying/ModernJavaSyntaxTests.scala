...
class ModernJavaSyntaxTests extends JavaSrcCode2CpgFixture:
  ...
  "Switch expressions (Java 14)" should {
      ...
      // A switch expression lowers to a selector binding plus the value. Only the VALUE may take
      // an argument slot: the binding is a statement, and returning it alongside the value made
      // `sink(switch ...)` a two-argument call and `"x" + switch ...` a three-operand addition.
      "keep a switch expression in argument position a single argument" in {
          val cpg = code(
            """
            |class ArgPos {
            |  static void sink(String s) {}
            |  void run(int k) {
            |    sink(switch (k) { case 1 -> "one"; default -> "other"; });
            |  }
            |}
            |""".stripMargin,
            "ArgPos.java"
          )
          val sink = cpg.method.name("run").call.name("sink").head
          sink.argument.size shouldBe 1
          sink.argument.head.code should startWith("switch")

          // The selector binding runs as a statement before the call, not inside it.
          val blockChildren = cpg.method.name("run").head.block.astChildren.l
          blockChildren.collect { case c: Call => c.code }.head should startWith("switch$")
      }

      "keep a switch expression as a binary operand a single operand" in {
          val cpg = code(
            """
            |class BinPos {
            |  String run(int k) {
            |    return "x" + switch (k) { case 1 -> "one"; default -> "other"; };
            |  }
            |}
            |""".stripMargin,
            "BinPos.java"
          )
          cpg.method.name("run").call.nameExact("<operator>.addition").head.argument.size shouldBe 2
      }

      "keep a switch expression in an expression-bodied lambda a single return value" in {
          val cpg = code(
            """
            |import java.util.function.IntFunction;
            |class LambdaPos {
            |  IntFunction<String> f = k -> switch (k) { case 1 -> "one"; default -> "other"; };
            |}
            |""".stripMargin,
            "LambdaPos.java"
          )
          // The selector binding is a sibling statement of the return, not a second RETURN child.
          cpg.method.fullName(".*lambda.*").ast.isReturn.l.foreach { ret =>
              ret.astChildren.size shouldBe 1
          }
      }

      "evaluate a switch selector exactly once" in {
          val cpg = code(
            """
            |class SelOnce {
          ...
          cpg.method.name("run").call.name("compute").size shouldBe 1
      }
      ...
      "evaluate the selector exactly once, however many arms test it" in {
          val cpg = code(
            """
            |class SelectorOnce {
            |  String compute() { return "a"; }
            |
            |  String run() {
            |    return switch (compute()) {
            |      case "a" -> "1";
          ...
          // Java evaluates the selector once; duplicating it per arm would multiply the call.
          cpg.method.name("run").head.call.name("compute").l.size shouldBe 1
          // ... and the once-bound selector local exists for the arms to read.
          cpg.method.name("run").head.local.name("switch\\$\\d").l should not be empty
      }
