...
class NewConstructorInvocationTests extends JavaSrcCode2CpgFixture:
  "constructor init method call" should {
      ...
      "have correct methodFullName and signature" in {
          val initCall = cpg.call.nameExact(io.appthreat.x2cpg.Defines.ConstructorMethodName).head
          initCall.signature shouldBe "void(long)"
          initCall.methodFullName shouldBe s"Foo.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(long)"
      }
  }
  ...
  "a simple single argument constructor" should {
      ...
      "create the correct Ast for the constructor" in {
          fooCpg.typeDecl.name("Foo").method.nameExact(
            io.appthreat.x2cpg.Defines.ConstructorMethodName
          ).l match
            case List(cons: Method) =>
                cons.fullName shouldBe s"Foo.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int)"
...
end NewConstructorInvocationTests
...
class ConstructorInvocationTests extends JavaSrcCode2CpgFixture:
  ...
  "it should create correct method nodes for constructors" in {
      ...
      cpg.typeDecl.name("Bar").method.nameExact(io.appthreat.x2cpg.Defines.ConstructorMethodName).l match
        case List(cons1: Method, cons2: Method) =>
            cons1.fullName shouldBe s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int)"
            ...
            cons2.fullName shouldBe s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int,int)"
  ...
  "it should create joint `alloc` and `init` calls for a constructor invocation in a vardecl" in {
      cpg.typeDecl.name("Bar").method.name("test1").l match
        case List(method) =>
            val List(_: Local, assign: Call, init: Call) =
            ...
            init.methodFullName shouldBe s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int,int)"
            init.callOut.head
                .fullName shouldBe s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int,int)"
            ...
            val List(obj: Identifier, initArg1: Literal, initArg2: Literal) =
                init.argument.l: @unchecked
  ...
  "it should create joint `alloc` and `init` calls for a constructor invocation in an assignment" in {
      cpg.typeDecl.name("Bar").method.name("test2").l match
        case List(method) =>
            val List(assign: Call, init: Call) =
                method.astChildren.isBlock.astChildren.l: @unchecked
  ...
  "it should create `alloc` and `init` calls in a block for constructor invocations not in assignments" in {
      cpg.typeDecl.name("Bar").method.name("bar").l match
        case List(method) =>
            ...
            val List(assign: Call, init: Call, ret: Identifier) =
                consBlock.astChildren.l: @unchecked
            ...
            init.name shouldBe io.appthreat.x2cpg.Defines.ConstructorMethodName
            init.methodFullName shouldBe s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int)"
            init.callOut.head
                .fullName shouldBe s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int)"
  ...
  "it should create only `init` call for direct invocation using `this`" in {
      cpg.typeDecl
          .name("Bar")
          .method
          .fullNameExact(s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int,int)")
          .l match
        case List(method) =>
  ...
  "it should create only `init` call for direct invocation using `super`" in {
      cpg.typeDecl
          .name("Bar")
          .method
          .fullNameExact(s"Bar.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int)")
          .l match
        case List(method) =>
            val List(init: Call) = method.astChildren.isBlock.astChildren.l: @unchecked
            init.name shouldBe io.appthreat.x2cpg.Defines.ConstructorMethodName
            init.methodFullName shouldBe s"Foo.${io.appthreat.x2cpg.Defines.ConstructorMethodName}:void(int)"
            init.dispatchType shouldBe DispatchTypes.STATIC_DISPATCH.toString
            init.typeFullName shouldBe "void"
            init.signature shouldBe "void(int)"
...
end ConstructorInvocationTests
