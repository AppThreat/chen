...
class FieldAccessTests extends JimpleCode2CpgFixture:
  ...
  "should handle object field accesses on RHS of assignments" in {
      val assigns = cpg.method(".*bar.*").call(".*assignment").codeExact("y = f.value").l
      assigns.size shouldBe 1
      val assign = assigns.head
  ...
  "should handle object field accesses on LHS of assignments" in {
      val assigns = cpg.method(".*baz.*").call(".*assignment").codeExact("g.value = 66").l
      assigns.size shouldBe 1
      val assign = assigns.head
  ...
  "it should reflect field access within constructor logic correctly" in {
      ...
      val instanceFieldAccesses =
          constructorMethod.call.name("<operator>.fieldAccess").filter { fa =>
      ...
      val maxValueAccessesInSetValue =
          setValueMethod.call.name("<operator>.fieldAccess").filter { fa =>
              fa.argument.order(1).headOption.exists(_.code == "Foo")
          }.l
