...
object ProgramHandlingUtil:
  ...
  sealed class Entry(entry: Either[Path, ZipEntry], parentArchive: Option[ZipFile] = None):
    ...
    private def isValidZipFile(zis: ZipInputStream): Boolean =
        Try(zis.getNextEntry != null).getOrElse(false)

    /** Determines if the given file is a valid and uncorrupted zip file by reading through the
      * whole archive until the end.
      * @param f
      *   the file to read.
      * @return
    ...
    def isConfigFile: Boolean =
      ...
      def hasConfigExt(f: Path): Boolean =
          configExt.exists(f.extension.map(_.toLowerCase).contains(_))

      if isDirectory then false
      else
        entry match
  ...
  /** Process files that may lead to more files to process or to emit a resulting value of [[A]]
    ...
    * @param emitOrUnpack
    *   A function that takes a file and either emits a value or returns more files to traverse. The
    *   key of Right was used to identify whether the file was unfold from an archive
    * @param maxDepth
  ...
  private def extractClassesToTmp(
    ...
    def shouldExtract(e: Entry) =
        !e.isZipSlip && e.maybeRegularFile() && ((isArchive(e) && recurse) || isClass(
          e
        ) || isConfigFile(e))
    ...
    unfoldArchives(
      src,
      {
          ...
          case f if Files.isDirectory(f) =>
              var files = f.walk().filterNot(_ == src).filterNot(Files.isDirectory(_))
              if onlyClasses then
                files = files.filter(_.endsWith(".class"))
              Right(Map(false -> files.toList))
          case f
              if isArchive(Entry(f)) && (f == src || (Files.isDirectory(src) && subOfSrc.contains(
                f
              )) || recurse) =>
