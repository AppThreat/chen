...
class AstCreator(filename: String, cls: SootClass, global: Global)(implicit
  withSchemaValidation: ValidationMode
    ...
    /** Add `typeName` to a global map and return it. The map is later passed to a pass that creates
      * TYPE nodes for each key in the map.
      */
    ...
    /** Creates a list of all inherited classes and implemented interfaces. If there are none then a
      * list with a single element 'java.lang.Object' is returned by default. Returns two lists in
      * the form of (List[Super Classes], List[Interfaces]).
      */
    ...
    /** Creates the AST root for type declarations and acts as the entry point for method
      * generation.
      */
    private def astForTypeDecl(typ: RefType, namespaceBlockFullName: String): Ast =
        ...
        if clz.isPublic then code.append("public ")
        else if clz.isPrivate then code.append("private ")
        if clz.isStatic then code.append("static ")
        if clz.isFinal then code.append("final ")
        if clz.isInterface then code.append("interface ")
        else if clz.isAbstract then code.append("abstract ")
        if clz.isEnum then code.append("enum ")
        if !clz.isInterface then code.append(s"class $shortName")
        ...
        if inherited.nonEmpty then code.append(s" extends ${inherited.mkString(", ")}")
        if implemented.nonEmpty then code.append(s" implements ${implemented.mkString(", ")}")
        ...
        val methodAsts = withOrder(typ.getSootClass.getMethods.asScala.toList.sortWith((x, y) =>
            x.getName > y.getName
        )) {
            (m, order) =>
    ...
    private def astForField(field: SootField, order: Int): Ast =
        ...
        Ast(
          NewMember()
              ...
              .order(order)
              .code(code)
        ).withChildren(withOrder(annotations) { (a, aOrder) =>
            astsForAnnotations(a, aOrder, field)
        })
    end astForField
    ...
    private def astForMethod(methodDeclaration: SootMethod, typeDecl: RefType, childNum: Int): Ast =
        ...
        try
            if !methodDeclaration.isConcrete then
                ...
                val parameterAsts =
                    Seq(createThisNode(methodDeclaration, NewMethodParameterIn())) ++
                        withOrder(locals) { (p, order) =>
                            astForParameter(p, order, methodDeclaration, Map())
                        }
                Ast(methodNode)
                ...
                val paramAnnos =
                    mTags.collect { case x: VisibilityParameterAnnotationTag => x }.flatMap(
                      _.getVisibilityAnnotations.asScala
                    )
                val paramNames =
                    mTags.collect { case x: ParamNamesTag => x }.flatMap(_.getNames.asScala)
                ...
                val parameterAsts =
                    Seq(createThisNode(methodDeclaration, NewMethodParameterIn())) ++ withOrder(
                      methodBody.getParameterLocals
                        ...
                        (p, order) =>
                            astForParameter(p, order, methodDeclaration, parameterAnnotations)
                    }
                Ast(
                  methodNode
            ...
            case e: RuntimeException =>
                ...
                val nonJavaLibs =
                    cls.getInterfaces.asScala.map(_.getPackageName).filter(JVM_LANGS.contains).toSet
                if nonJavaLibs.nonEmpty || cls.getMethods.asScala.exists(_.getName.endsWith("$"))
                then
                    val errMsg =
                        "The bytecode for this method suggests it is built with a non-Java JVM language. " +
                    ...
                    logger.warn(
                      if nonJavaLibs.nonEmpty then
                          s"$errMsg. Language(s) detected: ${nonJavaLibs.mkString(",")}."
                      else errMsg
                    )
                else
                    logger.warn(
    ...
    private def astForParameter(
      ...
      parameterAnnotations: Map[String, VisibilityAnnotationTag]
        ...
        parameterAnnotations.get(parameter.getName) match
            case Some(annoRoot) =>
                parameterNode.withChildren(withOrder(annoRoot.getAnnotations.asScala) {
                    (a, order) =>
    ...
    private def astsForHostTags(host: AbstractHost): Seq[Ast] =
        host.getTags.asScala
            .collect { case x: VisibilityAnnotationTag => x }
            .flatMap { x =>
                withOrder(x.getAnnotations.asScala) { (a, order) =>
                    astsForAnnotations(a, order, host)
                }
            }
            .toSeq
    ...
    private def astsForAnnotations(annotation: AnnotationTag, order: Int, host: AbstractHost): Ast =
        ...
        val annotationNode = NewAnnotation()
            .name(name)
            .code(
              s"@$name(${elementNodes.flatMap(_.root).flatMap(_.properties.get(PropertyNames.CODE)).mkString(", ")})"
            )
            .fullName(annoType)
            .order(order)
        Ast(annotationNode)
    ...
    private def astForAnnotationElement(
      annoElement: AnnotationElem,
      order: Int,
      parent: AbstractHost
    ): Ast =
        def getLiteralElementNameAndCode(annoElement: AnnotationElem): (String, String) =
            annoElement match
        ...
        if annoElement.getName != null then
            astChildren.append(
              Ast(
        ...
        astChildren.append(annoElement match
            case x: AnnotationAnnotationElem =>
                ...
                codeBuilder.append(
                  s"${rhsAst.root.flatMap(_.properties.get(PropertyNames.CODE)).mkString(", ")}"
                )
                rhsAst
            ...
            case x =>
                ...
                val rhsOrder = if annoElement.getName == null then order else astChildren.size + 1
                codeBuilder.append(code)
                Ast(NewAnnotationLiteral().name(name).code(code).order(rhsOrder).argumentIndex(
                  rhsOrder
                ))
        )

        if astChildren.size == 1 then
            astChildren.head
        else
    ...
    private def astForAnnotationArrayElement(
      x: AnnotationArrayElem,
      order: Int,
      parent: AbstractHost
    ): (Ast, String) =
        val elems = withOrder(x.getValues.asScala) { (elem, order) =>
            astForAnnotationElement(elem, order, parent)
        }
        val code =
            s"{${elems.flatMap(_.root).flatMap(_.properties.get(PropertyNames.CODE)).mkString(", ")}}"
      ...
      parentUnit: soot.Unit
        ...
        def astForTypeRef(t: String, order: Int) =
            Seq(
              Ast(
                NewTypeRef()
                    .code(if t.contains('.') then t.substring(t.lastIndexOf('.') + 1, t.length)
                    else t)
                    .order(order)
                    .argumentIndex(order)
                    .lineNumber(line(parentUnit))
    ...
    private def createThisNode(method: SootMethod, builder: NewNode): Ast =
        createThisNode(method.makeRef(), builder)
    ...
    /** Creates the AST for assignment statements keeping in mind Jimple is a 3-address code
      * language.
      */
    ...
    end astForSwitchWithDefaultAndCondition
    ...
    private def astsForReturnNode(returnStmt: ReturnStmt, order: Int): Seq[Ast] =
        ...
        val returnNode = NewReturn()
            ...
            .order(order)
            .code(
              s"return ${astChildren.flatMap(_.root).map(_.properties(PropertyNames.CODE)).mkString(" ")};"
            )
    ...
    private def astsForModifiers(methodDeclaration: SootMethod): Seq[Ast] =
        Seq(
          if methodDeclaration.isStatic then Some(ModifierTypes.STATIC) else None,
          if methodDeclaration.isPublic then Some(ModifierTypes.PUBLIC) else None,
          if methodDeclaration.isProtected then Some(ModifierTypes.PROTECTED) else None,
          if methodDeclaration.isPrivate then Some(ModifierTypes.PRIVATE) else None,
          if methodDeclaration.isAbstract then Some(ModifierTypes.ABSTRACT) else None,
          if methodDeclaration.isConstructor then Some(ModifierTypes.CONSTRUCTOR) else None,
          if !methodDeclaration.isFinal && !methodDeclaration.isStatic && methodDeclaration.isPublic
          then
              Some(ModifierTypes.VIRTUAL)
          else None,
          if methodDeclaration.isSynchronized then Some("SYNCHRONIZED") else None
    ...
    private def astsForModifiers(classDeclaration: SootClass): Seq[Ast] =
        Seq(
          if classDeclaration.isStatic then Some(ModifierTypes.STATIC) else None,
          if classDeclaration.isPublic then Some(ModifierTypes.PUBLIC) else None,
          if classDeclaration.isProtected then Some(ModifierTypes.PROTECTED) else None,
          if classDeclaration.isPrivate then Some(ModifierTypes.PRIVATE) else None,
          if classDeclaration.isAbstract then Some(ModifierTypes.ABSTRACT) else None,
          if classDeclaration.isInterface then Some("INTERFACE") else None,
          if !classDeclaration.isFinal && !classDeclaration.isStatic && classDeclaration.isPublic
          then
              Some(ModifierTypes.VIRTUAL)
          else None,
          if classDeclaration.isSynchronized then Some("SYNCHRONIZED") else None
        ).flatten.map { modifier =>
            Ast(NewModifier().modifierType(modifier).code(modifier.toLowerCase))
        }
...
object AstCreator:
    def line(node: Host): Option[Integer] =
        ...
        else if node.getJavaSourceStartLineNumber == -1 then None
        else Option(node.getJavaSourceStartLineNumber)

    def column(node: Host): Option[Integer] =
        if node == null then None
        else if node.getJavaSourceStartColumnNumber == -1 then None
