package io.appthreat.jssrc2cpg.passes.cfg

import io.appthreat.jssrc2cpg.testfixtures.JsCfgTestCpg
...
import io.appthreat.x2cpg.testfixtures.CfgTestFixture
...
class SimpleCfgCreationPassTest extends CfgTestFixture(() => new JsCfgTestCpg()):
  ...
  "CFG generation for simple fragments" should {
      "have correct structure for block expression" in {
          ...
          succOf("class Foo {}, bar()") shouldBe expected((
            "let x = (class Foo {}, bar())",
            AlwaysEdge
          ))
      ...
      "have correct structure for untagged runtime node in call" in {
          ...
          succOf("\"!\"") shouldBe expected((
            s"${Operators.formatString}(\"Hello \", world, \"!\")",
            AlwaysEdge
          ))
      ...
      "have correct structure for untagged runtime node" in {
          ...
          succOf("\"\"", 1) shouldBe expected((
            s"${Operators.formatString}(\"\", x + 1, \"\")",
            AlwaysEdge
          ))
          succOf(s"${Operators.formatString}(\"\", x + 1, \"\")") shouldBe expected((
            "RET",
            AlwaysEdge
          ))
      }
      ...
      "be correct for outer program function which declares foo function object" in {
          ...
          succOf("function foo = function foo(x, y) { return; }") shouldBe expected((
            "RET",
            AlwaysEdge
          ))
      }
      ...
      "be correct for labeled expressions with continue" in {
          ...
          succOf("i === 1 && j === 1") shouldBe expected(
      ...
      "be correct for nested while/do-while loops with break" in {
          implicit val cpg: Cpg =
              code("while (x) { do { while(y) { break; a; } z; } while (x < 1); } c;")
          succOf(":program") shouldBe expected(("x", AlwaysEdge))
          succOf("x") shouldBe expected(("y", TrueEdge), ("c", FalseEdge))
          succOf("y") shouldBe expected(("break;", TrueEdge), ("z", FalseEdge))
      ...
      "be correct for constructor call with new" in {
          ...
          succOf("new MyClass(arg1, arg2)", NodeTypes.CALL) shouldBe expected((
            "_tmp_0",
            2,
            AlwaysEdge
          ))
          succOf("_tmp_0", 2) shouldBe expected(("new MyClass(arg1, arg2)", AlwaysEdge))
          succOf("new MyClass(arg1, arg2)") shouldBe expected((
            "var x = new MyClass(arg1, arg2)",
            AlwaysEdge
          ))
  ...
  private def testForInOrOf()(implicit cpg: Cpg): Unit =
    ...
    succOf("<operator>.iterator(arr)") shouldBe expected((
      "_iterator_0 = <operator>.iterator(arr)",
      AlwaysEdge
    ))
