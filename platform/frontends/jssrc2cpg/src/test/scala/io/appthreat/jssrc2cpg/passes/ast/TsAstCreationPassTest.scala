...
class TsAstCreationPassTest extends AbstractPassTest:
  ...
  "AST generation for simple TS constructs" should {
      ...
      "have correct structure for type assertion" in TsAstFixture("let emptyArray = <VNode[]>[];") {
          cpg =>
              cpg.assignment.code.l shouldBe List("let emptyArray = <VNode[]>[]")
      }

      "have correct structure for satisfies expressions" in TsAstFixture("let x = y satisfies T;") {
          cpg =>
  ...
  private def checkForInOrOf(node: Block): Unit =
    ...
    val List(loopVarAssignmentCall) =
        whileLoopBlock.astChildren.isCall.codeExact("foo().x = _result_0.value").l
