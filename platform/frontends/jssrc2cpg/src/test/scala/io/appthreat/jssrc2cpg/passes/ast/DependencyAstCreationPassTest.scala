...
import io.appthreat.x2cpg.layers.Base
...
class DependencyAstCreationPassTest extends AbstractPassTest:
  ...
  "AST generation for dependencies" should {
      ...
      "have correct locals and require calls for imports" in AstFixture(
  ...
  "AST generation for exports" should {
      ...
      "have correct structure for export all with from clause" in AstFixture("""
       ...
       |""".stripMargin) { cpg =>
        ...
        val List(dep4ReqAssignment) =
            cpg.assignment.codeExact("var _ModuleB = require(\"./some/ModuleB\")").l
...
end DependencyAstCreationPassTest
