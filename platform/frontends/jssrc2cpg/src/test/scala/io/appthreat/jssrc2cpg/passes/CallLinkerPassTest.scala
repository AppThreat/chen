...
class CallLinkerPassTest extends DataFlowCodeToCpgSuite:
  ...
  "CallLinkerPass" should {
      ...
      "link exported anonymous functions across file boundaries correctly" in {
          ...
          inside(cpg.method.fullNameExact("bar.js::program:anonymous").callIn(using NoResolve).l) {
              case List(call) =>
          ...
          inside(cpg.method.fullNameExact("baz.js::program:anonymous").callIn(using NoResolve).l) {
              case List(call) =>
