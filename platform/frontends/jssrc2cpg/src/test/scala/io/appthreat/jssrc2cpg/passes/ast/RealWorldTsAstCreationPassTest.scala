...
class RealWorldTsAstCreationPassTest extends AbstractPassTest:
  ...
  "AST generation for a real NestJS decorated controller (Babel 8)" should {
      ...
      "create the controller type with its methods" in AstJsonFixture("babel8-ts-decorators") {
          cpg =>
            cpg.typeDecl.nameExact("CatsController").size shouldBe 1
            (cpg.typeDecl("CatsController").method.name.l should contain).allOf(
              "create",
              "findAll",
              "findOne"
            )
      ...
      "attach the class-level decorators" in AstJsonFixture("babel8-ts-decorators") { cpg =>
          (cpg.typeDecl("CatsController").astChildren.collectAll[Annotation].name
      ...
      "attach method-level decorators to the right methods" in AstJsonFixture(
        "babel8-ts-decorators"
      ) { cpg =>
      ...
      "model the injected constructor parameter" in AstJsonFixture("babel8-ts-decorators") { cpg =>
          cpg.typeDecl("CatsController").method.isConstructor.parameter.name.l should contain(
            "catsService"
          )
      }
  }
...
end RealWorldTsAstCreationPassTest
