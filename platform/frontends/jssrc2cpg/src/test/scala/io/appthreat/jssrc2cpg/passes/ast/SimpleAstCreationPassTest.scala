...
import io.appthreat.jssrc2cpg.passes.{AbstractPassTest, Defines, EcmaBuiltins}
...
class SimpleAstCreationPassTest extends AbstractPassTest:
  ...
  "AST generation for simple fragments" should {
      ...
      "have only 1 Block Node for arrow functions" in AstFixture("const foo = () => {return 42;}") {
          cpg =>
      ...
      "have correct type for literals" in AstFixture("let x = 1; let y = 'y'; let z = false;") {
          cpg =>
      ...
      "have correct structure for multiple declarators in one place" in AstFixture(
        "let x = 1, y = 2, z = 3;"
      ) { cpg =>
      ...
      "have correct structure for call on require" in AstFixture("var x = require(\"foo\").bar;") {
          cpg =>
      ...
      "have correct structure for block expression" in AstFixture("let x = (class Foo {}, bar())") {
          cpg =>
            ...
            val List(classFooMethod) =
                classFooTypeDecl.astChildren.isMethod.nameExact(
      ...
      "have correct structure for index access" in AstFixture("if(d = decorators[i]) foo();") {
          cpg =>
      ...
      "have correct structure for array literal with values" in AstFixture("var x = [1, 2]") {
          cpg =>
            ...
            val List(tmpAssignment) =
                pushBlock.astChildren.isCall.codeExact("_tmp_0 = __ecma.Array.factory()").l
      ...
      "be correct for lambdas returning lambdas" in AstFixture("() => async () => { }") { cpg =>
        ...
        val List(ret) =
            cpg.method.fullNameExact("code.js::program:anonymous").block.astChildren.isReturn.l
      ...
      "have correct structure for empty method with rest parameter" in AstFixture(
      ...
      "be correct for for-loop with for-of with object destruction" in AstFixture("""
      ...
      "handle switch statements and" should {
          ...
          "be correct for switch with default case" in AstFixture("switch (x) { default: y; }") {
              cpg =>
                val List(program)      = cpg.method.nameExact(":program").l
                val List(programBlock) = program.astChildren.isBlock.l
          ...
          "be correct for switch with case and default combined" in AstFixture(
            ...
            val List(break) =
                switchBlock.astChildren.isControlStructure.controlStructureTypeExact(
          ...
          "be correct for switch with nested switch" in AstFixture(
            "switch (x) { default: switch(y) { default: z; } }"
              ...
              cpg =>
                ...
                val List(topLevelSwitchExpr) =
                    topLevelSwitch.astChildren.isIdentifier.nameExact("x").l
                ...
                val List(topLevelCaseLabel) =
                    topLevelSwitchBlock._jumpTargetViaAstOut.codeExact("default:").l
                ...
                val List(nestedCaseLabel) =
                    nestedSwitchBlock._jumpTargetViaAstOut.codeExact("default:").l
  ...
  private def checkForInOrOfObject(node: Block): Unit =
    ...
    val List(loopTestCall) =
        loop.astChildren.isCall.codeExact("!(_result_0 = _iterator_0.next()).done").l
    loopTestCall.name shouldBe Operators.not
    loopTestCall.order shouldBe 1

    val List(doneMaCall) =
        loopTestCall.astChildren.isCall.codeExact("(_result_0 = _iterator_0.next()).done").l
    doneMaCall.name shouldBe Operators.fieldAccess

    val List(doneMaBase) =
        doneMaCall.astChildren.isCall.codeExact("(_result_0 = _iterator_0.next())").l
    ...
    val List(loopVarAssignmentCallA) =
        whileLoopBlock.astChildren.isCall.codeExact("a = _result_0.value.a").l
    loopVarAssignmentCallA.name shouldBe Operators.assignment
    loopVarAssignmentCallA.order shouldBe 1
    val List(loopVarAssignmentCallB) =
        whileLoopBlock.astChildren.isCall.codeExact("b = _result_0.value.b").l
    loopVarAssignmentCallB.name shouldBe Operators.assignment
    loopVarAssignmentCallB.order shouldBe 2
    val List(loopVarAssignmentCallC) =
        whileLoopBlock.astChildren.isCall.codeExact("c = _result_0.value.c").l
    loopVarAssignmentCallC.name shouldBe Operators.assignment
    loopVarAssignmentCallC.order shouldBe 3

    val List(fooCall) =
        whileLoopBlock.astChildren.isBlock.astChildren.isCall.codeExact("foo(a, b, c)").l
    fooCall.name shouldBe "foo"
  end checkForInOrOfObject
  ...
  private def checkForInOrOfArray(node: Block): Unit =
    ...
    val List(loopVarAssignmentCallA) =
        whileLoopBlock.astChildren.isCall.codeExact("a = _result_0.value[0]").l
    loopVarAssignmentCallA.name shouldBe Operators.assignment
    loopVarAssignmentCallA.order shouldBe 1
    val List(loopVarAssignmentCallB) =
        whileLoopBlock.astChildren.isCall.codeExact("b = _result_0.value[1]").l
    loopVarAssignmentCallB.name shouldBe Operators.assignment
    loopVarAssignmentCallB.order shouldBe 2
    val List(loopVarAssignmentCallC) =
        whileLoopBlock.astChildren.isCall.codeExact("c = _result_0.value[2]").l
    loopVarAssignmentCallC.name shouldBe Operators.assignment
    loopVarAssignmentCallC.order shouldBe 3
  ...
  private def checkForInOrOf(node: Block): Unit =
    ...
    val List(loopVarAssignmentCall) =
        whileLoopBlock.astChildren.isCall.codeExact("i = _result_0.value").l
