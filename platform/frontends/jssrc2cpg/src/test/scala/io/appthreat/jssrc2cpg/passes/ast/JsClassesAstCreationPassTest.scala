...
class JsClassesAstCreationPassTest extends AbstractPassTest {
  ...
  "AST generation for JS classes" should {
    ...
    "have member with initialization in TYPE_DECL for ClassA" in AstFixture("""
      ...
      val List(clInitMethod)         = classATypeDecl.method.nameExact(io.appthreat.x2cpg.Defines.StaticInitMethodName).l
      ...
      val List(constructor) =
        cpg.typeDecl.nameExact("ClassA").method.nameExact(io.appthreat.x2cpg.Defines.ConstructorMethodName).l
