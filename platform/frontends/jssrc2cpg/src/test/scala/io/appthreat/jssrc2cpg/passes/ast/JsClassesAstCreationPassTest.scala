...
class JsClassesAstCreationPassTest extends AbstractPassTest:
  ...
  "AST generation for JS classes" should {
      ...
      "have a synthetic assignment for ClassA" in AstFixture("class ClassA {}") { cpg =>
        ...
        inside(cpg.assignment.argument.l) { case List(id: Identifier, constructorRef: MethodRef) =>
            ...
            id.dynamicTypeHintFullName shouldBe List(
              s"code.js::program:ClassA:${Defines.ConstructorMethodName}"
            )
            constructorRef.code shouldBe "constructor() {}"
            constructorRef
                .typeFullName shouldBe s"code.js::program:ClassA:${Defines.ConstructorMethodName}"
            constructorRef
                .methodFullName shouldBe s"code.js::program:ClassA:${Defines.ConstructorMethodName}"
        }
      }
      ...
      "have a member function for static method in TYPE_DECL for ClassA" in AstFixture("""
       ...
       |}""".stripMargin) { cpg =>
        ...
        constructor.fullName shouldBe s"code.js::program:ClassA:${io.appthreat.x2cpg.Defines.ConstructorMethodName}"
      ...
      "have member with initialization in TYPE_DECL for ClassA" in AstFixture("""
        ...
        val List(clInitMethod) =
            classATypeDecl.method.nameExact(io.appthreat.x2cpg.Defines.StaticInitMethodName).l
        ...
        val List(constructor) =
            cpg.typeDecl.nameExact("ClassA").method.nameExact(
  ...
  "AST generation for constructor" should {
      ...
      "have correct structure for simple new with arguments" in AstFixture(
        ...
        val List(program)      = cpg.method.nameExact(":program").l
        val List(programBlock) = program.astChildren.isBlock.l
        val List(newCallBlock) =
            programBlock.astChildren.isBlock.codeExact("new MyClass(arg1, arg2)").l
        val tmpName = "_tmp_0"
        ...
        val List(constructorCall) =
            newCallBlock.astChildren.isCall.codeExact("new MyClass(arg1, arg2)").l
        constructorCall.astChildren.isIdentifier.nameExact("MyClass").size shouldBe 1
      ...
      "have correct structure for new with access path" in AstFixture("new foo.bar.MyClass();") {
          cpg =>
            ...
            val List(programBlock) = program.astChildren.isBlock.l
            val List(newCallBlock) =
                programBlock.astChildren.isBlock.codeExact("new foo.bar.MyClass()").l
            val tmpName = "_tmp_0"
            ...
            val List(tmpAssignment) =
                newCallBlock.astChildren.isCall.codeExact(s"$tmpName = .alloc").l
            tmpAssignment.name shouldBe Operators.assignment
            ...
            val List(constructorCall) =
                newCallBlock.astChildren.isCall.codeExact("new foo.bar.MyClass()").l
            constructorCall.name shouldBe "<operator>.new"
...
end JsClassesAstCreationPassTest
