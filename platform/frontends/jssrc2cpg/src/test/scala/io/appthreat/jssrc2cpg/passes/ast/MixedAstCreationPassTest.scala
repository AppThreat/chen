...
class MixedAstCreationPassTest extends AbstractPassTest:
  ...
  "AST method full names" should {
      ...
      "anonymous constructor full name 1" in AstFixture("class X { constructor(){} }") { cpg =>
          cpg.method.fullName.toSetMutable should contain(
            s"code.js::program:X:${io.appthreat.x2cpg.Defines.ConstructorMethodName}"
          )
      }
      "anonymous constructor of anonymous class full name" in AstFixture(
        "var x = class { constructor(y) {} };"
      ) { cpg =>
          cpg.method.fullName.toSetMutable should contain(
            s"code.js::program:_anon_cdecl:${io.appthreat.x2cpg.Defines.ConstructorMethodName}"
          )
      }
  }
  ...
  "AST variable scoping and linking" should {
      ...
      "have correct closure binding (destructing parameter)" in AstFixture("""
        ...
        |};
        """.stripMargin) { cpg =>
          cpg.local.name("value").closureBindingId.l shouldBe List(
            "code.js::program:anonymous:anonymous:value"
          )
      }
      ...
      "have correct closure binding (destructing assignment)" in AstFixture("""
        ...
        |}
        """.stripMargin) { cpg =>
        cpg.local.name("closureA").closureBindingId.l shouldBe List(
          "code.js::program:anonymous:closureA"
        )
        cpg.local.name("closureB").closureBindingId.l shouldBe List(
          "code.js::program:anonymous:closureB"
        )
      }
      ...
      "have correct closure binding (single variable)" in AstFixture("""
        ...
        val List(identifierX) =
            barMethodBlock.astChildren.isCall.astChildren.isIdentifier.nameExact("x").l
      ...
      "have correct closure binding (two variables)" in AstFixture("""
        ...
        val List(identifierY) =
            barMethodBlock.astChildren.isCall.astChildren.isIdentifier.nameExact("y").l
      ...
      "have correct closure binding for capturing over 2 levels" in AstFixture("""
        ...
        val List(bazIdentifierX) =
            bazMethodBlock.astChildren.isCall.astChildren.isIdentifier.nameExact("x").l
      ...
      "have correct closure binding for capturing over 2 levels with intermediate blocks" in AstFixture(
        ...
        val List(bazIdentifierX) =
            bazMethodInnerBlock.astChildren.isCall.astChildren.isIdentifier.nameExact("x").l
        bazIdentifierX.refOut.head shouldBe bazLocalX
      }
      ...
      "have correct closure binding for capturing the same variable into 2 different anonymous methods" in AstFixture(
        ...
        val List(anon1Ref) =
            fooBlock.astChildren.isCall.astChildren.isMethodRef.methodFullNameExact(
              "code.js::program:foo:anonymous"
            ).l
        ...
        val List(anon2Ref) =
            fooBlock.astChildren.isCall.astChildren.isMethodRef.methodFullNameExact(
              "code.js::program:foo:anonymous1"
            ).l
  ...
  "AST generation for destructing assignment" should {
        ...
        "var {a, b} = x;"
          ...
          cpg =>
            ...
            val List(assignmentToB) =
                destructionBlock.astChildren.isCall.codeExact("b = _tmp_0.b").l
      ...
      "have correct structure for object destruction assignment with defaults" in AstFixture(
        "var {a = 1, b = 2} = x;"
          ...
          cpg =>
            ...
            val List(assignmentToA) =
                destructionBlock.assignment.codeExact("a = _tmp_0.a === void 0 ? 1 : _tmp_0.a").l
            assignmentToA.astChildren.isIdentifier.size shouldBe 1

            val List(ifA) =
                assignmentToA.astChildren.isCall.codeExact("_tmp_0.a === void 0 ? 1 : _tmp_0.a").l
            ...
            val List(assignmentToB) =
                destructionBlock.assignment.codeExact("b = _tmp_0.b === void 0 ? 2 : _tmp_0.b").l
            assignmentToB.astChildren.isIdentifier.size shouldBe 1

            val List(ifB) =
                assignmentToB.astChildren.isCall.codeExact("_tmp_0.b === void 0 ? 2 : _tmp_0.b").l
      ...
      "have correct structure for object destruction assignment with reassignment and defaults" in AstFixture(
        ...
        val List(assignmentToN) =
            destructionBlock.assignment.codeExact("n = _tmp_0.a === void 0 ? 1 : _tmp_0.a").l
        assignmentToN.astChildren.isIdentifier.size shouldBe 1

        val List(ifA) =
            assignmentToN.astChildren.isCall.codeExact("_tmp_0.a === void 0 ? 1 : _tmp_0.a").l
        ifA.name shouldBe Operators.conditional
        ...
        val List(assignmentToM) =
            destructionBlock.assignment.codeExact("m = _tmp_0.b === void 0 ? 2 : _tmp_0.b").l
        assignmentToN.astChildren.isIdentifier.size shouldBe 1

        val List(ifB) =
            assignmentToM.astChildren.isCall.codeExact("_tmp_0.b === void 0 ? 2 : _tmp_0.b").l
        ifB.name shouldBe Operators.conditional
      ...
      "have correct structure for object destruction assignment with computed property name" in AstFixture(
        ...
        val List(assignmentToN) =
            destructionBlock.astChildren.isCall.codeExact("n = _tmp_0.propName").l
        assignmentToN.astChildren.isIdentifier.size shouldBe 1
      ...
      "have correct structure for nested object destruction assignment with defaults as parameter" in AstFixture(
       ...
       |""".stripMargin
        ...
        destructionBlock.astChildren.isCall.codeExact(
          "_tmp_1 = param1_0 === void 0 ? {} : param1_0"
        ).size shouldBe 1

        val List(assignmentToId) =
            destructionBlock.astChildren.isCall.codeExact(
              "id = _tmp_1.id === void 0 ? {} : _tmp_1.id"
            ).l
        ...
        val List(ternaryId) =
            assignmentToId.astChildren.isCall.codeExact("_tmp_1.id === void 0 ? {} : _tmp_1.id").l
      ...
      "have correct structure for array destruction assignment with defaults" in AstFixture(
        "var [a = 1, b = 2] = x;"
          ...
          cpg =>
            ...
            val List(assignmentToA) =
                destructionBlock.assignment.codeExact("a = _tmp_0[0] === void 0 ? 1 : _tmp_0[0]").l
            assignmentToA.astChildren.isIdentifier.size shouldBe 1

            val List(ifA) =
                assignmentToA.astChildren.isCall.codeExact("_tmp_0[0] === void 0 ? 1 : _tmp_0[0]").l
            ifA.name shouldBe Operators.conditional
            ...
            val List(assignmentToB) =
                destructionBlock.assignment.codeExact("b = _tmp_0[1] === void 0 ? 2 : _tmp_0[1]").l
            assignmentToB.astChildren.isIdentifier.size shouldBe 1

            val List(ifB) =
                assignmentToB.astChildren.isCall.codeExact("_tmp_0[1] === void 0 ? 2 : _tmp_0[1]").l
            ifB.name shouldBe Operators.conditional
      ...
      "have correct structure for array destruction assignment with rest" in AstFixture(
        "var [a, ...rest] = x;"
      ) { cpg =>
        val List(program)      = cpg.method.nameExact(":program").l
        val List(programBlock) = program.astChildren.isBlock.l
      ...
      "have correct structure for complex method spread argument" in AstFixture(
        "foo(...x.bar());"
      ) { cpg =>
  ...
  "AST generation for await/async" should {
      "have correct structure for await/async" in AstFixture(
  ...
  "AST generation for instanceof/delete" should {
      "have correct structure for instanceof" in AstFixture("x instanceof Foo;") { cpg =>
        ...
        val List(instanceOf) =
            program.astChildren.isBlock.astChildren.isCall.codeExact("x instanceof Foo").l
      ...
      "have correct structure for delete" in AstFixture("delete foo.x;") { cpg =>
        ...
        val List(delete) =
            program.astChildren.isBlock.astChildren.isCall.codeExact("delete foo.x").l
      ...
      "have correct structure for method mixed parameters with default" in AstFixture(
        "function foo(a, b = 1) {}"
          ...
          cpg =>
            ...
            val List(ternaryCallB) =
                assignmentB.astChildren.isCall.nameExact(Operators.conditional).l
            val List(testCallB) = ternaryCallB.astChildren.isCall.nameExact(Operators.equals).l
            testCallB.astChildren.isIdentifier.nameExact("b").size shouldBe 1
            testCallB.astChildren.isCall.nameExact("<operator>.void").size shouldBe 1
  ...
  "AST generation for import/require" should {
      "make available `import` statements via cpg.imports" in AstFixture(
        "import {x} from \"foo\";"
      ) { cpg =>
      ...
      "make available `require` statements via cpg.imports" in AstFixture(
        "const x = require(\"foo\");"
      ) { cpg =>
        val List(imp) = cpg.imports.l
      ...
      "allow traversing from dependency to import for `require` statements" in AstFixture(
        "const x = require(\"foo\");"
      ) {
          cpg =>
            val List(imp)  = cpg.imports.l
