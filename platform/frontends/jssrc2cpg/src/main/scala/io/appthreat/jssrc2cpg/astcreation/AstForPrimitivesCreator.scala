...
trait AstForPrimitivesCreator(implicit withSchemaValidation: ValidationMode):
  ...
  protected def astForSpreadOrRestElement(
    spreadElement: BabelNodeInfo,
    arg1Ast: Option[Ast] = None
  ): Ast =
  ...
  protected def astForTemplateLiteral(templateLiteral: BabelNodeInfo): Ast =
    ...
    if expressions.isEmpty && quasis.isEmpty then
      ...
      val callCode =
          s"$callName${(argsCodes :+ s"\"${quasisTail("value")("raw").str}\"").mkString("(", ", ", ")")}"
      val templateCall =
      ...
      val argumentAsts = expressions.zip(quasis).flatMap { case (expression, quasi) =>
          List(
            astForNodeWithFunctionReference(quasi),
            astForNodeWithFunctionReference(expression)
          )
      }
