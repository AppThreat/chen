...
import io.appthreat.jssrc2cpg.datastructures.BlockScope
...
import io.appthreat.x2cpg.utils.NodeBuilders.{newBindingNode, newLocalNode}
...
trait AstForTypesCreator(implicit withSchemaValidation: ValidationMode):
  ...
  protected def astForTypeAlias(alias: BabelNodeInfo, nameHint: Option[String] = None): Ast =
    ...
    val typeDeclNodeAst =
        if !Defines.JsTypes.contains(name) && !seenAliasTypes.exists(_.name == name) then
          ...
          seenAliasTypes
              .collectFirst {
                  case typeDecl if typeDecl.name == name =>
                      Ast(typeDecl.aliasTypeFullName(aliasFullName))
              }
              .getOrElse(Ast())
    ...
    constructorContent: List[Ast]
      ...
      findClassConstructor(classExpr) match
        case Some(classConstructor) if hasKey(classConstructor, "body") =>
            val result =
                createMethodAstAndNode(
                  createBabelNodeInfo(classConstructor),
        ...
        case _ =>
            val result = createFakeConstructor(
              "constructor() {}",
              classExpr,
              methodBlockContent = constructorContent
            )
  ...
  private def interfaceConstructor(typeName: String, tsInterface: BabelNodeInfo): NewMethod =
      findClassConstructor(tsInterface) match
        case Some(interfaceConstructor) =>
            createMethodDefinitionNode(createBabelNodeInfo(interfaceConstructor))
  ...
  private def astsForEnumMember(tsEnumMember: BabelNodeInfo): Seq[Ast] =
    ...
    if hasKey(tsEnumMember.json, "initializer") then
  ...
  private def astForClassMember(
    ...
    if hasKey(nodeInfo.json, "value") && !nodeInfo.json("value").isNull then
  ...
  protected def astForEnum(tsEnum: BabelNodeInfo): Ast =
    ...
    val memberAsts = tsEnum.json("members").arr.toList.flatMap(m =>
    ...
    val (calls, member) =
        memberAsts.partition(_.nodes.headOption.exists(_.isInstanceOf[NewCall]))
    if calls.isEmpty then
      ...
      val init =
          staticInitMethodAst(
            calls,
            s"$typeFullName:${io.appthreat.x2cpg.Defines.StaticInitMethodName}",
            None,
            Defines.Any
          )
  ...
  private def isStaticInitBlock(json: Value): Boolean =
      createBabelNodeInfo(json).node == StaticBlock
  ...
  protected def astForClass(
    ...
    val implements = Try(
      clazz.json("implements").arr.map(createBabelNodeInfo(_).code)
    ).toOption.toSeq.flatten
    val mixins =
        Try(clazz.json("mixins").arr.map(createBabelNodeInfo(_).code)).toOption.toSeq.flatten
    ...
    val staticInitBlockAsts =
        staticInitBlock.map(block =>
            block("body").arr.toList.map(astForNodeWithFunctionReference)
    ...
    if staticMemberInitCalls.nonEmpty || staticInitBlockAsts.nonEmpty then
    ...
    if shouldCreateAssignmentCall then
  ...
  protected def addModifier(node: NewNode, json: Value): Unit =
      createBabelNodeInfo(json).node match
        case ClassPrivateProperty =>
            diffGraph.addEdge(
              node,
              NewModifier().modifierType(ModifierTypes.PRIVATE),
              EdgeTypes.AST
            )
        case _ =>
            if safeBool(json, "abstract").contains(true) then
              diffGraph.addEdge(
                node,
                NewModifier().modifierType(ModifierTypes.ABSTRACT),
                EdgeTypes.AST
              )
            if safeBool(json, "static").contains(true) then
              diffGraph.addEdge(
                node,
                NewModifier().modifierType(ModifierTypes.STATIC),
                EdgeTypes.AST
              )
            if safeStr(json, "accessibility").contains("public") then
              diffGraph.addEdge(
                node,
                NewModifier().modifierType(ModifierTypes.PUBLIC),
                EdgeTypes.AST
              )
            if safeStr(json, "accessibility").contains("private") then
              diffGraph.addEdge(
                node,
                NewModifier().modifierType(ModifierTypes.PRIVATE),
                EdgeTypes.AST
              )
            if safeStr(json, "accessibility").contains("protected") then
              diffGraph.addEdge(
                node,
                NewModifier().modifierType(ModifierTypes.PROTECTED),
                EdgeTypes.AST
              )
  ...
  protected def astForModule(tsModuleDecl: BabelNodeInfo): Ast =
    ...
    val blockAst = if hasKey(tsModuleDecl.json, "body") then
  ...
  protected def astForInterface(tsInterface: BabelNodeInfo): Ast =
    ...
    val extendz = Try(
      tsInterface.json("extends").arr.map(createBabelNodeInfo(_).code)
    ).toOption.toSeq.flatten
