...
import io.appthreat.jssrc2cpg.datastructures.{MethodScope, Scope}
...
import io.appthreat.jssrc2cpg.parser.BabelJsonParser.ParseResult
...
class AstCreator(
  ...
  protected val functionNodeToNameAndFullName =
      mutable.HashMap.empty[BabelNodeInfo, (String, String)]
  ...
  protected def astForNode(json: Value): Ast =
    ...
    nodeInfo.node match
      ...
      case InterfaceDeclaration      => astForInterface(nodeInfo)
      case DeclareInterface          => astForInterface(nodeInfo)
      ...
      case DeclareFunction           => astForTSDeclareFunction(nodeInfo)
      ...
      case DeclareOpaqueType         => astForTypeAlias(nodeInfo)
      case OpaqueType                => astForTypeAlias(nodeInfo)
      ...
      case TSInstantiationExpression => astForTSInstantiationExpression(nodeInfo)
      ...
      case ImportExpression          => astForImportExpression(nodeInfo)
  ...
  protected def astForNodes(jsons: List[Value]): List[Ast] =
      jsons.map(astForNodeWithFunctionReference)

  private def astsForFile(file: BabelNodeInfo): List[Ast] =
      astsForProgram(createBabelNodeInfo(file.json("program")))

  private def astsForProgram(program: BabelNodeInfo): List[Ast] =
