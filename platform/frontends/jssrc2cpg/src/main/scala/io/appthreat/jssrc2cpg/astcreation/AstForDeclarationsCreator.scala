...
import io.appthreat.jssrc2cpg.datastructures.{BlockScope, MethodScope, ScopeType}
...
trait AstForDeclarationsCreator(implicit withSchemaValidation: ValidationMode):
  ...
  protected def codeForBabelNodeInfo(obj: BabelNodeInfo): Seq[String] =
    val codes = obj.node match
      ...
      case VariableDeclaration =>
          obj.json("declarations").arr.toSeq.flatMap(d =>
              codeForBabelNodeInfo(createBabelNodeInfo(d))
          )
      case _ => Seq.empty
    codes.map(_.replace("...", ""))
  end codeForBabelNodeInfo

  private def createExportCallAst(
    name: String,
    exportName: String,
    declaration: BabelNodeInfo
  ): Ast =
    val exportCallAst = if name == DefaultsKey then
  ...
  private def extractDeclarationsFromExportDecl(
    declaration: BabelNodeInfo,
    key: String
  ): Option[(Ast, Seq[String])] =
      safeObj(declaration.json, key)
          .map { d =>
            ...
            val names       = if codes.isEmpty then defaultName.toSeq else codes
            (ast, names)
          }
  ...
  private def cleanImportName(name: String): String = if name.contains("/") then
  ...
  private def createAstForFrom(fromName: String, declaration: BabelNodeInfo): Ast =
      if fromName == ExportKeyword then
        Ast()
      else
        ...
        val sourceCallArgNode =
            literalNode(declaration, s"\"${fromName.stripPrefix("_")}\"", None)
        val sourceCall =
            callNode(
              declaration,
  ...
  protected def astsForDecorators(elem: BabelNodeInfo): Seq[Ast] =
      if hasKey(elem.json, "decorators") && !elem.json("decorators").isNull then
              ...
              createBabelNodeInfo(arg).node match
                case AssignmentExpression =>
                    annotationAssignmentAst(
                      code(arg("left")),
                      code(arg),
                      astForNodeWithFunctionReference(arg("right"))
                    )
                case _ =>
                    annotationAssignmentAst(
                      "value",
                      code(arg),
                      astForNodeWithFunctionReference(arg)
                    )
          }
  ...
  protected def astForExportNamedDeclaration(declaration: BabelNodeInfo): Ast =
    val specifiers = declaration
        ...
        .toList
        .map { spec =>
            if createBabelNodeInfo(spec).node == ExportNamespaceSpecifier then
              val exported = createBabelNodeInfo(spec("exported"))
              (None, Option(exported))
            else
              val exported = createBabelNodeInfo(spec("exported"))
              val local = if hasKey(spec, "local") then
    ...
    val declAsts = declAstAndNames.toList.flatMap { case (ast, names) =>
        ast +: names.map { name =>
          if exportName != ExportKeyword then
            diffGraph.addNode(newDependencyNode(
              name,
              exportName.stripPrefix("_"),
              RequireKeyword
            ))
    ...
    val specifierAsts = specifiers.map {
        case (Some(name), Some(alias)) =>
            ...
            if exportName != ExportKeyword then
              ...
              createExportAssignmentCallAst(
                name.code,
                exportCallAst,
                declaration,
                Option(s"_$strippedCode")
              )
            else
  ...
  end astForExportNamedDeclaration
  ...
  protected def astForExportAllDeclaration(declaration: BabelNodeInfo): Ast =
    ...
    val assignmentCallAst =
        createExportAssignmentCallAst(s"_$name", exportCallAst, declaration, None)
  ...
  protected def astForVariableDeclaration(declaration: BabelNodeInfo): Ast =
    ...
    val scopeType = if kind == "let" then
      BlockScope
    else
      MethodScope
    val declAsts = declaration.json("declarations").arr.toList.map(astForVariableDeclarator(
      _,
      scopeType,
      kind
    ))
  ...
  private def handleRequireCallForDependencies(
    ...
    val groupId =
        rhsCode.substring(rhsCode.indexOf(s"$RequireKeyword(") + 9, rhsCode.indexOf(")") - 1)
  ...
  end handleRequireCallForDependencies
  ...
  private def astForRequireCallFromImport(
    ...
    val thisNode =
        identifierNode(nodeInfo, "this").dynamicTypeHintFullName(typeHintForThisExpression())
    ...
    val sourceAst = if isImportN then
  ...
  protected def astForImportDeclaration(impDecl: BabelNodeInfo): Ast =
    ...
    if specifiers.isEmpty then
      ...
      val requireCalls = specs.map { importSpecifier =>
        ...
        val assignment = astForRequireCallFromImport(
          reqName,
          alias,
          source,
          isImportN = isImportN,
        ...
        val call =
            assignment.nodes.collectFirst { case x: NewCall if x.name == "require" => x }
        val importNode = createImportNodeAndAttachToCall(
          impDecl,
          s"$source:$reqName",
          importedName,
          call
      ...
      if requireCalls.isEmpty then
        Ast()
      else if requireCalls.sizeIs == 1 then
        requireCalls.head
      else
        blockAst(createBlockNode(impDecl), requireCalls)
    end if
  end astForImportDeclaration

  private def reqNameFromImportSpecifier(importSpecifier: Value, name: String) =
      if hasKey(importSpecifier, "imported") then
  ...
  private def createImportNodeAndAttachToCall(
    ...
    call: Option[NewCall]
      ...
      createImportNodeAndAttachToCall(
        impDecl.code.stripSuffix(";"),
        importedEntity,
        importedAs,
        call
      )
  ...
  private def convertDestructingObjectElement(
  ...
  end convertDestructingObjectElement

  private def convertDestructingArrayElement(
    element: BabelNodeInfo,
    index: Int,
    localTmpName: String
  ): Ast =
  ...
  private def convertDestructingArrayElementWithDefault(
    ...
    val lhsAst = nodeInfo.node match
      case ObjectPattern | ArrayPattern =>
          val sourceAst =
              astForNodeWithFunctionReference(createBabelNodeInfo(rhsElement).json)
  ...
  end convertDestructingArrayElementWithDefault
  ...
  end convertDestructingObjectElementWithDefault
  ...
  protected def astForDeconstruction(
    ...
    val rhsAssignmentAst =
        paramName.map(createParamAst(pattern, _, sourceAst)).getOrElse(sourceAst)
          ...
          pattern.json("properties").arr.toList.map { element =>
            ...
            nodeInfo.node match
              ...
              case _ =>
                  ...
                  nodeInfo.node match
                    case Identifier =>
                        convertDestructingObjectElement(
                          nodeInfo,
              ...
              case (element, index) if !element.isNull =>
                  ...
                  nodeInfo.node match
                    case RestElement =>
                        ...
                        val keyNode =
                            literalNode(nodeInfo, index.toString, Option(Defines.Number))
                        val accessAst =
                            createIndexAccessCallAst(
                              fieldAccessTmpNode,
                              keyNode,
                    ...
                    case AssignmentPattern =>
                        convertDestructingArrayElementWithDefault(
                          nodeInfo,
                          index,
                          localTmpName
                        )
  ...
  end astForDeconstruction
end AstForDeclarationsCreator
