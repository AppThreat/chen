...
trait AstNodeBuilder(implicit withSchemaValidation: ValidationMode):
  ...
  protected def createJumpTarget(switchCase: BabelNodeInfo): NewJumpTarget =
    val (switchName, switchCode) = if switchCase.json("test").isNull then
      ("default", "default:")
    else
  ...
  protected def createControlStructureNode(
    node: BabelNodeInfo,
  ...
  protected def createTernaryCallAst(
    ...
    val code =
        s"${codeOf(testAst.nodes.head)} ? ${codeOf(trueAst.nodes.head)} : ${codeOf(falseAst.nodes.head)}"
    val callNode =
        createCallNode(code, Operators.conditional, DispatchTypes.STATIC_DISPATCH, line, column)
  ...
  protected def createEqualsCallAst(
    ...
    val callNode =
        createCallNode(code, Operators.equals, DispatchTypes.STATIC_DISPATCH, line, column)
  ...
  protected def createAssignmentCallAst(
    ...
    val callNode =
        createCallNode(code, Operators.assignment, DispatchTypes.STATIC_DISPATCH, line, column)
  ...
  protected def createFunctionTypeAndTypeDeclAst(
    ...
    Ast(functionTypeDeclNode).withBindsEdge(functionTypeDeclNode, bindingNode).withRefEdge(
      bindingNode,
      methodNode
    )
  end createFunctionTypeAndTypeDeclAst
end AstNodeBuilder
