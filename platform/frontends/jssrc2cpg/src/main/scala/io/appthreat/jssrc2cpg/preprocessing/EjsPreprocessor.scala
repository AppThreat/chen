...
class EjsPreprocessor:
      ...
      matches.foreach {
          case mat if mat.group(1) == "<%" && mat.group(3) == "-%>" =>
              scriptBlock = scriptBlock.replace(
                mat.toString(),
                " " * mat.toString().replaceAll("[^\\s]", " ").length
              )
          case _ =>
      }
      Tags.foreach { tag =>
  ...
  def preprocess(code: String): String =
    ...
    val positions = matches.flatMap {
        ...
        case ma if ma.group(2).trim.startsWith("include ") =>
            None // ignore including other ejs templates
        case ma =>
    ...
    codeAsCharArray.zipWithIndex.foreach {
        ...
        case (currChar, index) if positions.exists { case (start, end) =>
    ...
    matches.foreach {
        ...
        case ma
            if ma.group(2).trim.startsWith(
              "include "
            ) => // ignore including other ejs templates
        case ma if needsSemicolon(ma.group(2)) =>
            val start = ma.start + ma.group(1).length
            val end   = ma.end - ma.group(3).length
            if !alreadyReplaced.contains((start, end)) then
              val replacementCode = s"${ma.group(2)};"
              codeWithoutSemicolon =
                  s"${codeWithoutSemicolon.substring(0, start)}$replacementCode${codeWithoutSemicolon
                          .substring(end + 1, codeWithoutSemicolon.length)}"
