...
import io.appthreat.jssrc2cpg.passes.{Defines, EcmaBuiltins, GlobalBuiltins}
...
trait AstForExpressionsCreator(implicit withSchemaValidation: ValidationMode):
    ...
    callExpr.json.obj
        ...
        .flatMap {
            case callee if callee.node.isInstanceOf[FunctionLike] =>
                functionNodeToNameAndFullName.get(callee)
            case _ => None
        }
  ...
  protected def astForCallExpression(callExpr: BabelNodeInfo): Ast =
    ...
    if callee.node == Import then
      ...
      val (receiverAst, baseNode, callName) = callee.node match
        ...
        case _ =>
            val receiverAst = astForNodeWithFunctionReference(callee.json)
            val thisNode = identifierNode(callee, "this").dynamicTypeHintFullName(
              typeHintForThisExpression()
            )
  ...
  protected def astForMetaProperty(metaProperty: BabelNodeInfo): Ast =
    ...
    val memberAst = Ast(
      createFieldIdentifierNode(
        memberNodeInfo.code,
        memberNodeInfo.lineNumber,
        memberNodeInfo.columnNumber
      )
    )
    createFieldAccessCallAst(
      metaAst,
      memberAst.nodes.head,
      metaProperty.lineNumber,
      metaProperty.columnNumber
    )
  ...
  end astForMemberExpression

  protected def astForAssignmentExpression(assignment: BabelNodeInfo): Ast =
    val op = if hasKey(assignment.json, "operator") then
  ...
  end astForAssignmentExpression
    ...
    if elements.isEmpty then
      ...
      val elementAsts = elements.flatMap {
          case element if !element.isNull =>
              ...
              val memberNode =
                  createFieldIdentifierNode("push", elementLineNumber, elementColumnNumber)
              val receiverNode = createFieldAccessCallAst(
                baseNode,
                memberNode,
                elementLineNumber,
                elementColumnNumber
      ...
      val blockChildrenAsts =
          assignmentTmpArrayCallNode +: elementAsts :+ Ast(tmpArrayReturnNode)
      setArgumentIndices(blockChildrenAsts)
      blockAst(blockNode, blockChildrenAsts)
    end if
  end astForArrayExpression
      ...
      nodeInfo.node match
        ...
        case _ =>
            val (lhsNode, rhsAst) = nodeInfo.node match
              case ObjectMethod =>
                  val keyName =
                      if hasKey(nodeInfo.json("key"), "name") then
                        nodeInfo.json("key")("name").str
