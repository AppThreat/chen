...
object PackageJsonParser:
  ...
  def isValidProjectPackageJson(packageJsonPath: Path): Boolean =
      if packageJsonPath.toString.endsWith(PackageJsonParser.PackageJsonFilename) then
            ...
            lazy val deps = Try {
                ...
                ProjectDependencies
                    .foreach { dependency =>
                      ...
                      dependencyIt.foreach { entry =>
                          depToVersion =
                              depToVersion.updated(entry.getKey, entry.getValue.asText())
                      }
                    }
                depToVersion
            }.toOption

            if lockDeps.isDefined && lockDeps.get.nonEmpty then
              logger.debug(s"Loaded dependencies from '$lockDepsPath'.")
              lockDeps.get
            else if deps.isDefined && deps.get.nonEmpty then
