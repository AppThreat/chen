...
import io.appthreat.x2cpg.utils.NodeBuilders.newLocalNode
...
trait AstForStatementsCreator(implicit withSchemaValidation: ValidationMode):
    ...
    /** Sort all block statements with the following result:
      ...
      *
      * We do this to get TypeDecls created at the right spot so we can make use of them for the
      * type aliases.
      */
    ...
    protected def astForDoWhileStatement(doWhileStmt: BabelNodeInfo): Ast =
        ...
        Ast(whileNode).withChild(bodyAst).withChild(testAst).withConditionEdge(
          whileNode,
          testAst.nodes.head
        )
    ...
    protected def astForWhileStatement(whileStmt: BabelNodeInfo): Ast =
        ...
        Ast(whileNode).withChild(testAst).withConditionEdge(
          whileNode,
          testAst.nodes.head
        ).withChild(bodyAst)
    ...
    protected def astForContinueStatement(continueStmt: BabelNodeInfo): Ast =
        val labelAst = safeObj(continueStmt.json, "label")
            ...
            .getOrElse(Ast())
        Ast(createControlStructureNode(continueStmt, ControlStructureTypes.CONTINUE)).withChild(
          labelAst
        )
    end astForContinueStatement
    ...
    private def astsForSwitchCase(switchCase: BabelNodeInfo): List[Ast] =
        ...
        val testAsts = safeObj(switchCase.json, "test").map(t =>
            astForNodeWithFunctionReference(Obj(t))
        ).toList
    ...
    protected def astForSwitchStatement(switchStmt: BabelNodeInfo): Ast =
        ...
        val casesAsts =
            switchStmt.json("cases").arr.flatMap(c => astsForSwitchCase(createBabelNodeInfo(c)))
    ...
    /** De-sugaring from:
      ...
      * to:
      *
      * { var _iterator = <operator>.iterator(arr); var _result; var i; while (!(_result =
      * _iterator.next()).done) { i = _result.value; body } }
      */
    private def astForInOfStatementWithIdentifier(
      forInOfStmt: BabelNodeInfo,
      idNodeInfo: BabelNodeInfo
    ): Ast =
        // surrounding block:
        ...
        val rhsNode =
            callNode(forInOfStmt, s"$iteratorName.next()", "next", DispatchTypes.DYNAMIC_DISPATCH)

        val nextBaseNode = identifierNode(forInOfStmt, iteratorName)

        val nextMemberNode =
            createFieldIdentifierNode("next", forInOfStmt.lineNumber, forInOfStmt.columnNumber)
        ...
        val doneMemberNode =
            createFieldIdentifierNode("done", forInOfStmt.lineNumber, forInOfStmt.columnNumber)
        ...
        val whileLoopAst =
            Ast(whileLoopNode).withChild(testCallAst).withConditionEdge(whileLoopNode, testCallNode)
        ...
        val memberNode =
            createFieldIdentifierNode("value", forInOfStmt.lineNumber, forInOfStmt.columnNumber)
        ...
        val blockChildren =
            List(
              iteratorAssignmentAst,
              Ast(resultNode),
              Ast(loopVariableNode),
              whileLoopAst.withChild(whileLoopBlockAst)
            )
        setArgumentIndices(blockChildren)
        blockAst(blockNode, blockChildren)
    end astForInOfStatementWithIdentifier
    ...
    /** De-sugaring from:
      ...
      * to:
      *
      * { var _iterator = <operator>.iterator(arr); var _result; while (!(_result =
      * _iterator.next()).done) { expr = _result.value; body } }
      */
    private def astForInOfStatementWithExpression(
      forInOfStmt: BabelNodeInfo,
      idNodeInfo: BabelNodeInfo
    ): Ast =
        // surrounding block:
        ...
        val blockChildren =
            List(iteratorAssignmentAst, Ast(resultNode), whileLoopAst.withChild(whileLoopBlockAst))
        setArgumentIndices(blockChildren)
        blockAst(blockNode, blockChildren)
    end astForInOfStatementWithExpression
    ...
    /** De-sugaring from:
      ...
      * to:
      *
      * { var _iterator = <operator>.iterator(obj); var _result; var a; var b; var c; while
      * (!(_result = _iterator.next()).done) { a = _result.value.a; b = _result.value.b; c =
      * _result.value.c; body } }
      */
    private def astForInOfStatementWithObject(
      forInOfStmt: BabelNodeInfo,
      idNodeInfo: BabelNodeInfo
    ): Ast =
        // surrounding block:
        ...
        loopVariableLocalNodes.foreach(diffGraph.addEdge(
          localAstParentStack.head,
          _,
          EdgeTypes.AST
        ))
        loopVariableNames.zip(loopVariableNodes).foreach {
            case (loopVariableName, loopVariableNode) =>
                scope.addVariableReference(loopVariableName, loopVariableNode)
        }
        ...
        val blockNodeChildren =
            List(iteratorAssignmentAst, Ast(resultNode)) ++ loopVariableNodes.map(
              Ast(_)
            ) :+ whileLoopAst.withChild(
              whileLoopBlockAst
            )
        setArgumentIndices(blockNodeChildren)
        blockAst(blockNode, blockNodeChildren)
    end astForInOfStatementWithObject
    ...
    /** De-sugaring from:
      ...
      * to:
      *
      * { var _iterator = <operator>.iterator(arr); var _result; var a; var b; var c; while
      * (!(_result = _iterator.next()).done) { a = _result.value[0]; b = _result.value[1]; c =
      * _result.value[2]; body } }
      */
    private def astForInOfStatementWithArray(
      forInOfStmt: BabelNodeInfo,
      idNodeInfo: BabelNodeInfo
    ): Ast =
        // surrounding block:
        ...
        // while loop variable assignment:
        val loopVariableAssignmentAsts =
            loopVariableNames.zipWithIndex.map { case (loopVariableName, index) =>
                val whileLoopVariableNode = identifierNode(forInOfStmt, loopVariableName)
                val baseNode              = identifierNode(forInOfStmt, resultName)
                ...
                val variableMemberNode = literalNode(
                  forInOfStmt,
                  index.toString,
        ...
        setArgumentIndices(blockNodeChildren)
        blockAst(blockNode, blockNodeChildren)
    end astForInOfStatementWithArray
    ...
    private def extractLoopVariableNodeInfo(nodeInfo: BabelNodeInfo): Option[BabelNodeInfo] =
        nodeInfo.node match
            ...
            case VariableDeclaration =>
                ...
                if varDeclNodeInfo.node == VariableDeclarator then
                    Option(createBabelNodeInfo(varDeclNodeInfo.json("id")))
                else None
            case _ => None
        ...
        loopVariableNodeInfo.node match
            case VariableDeclaration | AssignmentPattern =>
                ...
                idNodeInfo.map(_.node) match
                    case Some(ObjectPattern) =>
                        astForInOfStatementWithObject(forInOfStmt, idNodeInfo.get)
                    case Some(ArrayPattern) =>
                        astForInOfStatementWithArray(forInOfStmt, idNodeInfo.get)
                    case Some(Identifier) =>
                        astForInOfStatementWithIdentifier(forInOfStmt, idNodeInfo.get)
