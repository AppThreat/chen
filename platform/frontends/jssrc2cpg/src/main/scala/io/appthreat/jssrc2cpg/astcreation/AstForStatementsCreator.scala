...
trait AstForStatementsCreator(implicit withSchemaValidation: ValidationMode):
  ...
  /** Sort all block statements with the following result:
    ...
    *
    * We do this to get TypeDecls created at the right spot so we can make use of them for the type
    * aliases.
    */
  ...
  /** De-sugaring from:
    ...
    * \= _iterator.next()).done) { a = _result.value.a; b = _result.value.b; c = _result.value.c;
    * body } }
    */
  ...
  /** De-sugaring from:
    ...
    * \= _iterator.next()).done) { a = _result.value[0]; b = _result.value[1]; c = _result.value[2];
    * body } }
    */
