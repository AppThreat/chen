...
class AstCreator(filename: String, phpAst: PhpFile)(implicit withSchemaValidation: ValidationMode)
    ...
    /** This is used to rewrite the short form $xs[] = <value_expr> as array_push($xs, <value_expr>)
      ...
      * engine.
      *
      * This representation is technically wrong in the case where the shorthand is used to
      * initialise a new array (since PHP expects the first argument to array_push to be an existing
      * array). This shouldn't affect dataflow, however.
      */
