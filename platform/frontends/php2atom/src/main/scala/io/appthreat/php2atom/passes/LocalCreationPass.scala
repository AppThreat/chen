...
abstract class LocalCreationPass[ScopeType <: AstNode](cpg: Cpg)
  ...
  protected def addRefEdges(
    diffGraph: DiffGraphBuilder,
    localPairs: List[(NewLocal, List[Identifier])]
  ): Unit =
  ...
  protected def prependLocalsToBody(
    diffGraph: DiffGraphBuilder,
    bodyNode: AstNode,
    locals: List[NewLocal]
  ): Unit =
...
class MethodLocalPass(cpg: Cpg) extends LocalCreationPass[Method](cpg):
  ...
  override def runOnPart(diffGraph: DiffGraphBuilder, method: Method): Unit =
    ...
    addLocalsToAst(
      diffGraph,
      method.body,
      excludeIdentifierFn = identifier => parameters.contains(identifier.name)
    )
