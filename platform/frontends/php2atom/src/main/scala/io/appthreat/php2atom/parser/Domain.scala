...
object Domain:
    ...
    final case class PhpWhileStmt(cond: PhpExpr, stmts: List[PhpStmt], attributes: PhpAttributes)
        extends PhpStmtWithBody
    final case class PhpDoStmt(cond: PhpExpr, stmts: List[PhpStmt], attributes: PhpAttributes)
        extends PhpStmtWithBody
    ...
    final case class PhpElseIfStmt(cond: PhpExpr, stmts: List[PhpStmt], attributes: PhpAttributes)
        extends PhpStmtWithBody
    final case class PhpElseStmt(stmts: List[PhpStmt], attributes: PhpAttributes)
        extends PhpStmtWithBody
    final case class PhpSwitchStmt(
      condition: PhpExpr,
      cases: List[PhpCaseStmt],
      attributes: PhpAttributes
    ) extends PhpStmt
    final case class PhpCaseStmt(
    ...
    final case class PhpFinallyStmt(stmts: List[PhpStmt], attributes: PhpAttributes)
        extends PhpStmtWithBody
    ...
    final case class PhpPropertyValue(
      name: PhpNameExpr,
      defaultValue: Option[PhpExpr],
      attributes: PhpAttributes
    ) extends PhpStmt

    final case class PhpConstStmt(
      modifiers: List[String],
    ...
    final case class PhpNamespaceStmt(
      name: Option[PhpNameExpr],
      stmts: List[PhpStmt],
      attributes: PhpAttributes
    ) extends PhpStmtWithBody
    ...
    final case class PhpDeclareItem(key: PhpNameExpr, value: PhpExpr, attributes: PhpAttributes)
        extends PhpStmt

    final case class PhpUnsetStmt(vars: List[PhpExpr], attributes: PhpAttributes) extends PhpStmt

    final case class PhpStaticStmt(vars: List[PhpStaticVar], attributes: PhpAttributes)
        extends PhpStmt

    final case class PhpStaticVar(
    ...
    final case class PhpNewExpr(
      className: PhpNode,
      args: List[PhpArgument],
      attributes: PhpAttributes
    ) extends PhpExpr

    final case class PhpIncludeExpr(expr: PhpExpr, includeType: String, attributes: PhpAttributes)
        extends PhpExpr
    ...
    final case class PhpUnaryOp(operator: String, expr: PhpExpr, attributes: PhpAttributes)
        extends PhpExpr
    object PhpUnaryOp:
    ...
    final case class PhpInt(val value: String, val attributes: PhpAttributes)
        extends PhpSimpleScalar(TypeConstants.Int)
    ...
    final case class PhpListExpr(items: List[Option[PhpArrayItem]], attributes: PhpAttributes)
        extends PhpExpr
    ...
    final case class PhpArrayExpr(items: List[Option[PhpArrayItem]], attributes: PhpAttributes)
        extends PhpExpr
    ...
    final case class PhpArrayDimFetchExpr(
      variable: PhpExpr,
      dimension: Option[PhpExpr],
      attributes: PhpAttributes
    ) extends PhpExpr

    final case class PhpErrorSuppressExpr(expr: PhpExpr, attributes: PhpAttributes) extends PhpExpr

    final case class PhpInstanceOfExpr(expr: PhpExpr, className: PhpExpr, attributes: PhpAttributes)
        extends PhpExpr
    ...
    final case class PhpMatchExpr(
      condition: PhpExpr,
      matchArms: List[PhpMatchArm],
      attributes: PhpAttributes
    ) extends PhpExpr

    final case class PhpMatchArm(
      conditions: List[PhpExpr],
      body: PhpExpr,
      isDefault: Boolean,
      attributes: PhpAttributes
    ...
    final case class PhpClosureUse(variable: PhpExpr, byRef: Boolean, attributes: PhpAttributes)
        extends PhpExpr
    ...
    private def readList(json: Value): PhpListExpr =
        val items =
            json("items").arr.map(item => Option.unless(item.isNull)(readArrayItem(item))).toList

        PhpListExpr(items, PhpAttributes(json))

    private def readNew(json: Value): PhpNewExpr =
        val classNode =
            if json("class")("nodeType").strOpt.contains("Stmt_Class") then
    ...
    private def readClassConstFetch(json: Value): PhpClassConstFetchExpr =
        ...
        val className =
            if classNameType.startsWith("Name") then
                readName(json("class"))
            else
                readExpr(json("class"))
        ...
        val constantName = json("name") match
            ...
            case other => throw new NotImplementedError(
                  s"unexpected constant name '$other' of type ${other.getClass}"
                )
    ...
    private def extendsForClassLike(json: Value): List[PhpNameExpr] =
        json.obj
            ...
            .map {
                ...
                case other => throw new NotImplementedError(
                      s"unexpected 'extends' entry '$other' of type ${other.getClass}"
                    )
            }
            .getOrElse(Nil)
    ...
    private def readClassLike(json: Value, classLikeType: String): PhpClassLikeStmt =
        ...
        val scalarType =
            json.obj.get("scalarType").flatMap(typ => Option.unless(typ.isNull)(readName(typ)))
    ...
    private def readNameOrExpr(json: Value, fieldName: String): PhpExpr =
        ...
        if field("nodeType").str.startsWith("Name") then
            readName(field)
        else if field("nodeType").str == "Identifier" then
            readName(field)
        else if field("nodeType").str == "VarLikeIdentifier" then
            readVariable(field)
        else
            readExpr(field)
    ...
    private def readCall(json: Value): PhpCallExpr =
        ...
        val target =
            jsonMap.get("var").map(readExpr).orElse(jsonMap.get("class").map(_ =>
                readNameOrExpr(jsonMap, "class")
            ))
    ...
    private def readUseUse(json: Value, parentType: PhpUseType): PhpUseUse =
        ...
        val useType =
            if parentType == PhpUseType.Unknown then
    ...
    private def readUnaryOp(json: Value): PhpUnaryOp =
        ...
        val expr =
            if json.obj.contains("expr") then
                readExpr(json.obj("expr"))
            else if json.obj.contains("var") then
                readExpr(json.obj("var"))
            else
                throw new UnsupportedOperationException(
                  s"Expected expr or var field in unary op but found $json"
                )
