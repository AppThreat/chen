...
import io.appthreat.php2atom.parser.Domain.PhpOperators
...
class CfgTests extends PhpCode2CpgFixture:
  ...
  "the CFG for if constructs" should {
      ...
      "find that sink2 is control dependent on the if and elseif conditions" in {
          inside(cpg.call.name("sink2").controlledBy.isCall.code.sorted.toList) {
              case List(xCode, yCode) =>
                  xCode shouldBe "$x < 10"
                  yCode shouldBe "$y > 5"
          }
      }
      ...
      "find that sink3 is control dependent on the if and elseif conditions" in {
          inside(cpg.call.name("sink3").controlledBy.isCall.code.sorted.toList) {
              case List(xCode, yCode) =>
                  xCode shouldBe "$x < 10"
                  yCode shouldBe "$y > 5"
          }
  ...
  "the CFG for switch constructs" should {
     ...
     |  }
      ...
      "find that all the sinks are control dependent on condition" in {
          (1 to 4).foreach { num =>
              inside(cpg.call.nameExact(s"sink$num").controlledBy.isCall.code.l) {
                  case List(code) =>
                      code shouldBe "$x < 10"
              }
          }
  ...
  "the CFG for try constructs" should {
      ...
      "find that call1 dominates all other calls" in {
          cpg.call.name("call1").dominates.collectAll[Call].name.toSet shouldBe (2 to 6).map(num =>
              s"call$num"
          ).toSet
      }

      "find that call6 is post dominated by the try and finally calls" in {
          cpg.call.name("call1").postDominatedBy.collectAll[Call].name.toSet shouldBe Set(
            "call2",
            "call5",
            "call6"
          )
