...
class ClosureTests extends PhpCode2CpgFixture:
  ...
  "long-form closures with uses " should {
      ...
      "have the correct method AST" in {
          val closureMethod =
              inside(cpg.method.name(".*<lambda>.*").l) { case List(closureMethod) =>
                  closureMethod
              }
          ...
          inside(closureMethod.body.astChildren.l) {
              case List(use1: Local, use2: Local, echoCall: Call) =>
                  use1.name shouldBe "use1"
                  use1.code shouldBe "$use1"
                  use1.closureBindingId shouldBe Some(s"foo.php:$expectedName:use1")
                  inside(cpg.all.collectAll[ClosureBinding].filter(
                    _.closureBindingId == use1.closureBindingId
                  ).l) {
      ...
      "have a ref edge from the closure binding for a use to the captured node" in {
          inside(
            cpg.all.collectAll[ClosureBinding].filter(_.closureOriginalName.contains("use1")).l
          ) {
              case List(closureBinding) =>
      ...
      "have a correct MethodRef added to the AST where the closure is defined" in {
          inside(cpg.assignment.code(".*<lambda>.*").argument.l) {
              case List(_: Identifier, methodRef: MethodRef) =>
                  val expectedName = s"foo.php:<global>-><lambda>0"
  ...
  "multiple closures in the same file should have the correct names" in {
     ...
     |""".stripMargin)
      ...
      inside(cpg.method.name(".*<lambda>.*").fullName.sorted.l) {
          case List(bar0, bar1, foo0, foo1) =>
