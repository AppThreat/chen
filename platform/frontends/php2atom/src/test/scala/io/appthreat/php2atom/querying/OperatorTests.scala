...
import io.appthreat.php2atom.astcreation.AstCreator.{NameConstants, TypeConstants}
import io.appthreat.php2atom.parser.Domain.{PhpDomainTypeConstants, PhpOperators}
...
class OperatorTests extends PhpCode2CpgFixture:
  ...
  "cast operations" should {
      ...
      "have the correct types" in {
          ...
          testData.foreach { case (testCode, expectedType) =>
              ...
              inside(cpg.call.nameExact(Operators.cast).argument(1).l) {
  ...
  "ternary operators" should {
      "be created correctly for general cond ? then : else style operators" in {
          ...
          val call =
              inside(cpg.call.nameExact(Operators.conditional).l) { case List(conditionalOp) =>
                  conditionalOp
              }
  ...
  "declare calls without statements should be correctly represented" in {
      ...
      inside(declareCall.argument.l) { case List(tickAssign: Call, encodingAssign: Call) =>
          ...
          inside(encodingAssign.argument.l) {
              case List(encodingIdentifier: Identifier, value: Literal) =>
