...
trait MacroHandler(implicit withSchemaValidation: ValidationMode):
    ...
    /** For the given node, determine if it is expanded from a macro, and if so, create a Call node
      * to represent the macro invocation and attach `ast` as its child.
      */
            ...
            case Some(callAst) =>
                ...
                val childAst = newAst.root match
                    case Some(_: NewBlock) =>
                        newAst
                    case _ =>
                        val b = NewBlock().argumentIndex(1).typeFullName(
    ...
    /** For the given node, determine if it is expanded from a macro, and if so, find the first
      * matching (offset, macro) pair in nodeOffsetMacroPairs, removing non-matching elements from
      * the start of nodeOffsetMacroPairs. Returns (Some(macroDefinition, arguments)) if a macro
    ...
    /** Create an AST that represents a macro expansion as a call. The AST is rooted in a CALL node
      * and contains sub trees for arguments. These are also connected to the AST via ARGUMENT
      * edges. We include line number information in the CALL node that is picked up by the
      * MethodStubCreator.
      */
