...
trait AstForTypesCreator(implicit withSchemaValidation: ValidationMode):
  ...
  protected def templateParameters(e: IASTNode): Option[String] =
    ...
    val templateParams =
        decl.map(d => ASTStringUtil.getTemplateParameterArray(d.getTemplateParameters))
  ...
  protected def handleUsingDeclaration(usingDecl: ICPPASTUsingDeclaration): Seq[Ast] =
    ...
    if !isQualifiedName(simpleName) then
      usingDecl.getParent match
        case ns: ICPPASTNamespaceDefinition =>
            usingDeclarationMappings.put(
              s"${fullName(ns)}.$mappedName",
              fixQualifiedName(simpleName)
            )
        case _ =>
    ...
    val declAsts = decl match
      ...
      case declaration: IASTSimpleDeclaration =>
          declaration.getDeclSpecifier match
            ...
            case _
                if declaration.getDeclarators.isEmpty && declaration.getParent.isInstanceOf[
                  IASTTranslationUnit
                ] =>
                Seq.empty // dangling decls from unresolved macros; we ignore them
      ...
      case namespaceDefinition: ICPPASTNamespaceDefinition =>
          Seq(astForNamespaceDefinition(namespaceDefinition))
          ...
          declaration.getDeclarators.toList.map {
              ...
              case arrayDecl: IASTArrayDeclarator =>
                  ...
                  val initCallNode = callNode(
                    arrayDecl,
                    nodeSignature(arrayDecl),
                    op,
                    op,
                    DispatchTypes.STATIC_DISPATCH
                  )
                  val initArgs =
                      arrayDecl.getArrayModifiers.toList.filter(m =>
                          m.getConstantExpression != null
                      ).map(astForNode)
  ...
  private def astsForCompositeType(
    ...
    val nameWithTemplateParams =
        templateParameters(typeSpecifier).map(t => registerType(s"$fullname$t"))
  ...
  private def astsForEnumerator(enumerator: IASTEnumerationSpecifier.IASTEnumerator): Seq[Ast] =
    ...
    if enumerator.getValue != null then
      val operatorName = Operators.assignment
      val callNode_ =
          callNode(
            enumerator,
  ...
  private def astsForEnum(
    ...
    val (deAliasedName, deAliasedFullName, newAlias) =
        if name.contains("anonymous_enum") && alias.isDefined then
          (
            alias.get,
            fullname.substring(0, fullname.indexOf("anonymous_enum")) + alias.get,
            None
          )
    ...
    if calls.isEmpty then
      ...
      val init = staticInitMethodAst(
        calls,
        s"$deAliasedFullName:${io.appthreat.x2cpg.Defines.StaticInitMethodName}",
        None,
        Defines.anyTypeName,
        Some(filename),
        lineNumber,
