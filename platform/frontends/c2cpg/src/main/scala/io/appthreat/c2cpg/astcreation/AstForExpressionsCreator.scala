...
trait AstForExpressionsCreator(implicit withSchemaValidation: ValidationMode):
    ...
    private def astForCallExpression(call: IASTFunctionCallExpression): Ast =
        val rec = call.getFunctionNameExpression match
            case unaryExpression: IASTUnaryExpression
                if unaryExpression.getOperand.isInstanceOf[IASTBinaryExpression] =>
                astForBinaryExpression(
                  unaryExpression.getOperand.asInstanceOf[IASTBinaryExpression]
                )
            case unaryExpression: IASTUnaryExpression
                if unaryExpression.getOperand.isInstanceOf[IASTFieldReference] =>
            ...
            case unaryExpression: IASTUnaryExpression
                ...
                astForArrayIndexExpression(
                  unaryExpression.getOperand.asInstanceOf[IASTArraySubscriptExpression]
                )
            case unaryExpression: IASTUnaryExpression
                if unaryExpression.getOperand.isInstanceOf[IASTConditionalExpression] =>
                astForUnaryExpression(unaryExpression)
            case unaryExpression: IASTUnaryExpression
                if unaryExpression.getOperand.isInstanceOf[IASTUnaryExpression] =>
        ...
        val (dd, name) = call.getFunctionNameExpression match
            ...
            case _
                ...
                (
                  ...
                  nodeSignature(
                    call.getFunctionNameExpression.asInstanceOf[IASTFieldReference].getFieldName
                  )
                )
    ...
    private def astForArrayIndexExpression(arrayIndexExpression: IASTArraySubscriptExpression)
      : Ast =
        val name = Operators.indirectIndexAccess
        val cpgArrayIndexing =
            callNode(
              arrayIndexExpression,
              nodeSignature(arrayIndexExpression),
              name,
              name,
              DispatchTypes.STATIC_DISPATCH
            )
    ...
    private def astsForConstructorInitializer(initializer: IASTInitializer): List[Ast] =
        initializer match
            case init: ICPPASTConstructorInitializer =>
                init.getArguments.toList.map(x => astForNode(x))
    ...
    private def astForNewExpression(newExpression: ICPPASTNewExpression): Ast =
        ...
        if newExpression.isArrayAllocation then
            ...
            Ast(cpgNewExpression).withChild(cpgTypeId).withArgEdge(
              cpgNewExpression,
              cpgTypeId.root.get
            )
        else
    ...
    protected def astForExpression(expression: IASTExpression): Ast =
        if config.includeFunctionBodies then
    ...
    protected def astForExpressionFull(expression: IASTExpression): Ast =
        val r = expression match
            ...
            case typeIdInit: IASTTypeIdInitializerExpression =>
                astForTypeIdInitExpression(typeIdInit)
            case c: ICPPASTSimpleTypeConstructorExpression => astForConstructorExpression(c)
            case lambdaExpression: ICPPASTLambdaExpression =>
                astForMethodRefForLambda(lambdaExpression)
            case cExpr: IGNUASTCompoundStatementExpression =>
                astForCompoundStatementExpression(cExpr)
