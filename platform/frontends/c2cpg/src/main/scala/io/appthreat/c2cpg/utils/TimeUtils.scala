...
object TimeUtils:
  ...
  private def chooseUnit(nanos: Long): TimeUnit =
    ...
    if d.toDays > 0 then DAYS
    else if d.toHours > 0 then HOURS
    else if d.toMinutes > 0 then MINUTES
    else if d.toSeconds > 0 then SECONDS
    else if d.toMillis > 0 then MILLISECONDS
    else if d.toMicros > 0 then MICROSECONDS
    else NANOSECONDS
