...
object IncludeAutoDiscovery:
    ...
    private def extractPaths(output: Seq[String]): Set[Path] =
        ...
        val endIndex =
            if IS_WIN then output.indexWhere(_.startsWith("End of search list.")) - 1
    ...
    private def discoverPaths(command: String): Set[Path] = ExternalCommand.run(command) match
        ...
        case Failure(exception) =>
            logger.warn(
              s"Unable to discover system include paths. Running '$command' failed.",
              exception
            )
            Set.empty

    def discoverIncludePathsC(config: Config): Set[Path] =
        if config.includePathsAutoDiscovery && systemIncludePathsC.nonEmpty then
            systemIncludePathsC
        ...
        then
            ...
            if includePathsC.nonEmpty then
                logger.debug(
                  s"Using the following C system include paths:${includePathsC
    ...
    def discoverIncludePathsCPP(config: Config): Set[Path] =
        if config.includePathsAutoDiscovery && systemIncludePathsCPP.nonEmpty then
        ...
        then
            ...
            if includePathsCPP.nonEmpty then
                logger.debug(
                  s"Using the following CPP system include paths:${includePathsCPP
                          .mkString(s"${System.lineSeparator()}- ", s"${System.lineSeparator()}- ", System.lineSeparator())}"
                )
