...
trait ParseProblemsLogger:
    ...
    /** The exception message might be null for parse failures due to ambiguous nodes that can't be
      * resolved successfully. We extract better log messages for this case here.
      */
        ...
        Option(exception.getMessage) match
            ...
            case None =>
                exception.getStackTrace
                    .collectFirst {
                        case stackTraceElement: StackTraceElement
                            if stackTraceElement.getClassName.endsWith("ASTAmbiguousNode") =>
                            "Could not resolve ambiguous node!"
                    }
                    .getOrElse(exception.getStackTrace.mkString(System.lineSeparator()))
end ParseProblemsLogger
