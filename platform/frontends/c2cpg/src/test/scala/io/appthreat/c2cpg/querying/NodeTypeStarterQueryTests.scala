...
/** The following tests show in detail how queries can be started. For all node types, for which it
  * seems reasonable, all nodes of that type can be used as a starting point, e.g., `cpg.method`
  * starts at all methods while `cpg.local` starts at all locals.
  */
class NodeTypeStarterQueryTests extends CCodeToCpgSuite:
  ...
  /** All methods - whether defined (e.g., "main") or only referenced, e.g., "libfunc" are
    * represented by METHOD nodes in the CPG. Any method for which a definition exists is in
    * `cpg.method.internal`. All other methods are in `cpg.method.external`.
    */
  ...
  "should allow retrieving type declarations" in {
      cpg.typeDecl.nameNot(NamespaceTraversal.globalNamespaceName).internal.name
          .toSetMutable shouldBe Set("foo")
  }
