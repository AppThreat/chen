...
class EnumTypeTests extends CCodeToCpgSuite(fileSuffix = FileDefaults.CPP_EXT):
  ...
  "Enums" should {
      ...
      "be correct for simple enum" in {
        ...
        |    blue
        |};""".stripMargin)
          inside(cpg.typeDecl.nameNot(NamespaceTraversal.globalNamespaceName).internal.l) {
              case List(color) =>
                  ...
                  inside(color.astChildren.isMethod.l) { case List(clinit) =>
                      clinit.name shouldBe io.appthreat.x2cpg.Defines.StaticInitMethodName
                      clinit.fullName shouldBe s"color:${io.appthreat.x2cpg.Defines.StaticInitMethodName}"
      ...
      "be correct for simple enum typedef" in {
        ...
        |    blue
        |} C;""".stripMargin)
          inside(cpg.typeDecl.nameNot(NamespaceTraversal.globalNamespaceName).filter(x =>
              !x.isExternal
          ).l) {
              case List(color, c) =>
                  color.name shouldBe "color"
          ...
          inside(cpg.typeDecl.nameNot(NamespaceTraversal.globalNamespaceName).internal.l) {
              case List(altitude) =>
                  ...
                  inside(altitude.astChildren.isMethod.l) { case List(clinit) =>
                      clinit.name shouldBe io.appthreat.x2cpg.Defines.StaticInitMethodName
                      clinit.fullName shouldBe s"altitude:${io.appthreat.x2cpg.Defines.StaticInitMethodName}"
