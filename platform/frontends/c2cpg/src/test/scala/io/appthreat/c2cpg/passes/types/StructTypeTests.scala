  ...
  "Struct with array members" should {
      ...
      "initialize array members correctly" in {
          val List(clInitMethod) =
              cpg.method.nameExact(io.appthreat.x2cpg.Defines.StaticInitMethodName).l
          val List(aInitCall, bInitCall, cInitCall) =
              clInitMethod.call.nameExact(Operators.arrayInitializer).l
