          ...
          inside(cpg.method.name("main").dotCfg.l) { case List(dotStr) =>
              dotStr should (
                startWith("digraph \"main\" {").and(
                  include("(&lt;operator&gt;.assignment,i = 0)")
                ).and(
                  endWith("}\n")
                )
              )
          ...
          inside(cpg.method.name("example").dotCfg.l) { case List(dotStr) =>
              dotStr should (
                startWith("digraph \"example\" {").and(
                  include("<(IDENTIFIER,a,if (a))<SUB>4</SUB>>")
                ).and(
                  include("<(IDENTIFIER,b,if (b))<SUB>7</SUB>>")
                ).and(
                  include("<(IDENTIFIER,c,if (c))<SUB>10</SUB>>")
                ).and(
                  endWith("}\n")
                )
              )
