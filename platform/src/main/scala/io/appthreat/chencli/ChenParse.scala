...
import io.appthreat.console.cpgcreation.{CpgGenerator, cpgGeneratorForLanguage, guessLanguage}
...
object ChenParse:
    ...
    val optionParser = new scopt.OptionParser[ParserConfig]("chen-parse"):
        ...
        note(
          s"Args specified after the $ArgsDelimitor separator will be passed to the front-end verbatim"
        )
    ...
    private def checkInputPath(config: ParserConfig): Try[Unit] =
        Try {
            if config.inputPath == "" then
                println(optionParser.usage)
                throw new AssertionError(s"Input path required")
            else if !File(config.inputPath).exists then
                throw new AssertionError(
                  s"Input path does not exist at `${config.inputPath}`, exiting."
                )
            else ()
        }
    ...
    private def getLanguage(config: ParserConfig): Try[String] =
        Try {
            if config.language.nonEmpty then
                config.language
            else
    ...
    private def parseConfig(parserArgs: Seq[String]): Try[ParserConfig] =
        Try {
            optionParser
                ...
                .getOrElse(
                  throw new RuntimeException(
                    s"Error while not parsing command line options: `${parserArgs.mkString(",")}`"
                  )
                )
        }
end ChenParse
