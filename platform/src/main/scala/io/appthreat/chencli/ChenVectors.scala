...
class BagOfPropertiesForNodes extends EmbeddingGenerator[AstNode, (String, String)]:
    ...
    override def extractObjects(cpg: Cpg): Iterator[AstNode] =
        cpg.graph.V.collect { case x: AstNode => x }
    override def enumerateSubStructures(obj: AstNode): List[(String, String)] =
        val relevantFieldTypes =
            Set(PropertyNames.NAME, PropertyNames.FULL_NAME, PropertyNames.CODE)
...
/** Creates an embedding from a code property graph by following three steps: (1) Objects are
  * extracted from the graph, each of which is ultimately to be mapped to one vector (2) For each
  * object, enumerate its sub structures. (3) Employ feature hashing to associate each sub structure
  * with a dimension. See "Pattern-based Vulnerability Discovery - Chapter 3"
  *
