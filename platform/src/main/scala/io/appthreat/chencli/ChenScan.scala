...
import better.files.*
import io.appthreat.chencli.console.ReplBridge
import io.appthreat.console.scan.{ScanPass, outputFindings}
import io.appthreat.console.{BridgeBase, DefaultArgumentProvider, ChenProduct, Query, QueryDatabase}
...
import JoernScan.getQueriesFromQueryDb
import Scan.{allTag, defaultTag}
...
object JoernScan extends BridgeBase {
  ...
  private def runScanPlugin(config: JoernScanConfig, frontendArgs: List[String]): Unit = {
    ...
    val shellConfig = io.appthreat.console
      .Config()
      .copy(
  ...
  private def removeQueryDatabase(): Unit = {
    ...
    val rmPluginConfig = io.appthreat.console
      .Config()
  ...
  private def addQueryDatabase(absPath: String): Unit = {
    ...
    val addPluginConfig = io.appthreat.console
      .Config()
...
object Scan {
  ...
  val description = "Chen Code Scanner"
