...
import io.appthreat.dataflowengineoss.layers.dataflows.*
import io.appthreat.dataflowengineoss.semanticsloader.Semantics
import CpgBasedTool.exitIfInvalid
import io.appthreat.x2cpg.layers.*
...
object ChenExport:
  ...
  private def parseConfig(args: Array[String]): Option[Config] =
      new scopt.OptionParser[Config]("chen-export"):
        ...
        opt[String]("repr")
            .text(
              s"representation to extract: [${Representation.values.toSeq.map(
                    _.toString.toLowerCase
                  ).sorted.mkString("|")}] - defaults to `${Representation.Cpg14}`"
            )
            .action((x, c) => c.copy(repr = Representation.withNameIgnoreCase(x)))
        opt[String]("format")
            .action((x, c) => c.copy(format = Format.withNameIgnoreCase(x)))
            .text(
              s"export format, one of [${Format.values.toSeq.map(_.toString.toLowerCase).sorted
                      .mkString("|")}] - defaults to `${Format.Dot}`"
            )
      .parse(args, Config())
  ...
  def exportCpg(
    ...
    if semantics.elements.isEmpty then
    ...
    format match
      ...
      case other =>
          throw new NotImplementedError(
            s"repr=$representation not yet supported for format=$format"
          )
  end exportCpg

  private def exportDot(
  ...
  end exportWithOdbFormat

  /** for each method in the cpg: recursively traverse all AST edges to get the subgraph of nodes
    * within this method add the method and this subgraph to the export add all edges between all of
    * these nodes to the export
    */
  ...
  private def sanitizedFileName(
    ...
    val sanitizedFilename =
        if scala.util.Properties.isWin then
          ...
          if windowsFilenameDeduplicationHelper.contains(sanitizedMethodName) then
            sanitizedFileName(
              s"${methodName}_",
              methodFilename,
              fileExtension,
          ...
          val sanitizedPath =
              if methodFilename.startsWith("/") then s"_root_/$methodFilename"
              else methodFilename
